...
// ── Clés publiques (safe côté client — jamais de clé secrète ici) ──
const NOTCHPAY_PK = import.meta.env.VITE_NOTCHPAY_PK || "pk.sAM1JD0mJGWiNlPbwwuxTVfnrz3U7tRIrvDcaeQMm7btmZEuBwNeUKLLWmRWGavgynKMGbh3WhHJUX5VeRGueBwPkjBF9aF2vI7v0v0o5iL1HL2q7TMq5TtoxnS4q";
...
// ── Supabase Client ──
// ⚠️ SÉCURITÉ : Clé publishable/anon safe côté client UNIQUEMENT si
// Row Level Security (RLS) est activé sur toutes les tables Supabase.
const SUPA_URL = import.meta.env.VITE_SUPA_URL || "https://oexzpfygeunehkcpoukv.supabase.co";
const SUPA_KEY = import.meta.env.VITE_SUPA_KEY || "sb_publishable_Lv5dex98pKdLnq1Sz_XvZQ_oz3vJaL6";
let _supa = null;
const getSupa = async () => {
...
Réponds en 2-3 phrases max. Français uniquement. Emojis africains. Sois direct et actionnable.`;
  ...
  const NAV=[{id:"dash",ic:"📊",lb:"Dashboard"},{id:"tx",ic:"💸",lb:"Transactions"},{id:"inv",ic:"🧾",lb:"Factures"},{id:"cli",ic:"👥",lb:"Clients"},{id:"stats",ic:"📈",lb:"Stats"},{id:"coach",ic:"🤖",lb:"Coach IA"},{id:"plans",ic:"💎",lb:"Plans"},{id:"ambass",ic:"🤝",lb:"Ambassadeur"},{id:"prefs",ic:"⚙️",lb:"Compte"}];
  ...
  const PgAmbassadeur=()=>{
    ...
    useEffect(()=>{
      (async()=>{
        try{
          const s=await getSupa();
          // Lire depuis table referrals
          const {data,error}=await s.from("referrals")
            .select("*")
            .eq("ambassador_code",ses.refCode||"");
          ...
          setStats({total,paid:paidRows.length,earnings,list});
        }catch(e){
          setStats({total:0,paid:0,earnings:0,list:[]});
    ...
    const copyLink=async()=>{
      ...
      if(navigator.clipboard && window.isSecureContext){
        try{
          ...
          setCopied(true);
          toast("🔗 Lien copié ! Partagez-le !");
          setTimeout(()=>setCopied(false),2500);
          return;
        }catch(e){}
      }
      // Méthode 2 : Fallback textarea robuste (mobile/PWA)
      try{
    ...
    const shareWA=()=>{
      const msg="Bonjour,\n\nJe utilise VierAfrik pour gerer mon business et je te le recommande.\n\nAvec VierAfrik tu peux :\n- Suivre tes ventes et depenses\n- Creer des factures PDF\n- Encaisser via Mobile Money\n- Avoir un Coach IA\n\nInscris-toi gratuitement :\n"+refLink+"\n\nVierAfrik - Ton business dans ton telephone.";
    ...
    return(
      <div>
        ...
        {/* Lien de parrainage */}
        <div style={{background:`linear-gradient(135deg,${T.gr}18,${T.teal}08,${T.c1})`,border:`2px solid ${T.gr}44`,borderRadius:20,padding:"1.6rem",marginBottom:16,position:"relative",overflow:"hidden"}}>
          <div style={{position:"absolute",right:-40,top:-40,width:160,height:160,borderRadius:"50%",background:`${T.gr}08`}}/>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            ...
            <button onClick={copyLink} style={{flex:1,background:"rgba(26,120,255,.12)",border:"1px solid rgba(26,120,255,.3)",borderRadius:10,padding:"11px",color:T.blue,cursor:"pointer",fontFamily:"inherit",fontWeight:700,fontSize:13}}>
              🔗 Copier le lien
            </button>
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:16}}>
          {[
            {ic:"👥",l:"Inscrits",v:stats.total,co:T.blue,hint:"Via votre lien"},
            {ic:"💎",l:"Convertis",v:stats.paid,co:T.gr,hint:"Plan payant"},
            {ic:"💰",l:"Gains",v:Math.round(stats.earnings).toLocaleString()+" FCFA",co:T.gold,hint:"À recevoir"},
          ].map(({ic,l,v,co,hint})=>(
            <div key={l} style={{background:`linear-gradient(135deg,${T.c1},${T.c2})`,border:`1px solid ${co}28`,borderRadius:16,padding:"1rem",textAlign:"center",position:"relative",overflow:"hidden"}}>
              <div style={{position:"absolute",inset:0,background:`${co}05`}}/>
              ...
              <div style={{fontSize:22,fontWeight:900,color:co,lineHeight:1}}>{loadingStats?"…":v}</div>
              <div style={{fontSize:9,color:T.sub2,marginTop:3}}>{hint}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{background:`linear-gradient(135deg,${T.c1},${T.c2})`,border:`1px solid ${T.border}`,borderRadius:16,padding:"1.4rem",marginBottom:14}}>
          <div style={{fontWeight:800,fontSize:13,marginBottom:14,display:"flex",alignItems:"center",gap:8}}>
            <span style={{background:`${T.gold}18`,border:`1px solid ${T.gold}30`,borderRadius:8,padding:"4px 8px",fontSize:14}}>💡</span>
            Comment gagner de l'argent
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {[
              {n:"1",t:"Copiez votre lien unique",d:"Chaque ambassadeur a son lien personnel — le vôtre est déjà prêt ci-dessus.",c:T.blue},
              {n:"2",t:"Partagez sur WhatsApp, Facebook, TikTok",d:"Envoyez le lien à vos contacts, dans des groupes, sur vos réseaux. Plus vous partagez, plus vous gagnez.",c:T.gr},
              {n:"3",t:"Les gens s'inscrivent gratuitement",d:"Quand quelqu'un clique votre lien et crée un compte, il est lié à vous automatiquement.",c:T.teal},
              {n:"4",t:"Ils passent à Pro ou Business — vous recevez 20%",d:"Pro (4 900 FCFA) → vous recevez 980 FCFA. Business (9 900 FCFA) → vous recevez 1 980 FCFA.",c:T.gold},
              {n:"5",t:"Paiement via Mobile Money",d:"Dès que vous atteignez 5 000 FCFA de gains, demandez votre paiement via Orange Money, MTN ou Wave.",c:T.purple},
            ].map(({n,t,d,c})=>(
              <div key={n} style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                <div style={{width:28,height:28,borderRadius:9,background:`${c}20`,border:`1px solid ${c}40`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:12,color:c,flexShrink:0,marginTop:1}}>{n}</div>
                <div>
                  <div style={{fontWeight:700,fontSize:12,color:T.text,marginBottom:2}}>{t}</div>
                  <div style={{fontSize:11,color:T.sub2,lineHeight:1.5}}>{d}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{background:`linear-gradient(135deg,${T.c1},${T.c2})`,border:`1px solid ${T.border}`,borderRadius:16,padding:"1.4rem",marginBottom:14}}>
          <div style={{fontWeight:800,fontSize:13,marginBottom:12,display:"flex",alignItems:"center",gap:8}}>
            ...
            Tableau des commissions (20%)
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:1,borderRadius:12,overflow:"hidden",border:`1px solid ${T.border}`}}>
            {[["Clients convertis","Plan Pro","Plan Business"],["5 clients","24 500 FCFA","49 500 FCFA"],["10 clients","49 000 FCFA","99 000 FCFA"],["20 clients","98 000 FCFA","198 000 FCFA"],["50 clients","245 000 FCFA","495 000 FCFA"]].map((row,ri)=>(
              row.map((cell,ci)=>(
                <div key={ri+"-"+ci} style={{padding:"10px 12px",background:ri===0?T.c3:ri%2===0?T.c2:T.c1,fontSize:ri===0?9:11,fontWeight:ri===0?700:ci===0?600:800,color:ri===0?T.sub:ci===1?T.gr:ci===2?T.gold:T.text,textTransform:ri===0?"uppercase":"none",letterSpacing:ri===0?".06em":"normal"}}>{cell}</div>
              ))
            ))}
          </div>
        </div>
        ...
        <div style={{background:"rgba(240,176,32,.06)",border:"1px solid rgba(240,176,32,.2)",borderRadius:16,padding:"1.2rem"}}>
          ...
          <Btn full v="gold" ch={"💰 Demander le paiement — "+Math.round(stats.earnings).toLocaleString()+" FCFA"} onClick={()=>{
            if(stats.earnings<5000){toast("⚠️ Minimum 5 000 FCFA requis pour demander un paiement. Continuez à parrainer !","warn");return;}
  ...
  const PMAP={dash:<PgDash/>,tx:<PgTx/>,inv:<PgInv/>,cli:<PgCli/>,stats:<PgStats/>,coach:<PgCoach/>,plans:<PgPlans/>,prefs:<PgParams/>,ambass:<PgAmbassadeur/>};
