        ...
        <div style={{marginTop:16,display:"flex",gap:7,justifyContent:"center",flexWrap:"wrap"}}>
          {Object.entries(PLANS).map(([k,p])=>(
            <div key={k} style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:12,padding:"8px 13px",textAlign:"center"}}>
              ...
              <div style={{fontWeight:900,fontSize:14,color:T.text,marginTop:1}}>{p.price===0?"Free":`${fmtPrice(p.price,"XOF")}/mo`}</div>
            </div>
          ))}
        </div>
...
const getPublicSupa = async () => {
  if (_publicSupa) return _publicSupa;
...
function Dashboard({ses,logout,updSes}){
  ...
  const fmtWithHint = (v, skipHint = false) => {
    ...
    if (isAfrican) {
      ...
      if (skipHint || n === 0) return main;
      // Hint EUR en indicatif
      const inEur = Math.round(n * FCFA_TO_EUR);
      if (inEur === 0) return main;
      return { main, hint: `~${fmtPrice(inEur, "EUR")}` };
    } else {
      // Montant converti dans la devise choisie
  ...
  const FmtHint = ({ v, size = 22, color, noHint = false }) => {
    const result = fmtWithHint(v, noHint);
    if (typeof result === "string") {
      return <span style={{ fontSize: size, fontWeight: 900, color: color || "inherit", lineHeight: 1, letterSpacing: "-.03em" }}>{result}</span>;
    }
    return (
      <span>
        <span style={{ fontSize: size, fontWeight: 900, color: color || "inherit", lineHeight: 1, letterSpacing: "-.03em" }}>{result.main}</span>
        <span style={{ fontSize: Math.max(9, size * 0.42), fontWeight: 500, color: T.sub, marginLeft: 4, letterSpacing: 0 }}>{result.hint}</span>
      </span>
    );
  };

  // Pages qui nécessitent un plan payant
  const PREMIUM_PAGES=["coach","carte","logo","reseau","ambass","stats"];
  const isPremiumPage=(pg)=>PREMIUM_PAGES.includes(pg);

  // Écran de blocage si abonnement expiré sur page premium
      ...
      (async()=>{
        try{
          ...
          if(trxRef){
            ...
            confirmed=true; // FedaPay contrôle le callback
          }
          if(confirmed){            // ── Enregistrer l'abonnement dans la table subscriptions ──
            const paidAt=new Date().toISOString();
            const expiresAt=new Date(Date.now()+30*24*60*60*1000).toISOString();
            // Chercher un abonnement existant pour upsert
  ...
  const canAdd=type=>{
    if(isAdmin) return true; // Admin — accès illimité
  ...
  const nextNum=()=>{
    ...
    const maxSeq=invs.reduce((max,inv)=>{
      if(inv.num&&inv.num.startsWith(prefix)){
        // Ignorer le suffixe anti-doublon éventuel (ex: VAF-2025-0003-a1b2)
    ...
    // Suffixe aléatoire court pour éviter les collisions en cas de double-clic
    const suffix=Math.random().toString(36).slice(2,5);
    return `${prefix}${String(maxSeq+1).padStart(4,"0")}-${suffix}`;
  };
  ...
  const saveInv=async()=>{
    ...
    flashNew(inv.id);
    toast(`🧾 Facture ${inv.num} en cours d'enregistrement…`);
    setMdl(null);setFm({});
  ...
  const doPay=async()=>{
    ...
    try{
      ...
      if(url){
        ...
        setInvs(next);
        await supaUpdate("invoices",{status:"pending"},inv.id);
        setMdl(null);setFm({});
