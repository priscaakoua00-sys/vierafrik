...
function Dashboard({ses,logout,updSes}){
  ...
  const notifs=[
    ...invs.filter(i=>i.status==="overdue").map(i=>({id:i.id,k:"warn",msg:(_globalLang==="fr"?`Retard : ${i.num} — ${i.clientName}`:`Overdue: ${i.num} — ${i.clientName}`)})),
    ...invs.filter(i=>i.status==="partial").map(i=>({id:"pt"+i.id,k:"info",msg:(_globalLang==="fr"?`Part. payée : ${i.num} — Reste ${fmtf(Math.max(0,i.total-(i.amtPaid||0)))}`:`Part. paid: ${i.num} — Remaining ${fmtf(Math.max(0,i.total-(i.amtPaid||0)))}`)})),
    ...(gPct>=100?[{id:"goal",k:"win",msg:(_globalLang==="fr"?"🏆 Objectif mensuel atteint !":"🏆 Monthly goal reached!")}]:[]),
    ...invs.filter(i=>i.payStatus==="paid"&&i.payRef).slice(0,1).map(i=>({id:"p"+i.id,k:"ok",msg:(_globalLang==="fr"?`Payé : ${i.num}`:`Paid: ${i.num}`)})),
  ];
