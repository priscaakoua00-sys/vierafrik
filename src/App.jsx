...
Réponds en 2-3 phrases max. Français. Emojis africains.`,
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        <div style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:14,padding:"1.2rem"}}>
          ...
          <FL l="Téléphone" ch={<input style={IS} placeholder="+225 07 000 0000" value={fm.phone!==undefined?fm.phone:ses.phone||""} onChange={ev=>{const v=ev.target.value;setFm(f=>({...f,phone:v}));}}/>}/>
          ...
          <Btn ch="💾 Sauvegarder" onClick={()=>{const g=parseInt(fm.goal!==undefined?fm.goal:goal)||2500000;const newPhone=fm.phone!==undefined?fm.phone:ses.phone||"";const newName=fm.name!==undefined?fm.name:ses.name;const newBiz=fm.biz!==undefined?fm.biz:ses.business;setGoal(g);updSes({name:newName,business:newBiz,phone:newPhone,goal:g});toast("✅ Profil mis à jour !");setFm({});}}/>
        </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Segoe UI',system-ui,sans-serif",position:"relative",overflow:"hidden"}}>
      ...
      `}</style>
      {loading&&<div style={{position:"fixed",inset:0,background:T.bg,zIndex:999,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:18}}>
        <div style={{fontSize:32}}>🌍</div>
        <div style={{width:36,height:36,border:`3px solid ${T.c3}`,borderTop:`3px solid ${T.gr}`,borderRadius:"50%",animation:"spin .8s linear infinite"}}/>
        <div style={{color:T.sub2,fontSize:13}}>Chargement de vos données…</div>
      </div>}
