...
function Dashboard({ses,logout,updSes}){
  ...
  const cMsgRef=useRef("");
  const setCMsgSync=(val)=>{cMsgRef.current=val;setCMsg(val);};
...
<script>window.onload=()=>window.print();</script></body></html>`);
  ...
  // Coach IA — moteur local, lecture via ref pour éviter closure stale
  const sendAI=useCallback(()=>{
    const msg=cMsgRef.current.trim();
    if(!msg)return;
    setCMsgSync("");
    ...
    setTimeout(()=>{
      try{
        ...
        if(q.includes("vente")||q.includes("gagner")||q.includes("argent")||q.includes("revenu")||q.includes("chiffre")){
          reply=sales>0
            ?`💰 Ce mois : ${fmtf(sales)} de ventes (${mTxs.filter(t=>t.type==="sale").length} transactions). `+(topClientEntry?`Top client : ${topClientEntry[0]} → ${fmtf(topClientEntry[1])}. Contactez-le pour une nouvelle commande 📱.`:`Continuez à enregistrer chaque vente.`)
            :`🎯 Aucune vente ce mois encore. Publiez votre service dans "Action Rapide" — vos premiers clients vous contactent directement 🌍`;
        }else if(q.includes("dépense")||q.includes("charge")||q.includes("coût")||q.includes("depense")){
          reply=exps>0
            ?`📤 Dépenses : ${fmtf(exps)}. `+(profit>=0?`✅ Rentable : ${fmtf(profit)} de bénéfice net.`:`⚠️ Déficit de ${fmtf(Math.abs(profit))}. Réduisez les charges non essentielles.`)
            :`📝 Aucune dépense enregistrée. Notez toutes vos charges pour connaître votre vraie rentabilité.`;
        }else if(q.includes("bénéfice")||q.includes("profit")||q.includes("rentable")||q.includes("benefice")){
          reply=profit>=0
            ?`📈 Bénéfice net : ${fmtf(profit)} (${gPct}% de l'objectif ${fmtf(goal)}). `+(gPct>=100?`🏆 Objectif dépassé ! Réinvestissez 20%.`:`Il reste ${fmtf(Math.max(0,goal-profit))} pour atteindre la cible.`)
            :`⚠️ Résultat : -${fmtf(Math.abs(profit))}. Action : augmenter les ventes OU réduire les dépenses. Objectif : ${fmtf(goal)}.`;
        }else if(q.includes("client")||q.includes("prospect")||q.includes("trouver")||q.includes("contact")){
          reply=activeClients>0
            ?`👥 ${activeClients} client${activeClients>1?"s":""} actif${activeClients>1?"s":""}. `+(overdueInvs>0?`Relancez d'abord vos ${overdueInvs} facture(s) en retard. `:`Tous à jour ✅. `)+`Pour de nouveaux clients : publiez dans "Action Rapide" 🚀`
            :`🌍 Allez dans "Action Rapide" pour être visible, puis enregistrez chaque contact dans "Clients".`;
        }else if(q.includes("facture")||q.includes("paiement")||q.includes("impayé")||q.includes("retard")){
          reply=overdueInvs>0
            ?`🔴 ${overdueInvs} facture${overdueInvs>1?"s":""} en retard. Allez dans "Factures" → 📲 WhatsApp → "Bonjour, petit rappel pour votre facture 🙏"`
            :invs.length>0
              ?`✅ Toutes vos factures sont à jour ! Créez une facture pour chaque nouvelle vente.`
              :`📄 Créez votre première facture depuis l'onglet "Factures".`;
        }else if(q.includes("objectif")||q.includes("goal")||q.includes("cible")){
          reply=`🎯 Objectif : ${fmtf(goal)}/mois. Actuellement à ${gPct}% (${fmtf(profit)} de bénéfice). `+(gPct>=100?`Félicitations 🏆 Rehaussez votre objectif !`:gPct>50?`Bonne trajectoire ! Encore ${fmtf(Math.max(0,goal-profit))} à réaliser.`:`Concentrez-vous sur l'augmentation des ventes quotidiennes 💪`);
        }else if(q.includes("mobile money")||q.includes("orange")||q.includes("wave")||q.includes("mtn")||q.includes("encaisser")||q.includes("payer")){
          reply=`💳 Pour encaisser : bouton "💰 Gagner" → "Encaisser" → Orange Money, MTN ou Wave. `+(invs.filter(i=>i.status!=="paid").length>0?`Vous avez ${invs.filter(i=>i.status!=="paid").length} facture(s) non payée(s). Envoyez le lien depuis "Factures" → 💳 Payer.`:`Créez une facture puis utilisez "💳 Payer" pour envoyer le lien.`);
          ...
          reply=[
            `🚀 3 priorités : 1) Relancez vos ${activeClients} clients actifs 2) Publiez dans "Action Rapide" 3) Créez une facture pour chaque vente. La régularité fait la différence 🌍`,
            `💡 Notez chaque vente dans l'app. En 30 jours vous verrez exactement quels clients et services sont les plus rentables 📊`,
            `🎯 ${activeClients>0?`Vos ${activeClients} clients actifs sont votre ressource principale. Contactez-en un aujourd'hui.`:`Publiez votre service dans "Action Rapide" et répondez vite aux contacts.`}`,
          ][idx];
        }else if(q.includes("bonjour")||q.includes("salut")||q.includes("hello")||q.includes("bonsoir")){
          reply=`Bonjour ! 👋 `+(sales>0?`Ce mois : ${fmtf(sales)} de ventes, ${fmtf(profit)} de bénéfice. `:`Prêt à vous aider ! `)+`Posez votre question sur vos ventes, clients, factures ou stratégie 🌍`;
        }else if(q.includes("merci")||q.includes("super")||q.includes("bravo")||q.includes("parfait")){
          reply=`🙏 Avec plaisir ! C'est vous qui faites le vrai travail. `+(gPct>0?`${gPct}% de l'objectif — continuez ! `:`Chaque action compte. `)+`N'hésitez pas 💪`;
        }else{
          const idx=Math.floor(Date.now()/10000)%3;
          reply=[
            `📊 Situation : ${fmtf(sales)} de ventes · ${fmtf(exps)} de dépenses · ${fmtf(profit)} de bénéfice · ${activeClients} clients actifs${overdueInvs>0?` · ${overdueInvs} facture(s) en retard`:""}. Sur quoi souhaitez-vous un conseil ?`,
            `🌍 Posez votre question sur : ventes (${fmtf(sales)}), ${activeClients} clients, factures${overdueInvs>0?` (${overdueInvs} en retard)`:""}. Je vous donne un conseil concret.`,
            `💡 ${gPct}% de l'objectif atteint. Dites-moi ce qui vous préoccupe : ventes, dépenses, clients, factures ou stratégie ?`,
          ][idx];
        }
        setChat(h=>[...h,{r:"ai",t:reply}]);
      }catch(e){
        setChat(h=>[...h,{r:"ai",t:"🌍 Je suis votre Coach VierAfrik. Posez votre question sur vos ventes, clients, factures ou stratégie."}]);
      }finally{setCL(false);}
    },600);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[clis,invs,txs,mTxs,sales,exps,profit,gPct,goal]);
  ...
  const NAV_BOTTOM=[{id:"action",ic:"⚡",lb:"Action"},{id:"dash",ic:"📊",lb:"Accueil"},{id:"inv",ic:"🧾",lb:"Factures"},{id:"cli",ic:"👥",lb:"Clients"},{id:"coach",ic:"🤖",lb:"Coach"},{id:"prefs",ic:"⚙️",lb:"Compte"}];
          ...
          <div style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:14,padding:"1rem",marginBottom:9}}>
            ...
            <div style={{display:"flex",gap:7}}>
              <input style={{...IS,flex:1,marginTop:0,fontSize:12}}
                ...
                value={cMsg}
                onChange={ev=>setCMsgSync(ev.target.value)}
            ...
            <div style={{display:"flex",gap:5,marginTop:7,flexWrap:"wrap"}}>
              {["Comment gagner plus d'argent ?","Comment trouver des clients ?","Analyse mes dépenses","Stratégie Mobile Money"].map(q=>(
                <button key={q} onClick={()=>{setCMsgSync(q);setTimeout(()=>sendAI(),50);}}
