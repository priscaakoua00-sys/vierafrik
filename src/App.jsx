...
async function seed(uid){
  ...
  const invData=[
    {id:"i1",user_id:uid,number:`VAF-${y}-0001`,client_name:"TechLagos Inc",  phone:"+234800002222",total:1850000,subtotal:1850000,tax:0,status:"paid",   issued:d(12),due:d(28),items:JSON.stringify([{id:xid(),name:"Consulting mensuel",qty:1,price:1850000,line:1850000}]),notes:""},
    {id:"i2",user_id:uid,number:`VAF-${y}-0002`,client_name:"Ama Owusu",      phone:"+233240001111",total:950000, subtotal:950000, tax:0,status:"paid",   issued:d(11),due:d(25),items:JSON.stringify([{id:xid(),name:"Produits commerce",qty:1,price:950000,line:950000}]),notes:""},
    {id:"i3",user_id:uid,number:`VAF-${y}-0003`,client_name:"StartupDakar",   phone:"+221780005555",total:1100000,subtotal:1100000,tax:0,status:"pending", issued:d(5), due:d(20),items:JSON.stringify([{id:xid(),name:"Design Sprint 3j",qty:3,price:366667,line:1100001}]),notes:""},
    {id:"i4",user_id:uid,number:`VAF-${y}-0004`,client_name:"Marché Central", phone:"+225070003333",total:675000, subtotal:675000, tax:0,status:"overdue", issued:d(1), due:d(8), items:JSON.stringify([{id:xid(),name:"Livraison alimentaire",qty:1,price:675000,line:675000}]),notes:""},
  ];
  const s=await getSupa();
  await Promise.all([
      ...
      <div style={{position:"relative",zIndex:1,width:"95%",maxWidth:430}}>
        ...
        <div style={{marginTop:18,textAlign:"center"}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:`${T.gr}12`,border:`1px solid ${T.gr}33`,borderRadius:30,padding:"8px 18px"}}>
            ...
            <span style={{fontSize:12,color:T.sub2}}>entrepreneurs utilisent déjà VierAfrik</span>
          </div>
        </div>
...
const getPublicSupa = async () => {
  ...
  console.log("🔑 [PublicPayPage] Clé Supabase:", key ? key.slice(0,20)+"…" : "VIDE ⚠️");
  const { createClient } = await import('https://esm.sh/@supabase/supabase-js@2');
...
function PublicPayPage({ invoiceId }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      console.log("🔑 [PublicPayPage] Clé valide:", diagKey?.startsWith("eyJ") ? "OUI ✅" : "NON ❌ — clé vide ou invalide");
      console.log("🔍 [PublicPayPage] Invoice ID:", invoiceId);
      try {
        ...
        console.log("🔍 [PublicPayPage] SELECT by id →", { data: r1.data, error: r1.error });
        ...
        if (!data && !rlsBlocked) {
          ...
          console.log("🔍 [PublicPayPage] SELECT by number →", { data: r2.data, error: r2.error });
          if (!r2.error && r2.data) {
            data = r2.data;
            console.log("✅ [PublicPayPage] Trouvée via number:", invoiceId);
          }
          if (r2.error) {
        ...
        if (!data && rlsBlocked) {
          ...
          try {
            ...
            if (!r3.error && r3.data) {
              ...
              console.log("✅ [PublicPayPage] Trouvée via RPC:", data?.id);
              rlsBlocked = false;
            }
          } catch(rpcErr) {
        ...
        if (!data) {
          ...
          try {
            ...
            if (r4.ok) {
              ...
              if (json4?.id) {
                data = json4;
                console.log("✅ [PublicPayPage] Trouvée via /api/invoice:", data?.id);
              } else {
...
function Dashboard({ses,logout,updSes}){
        ...
        try{
          ...
          if(trxRef){
            try{
              ...
              console.log("✅ [FedaPay] Vérification retour:", verifData);
  ...
  const saveTx=async()=>{
    ...
    const fmSnap = {...fm};
    console.log("📝 saveTx démarré", {amount:fmSnap.amount, type:fmSnap.type, who:fmSnap.who});
    try {
      ...
      console.log("💾 INSERT transaction résultat:", txOk);
      ...
      if (txObj.type === "sale" && txObj.who) {
        ...
        if (!existing) {
          ...
          if (canAdd("cli")) {
            ...
            console.log("💾 INSERT client résultat:", cliOk);
        ...
        if (canAdd("inv")) {
          ...
          // INSERT Supabase
          console.log("💾 INSERT facture auto...");
          ...
          console.log("💾 INSERT facture résultat:", invOk);
  ...
  const saveCli=async()=>{
    ...
    if(snap._edit){
      ...
      console.log("💾 INSERT client:", c.name);
      ...
      console.log("💾 INSERT client résultat:", ok?"OK":"ERREUR");
      if(!ok){
  ...
  const saveInv=async()=>{
    ...
    console.log("💾 INSERT facture:", inv.num, "| UUID:", inv.id);
    ...
    console.log("💾 INSERT facture résultat:", ok?"✅ OK — UUID en base: "+inv.id:"❌ ERREUR — QR code invalide !");
    if(!ok){
      // Rollback si INSERT échoué
    ...
    try{
      ...
      if(fresh&&fresh.length>0){
        ...
        console.log("✅ Factures re-fetchées:", fresh.length);
      }
...
<script>window.onload=()=>window.print();</script></body></html>`);
  ...
  const doPay=async()=>{
    ...
    try{
      ...
      const data=await res.json();
      console.log("✅ [FedaPay] Réponse paiement facture:", data);
      const url=data?.payment_url||data?.url||data?.transaction?.payment_url;
      const txId=data?.transaction_id||data?.transaction?.id;
      if(url){
        console.log("✅ [FedaPay] transaction_id:", txId);
  ...
  const PgEmployees = () => {
    ...
    const addEmployee = async () => {
      ...
      // Limite plan Free
      if (!isAdmin && activePlan === "free" && emps.length >= 1) {
        ...
        {emps.length > 0 && (
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:16 }}>
            {[
              ...
              { l:"Masse salariale", v: fmtAmt(emps.reduce((s,e)=>s+e.salary,0), emps[0]?.currency||""), co: T.orange, ic:"💰" },
          ...
          {lastReceipt && (() => {
            ...
            return (
              <div style={{ textAlign:"center" }}>
                ...
                {/* ID reçu */}
                <div style={{ fontSize:11, color:T.sub, marginBottom:18, fontWeight:600 }}>
                  🧾 Reçu #{lastReceipt.receiptId}
                </div>
                {/* Bloc lignes */}
                <div style={{ background:`linear-gradient(135deg,${T.c2},${T.c3})`, border:`1px solid ${T.gr}22`, borderRadius:16, padding:"1rem", marginBottom:14, textAlign:"left" }}>
                  {[
                    ...
                    { icon:"💰", label:"Montant",   val: fmtReceiptAmt(lastReceipt.amount, lastReceipt.currency), green:true },
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <button onClick={()=>{
                    navigator.clipboard.writeText(copyText).then(()=>toast("📋 Reçu copié !","ok",T.gr)).catch(()=>toast("❌ Copie échouée","err"));
                  }} style={{ flex:1, padding:"11px", background:T.c3, border:`1px solid ${T.border}`, borderRadius:11, color:T.sub2, cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:700 }}>
                    📋 Copier
                  </button>
                  <button onClick={()=>{setMdlEmp(null);setLastReceipt(null);}} style={{ flex:1, padding:"11px", background:T.gr, border:"none", borderRadius:11, color:T.ink, cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:900 }}>
                    ✅ Fermer
                  </button>
                </div>
              </div>
  ...
  const PgPlans=()=>{
    ...
    const doSubPayment=async(planKey,planObj)=>{
      ...
      try{
        ...
        const data=await res.json();
        console.log("✅ [FedaPay] Plan payment:",data);
        const url=data?.payment_url||data?.url||data?.transaction?.payment_url;
      ...
      <Modal open={mdl==="mm"} onClose={()=>{setMdl(null);setFm({});}} title={t("mmTitle")}>
        <div style={{marginBottom:10}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
            {[
              {id:"cinetpay",  emoji:"🟠",label:"CinetPay",    desc:"CI · SN · CM · BF", col:"#FF6600"},
              {id:"paystack",  emoji:"🟢",label:"Paystack",    desc:"NG · GH · ZA · KE", col:"#00C300"},
              {id:"flutterwave",emoji:"🔵",label:"Flutterwave",desc:"20+ pays Afrique",  col:"#F5A623"},
          ...
          <Btn full ch={fm.prov?`💳 ${fm.prov.toUpperCase()}${fm.amount?" — "+fmtPrice(parseFloat(fm.amount)||0, DEFAULT_CURRENCY):""}` : t("chooseOperator")} onClick={async()=>{
            ...
            try{
              ...
              const data=await res.json();
              console.log("✅ [FedaPay] Mobile Money:", data);
              const url=data?.payment_url||data?.url||data?.transaction?.payment_url;
              const txId=data?.transaction_id||data?.transaction?.id;
              if(url){
                console.log("✅ [FedaPay] transaction_id:", txId);
