  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // ── TENTATIVE 1 : recherche par UUID (id exact) ──
        const r1 = await s.from("invoices").select("*").eq("id", invoiceId).maybeSingle();

        if (r1.error) {
          // RLS bloque probablement la lecture publique
          console.error("❌ [PublicPayPage] Erreur SELECT par id:", r1.error);
          // On tente quand même par number au cas où
        } else {
          data = r1.data;
        }

        // ── TENTATIVE 2 : si id non trouvé, recherche par numéro (ex: VAF-2026-0005) ──
        // Couvre le cas où le QR contient le numéro au lieu de l'UUID
        if (!data) {
          const r2 = await s.from("invoices").select("*").eq("number", invoiceId).maybeSingle();
          if (!r2.error && r2.data) {
            data = r2.data;
            console.log("✅ [PublicPayPage] Facture trouvée via number:", invoiceId);
          }
        }

        if (!data) {
          // Diagnostic : on vérifie si c'est un problème RLS ou vraiment absent
          const errMsg = r1.error?.code === "42501" || r1.error?.message?.includes("row-level security")
            ? "Accès refusé (RLS). Configurez une policy SELECT publique sur la table invoices dans Supabase."
            : "Cette facture n'existe pas ou a été supprimée.";
          console.error("❌ [PublicPayPage] Facture introuvable pour:", invoiceId, r1.error);
          setError(errMsg);
          setLoading(false);
          return;
        }
      ...
      <div style={{ maxWidth:480, margin:"0 auto", padding:"20px 16px" }}>
        ...
        {error && (
          <div style={{ textAlign:"center", padding:"40px 16px" }}>
            ...
            <div style={{ color:"#80a8c8", fontSize:13, marginBottom:16, lineHeight:1.6 }}>{error}</div>
            {error.includes("RLS") && (
              <div style={{ background:"rgba(255,90,24,0.1)", border:"1px solid rgba(255,90,24,0.3)", borderRadius:12, padding:14, textAlign:"left", fontSize:11, color:"#ff9060", lineHeight:1.7 }}>
                <div style={{ fontWeight:800, marginBottom:6 }}>🔧 Fix Supabase requis :</div>
                <div>Dashboard → Table Editor → <strong>invoices</strong></div>
                <div>→ Auth → Policies → New Policy</div>
                <div>→ Nom : <code>public_read_invoices</code></div>
                <div>→ Operation : <strong>SELECT</strong></div>
                <div>→ Target roles : <strong>anon</strong></div>
...
function LogoGenerator({ user, accent = "#00d478", toast }) {
  ...
  const PgSmartQR = () => <SmartQRPage user={ses} />;
...
function CommerçantsProches({ user, supabase, accent="#00d478", toast, onAddClient, onCreateInvoice, onPayment }) {
  ...
  const renderActivePage = () => {
    ...
    if(!subLoading && isPremiumPage(page) && !isSubActive){
