...
function CommerçantsProches({ user, supabase, accent="#00d478", toast, onAddClient, onCreateInvoice, onPayment }) {
  ...
  function PgActionRapide(){
    ...
    const startRecording=async()=>{
      try{
        ...
        mediaRecRef.current.onstop=()=>{
          ...
          setAudioBlob(b);
          setAudioBlobUrl(URL.createObjectURL(b));
        };
    ...
    const resetAR=()=>{setScreen(1);setSelCat(null);setMode(null);setPropForm({city:"",phone:"",desc:"",imageUrl:"",imageFile:null});setPropPreview(null);setPublished(false);setAudioBlob(null);setAudioBlobUrl(null);};
    ...
    if(screen===3&&mode==="propose"){
      if(published) return(
        <div style={{maxWidth:460,margin:"0 auto",textAlign:"center",animation:"slideUp .3s ease both"}}>
          ...
          {audioBlob&&audioBlobUrl&&<audio controls src={audioBlobUrl} style={{width:"100%",borderRadius:8,marginBottom:16}}/>}
      ...
      return(
        <div style={{maxWidth:480,margin:"0 auto",animation:"slideUp .3s ease both"}}>
          ...
          <div style={{marginBottom:13}}>
            ...
            <input id="photo-upload" type="file" accept="image/*" capture="environment" onChange={handleImage} style={{display:"none"}}/>
            {propPreview ? (
              <div style={{position:"relative"}}>
    ...
    if(screen===3&&mode==="search") return(
      <div style={{animation:"slideUp .3s ease both"}}>
        ...
        {audioBlob&&audioBlobUrl&&!recording&&<audio controls src={audioBlobUrl} style={{width:"100%",borderRadius:8,marginTop:12}}/>}
      </div>
    );
