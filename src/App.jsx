...
<script>window.onload=()=>window.print();</script></body></html>`);
  ...
  const CarteVisite = React.memo(function CarteVisite({ user, accent = "#00d478", toast }) {
    ...
    const [saving, setSaving] = useState(false);
    const [styleIdx, setStyleIdx] = useState(0);
    const [form, setForm] = useState({ nom:user?.name||"", business:user?.business||"", phone:user?.phone||"", activite:"", ville:"", couleur:accent, couleur2:"#00bfcc" });
    const cardRef = useRef(null);
    const STYLES_C = [{label:"Sombre Pro",bg:`linear-gradient(135deg,#0a1628,#0d1f3c)`},{label:"Vert Afrik",bg:`linear-gradient(135deg,#021a0e,#0d2e1a)`},{label:"Or Premium",bg:`linear-gradient(135deg,#1a1200,#2a1e00)`},{label:"Violet",bg:`linear-gradient(135deg,#100820,#1a0e30)`}];
    useEffect(()=>{
      (async()=>{try{const s=await getSupa();const{data}=await s.from("cartes_visite").select("*").eq("user_id",user.id).maybeSingle();if(data){setCarte(data);setForm({nom:data.nom||user?.name||"",business:data.business||user?.business||"",phone:data.phone||user?.phone||"",activite:data.activite||"",ville:data.ville||"",couleur:data.couleur||accent,couleur2:data.couleur2||"#00bfcc"});setStyleIdx(data.style_idx||0);}}catch(e){}setLoading(false);})();
    },[user.id]);
    const initials=(form.business||form.nom||"VA").split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase();
    const saveCarte=async()=>{if(!form.nom||!form.business){toast("⚠️ Nom et business requis","err");return;}setSaving(true);try{const s=await getSupa();const payload={...form,style_idx:styleIdx,user_id:user.id,updated_at:new Date().toISOString()};if(carte?.id){await s.from("cartes_visite").update(payload).eq("id",carte.id);setCarte({...carte,...payload});}else{const{data}=await s.from("cartes_visite").insert({...payload,created_at:new Date().toISOString()}).select().single();setCarte(data);}toast("✅ Carte sauvegardée !");}catch(e){toast("❌ Erreur","err");}setSaving(false);};
    ...
    return(
      <div>
        ...
        <div ref={cardRef} style={{background:STYLES_C[styleIdx].bg,border:`2px solid ${form.couleur}55`,borderRadius:20,padding:"1.4rem",marginBottom:14,boxShadow:`0 20px 60px rgba(0,0,0,.7)`,position:"relative",overflow:"hidden",minHeight:160}}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}>
            ...
            <div><div style={{fontWeight:900,fontSize:17,color:"#fff"}}>{form.business||"Business"}</div><div style={{fontSize:12,color:form.couleur,fontWeight:600,marginTop:2}}>{form.activite||"Activité"}</div></div>
          </div>
        ...
        <div style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:14,padding:"1rem",marginBottom:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {[["Nom complet","nom","text","Prénom Nom"],["Business","business","text","Nom entreprise"],["Téléphone","phone","tel","+225 07 000 0000"],["Activité","activite","text","Coiffure, Commerce..."],["Ville","ville","text","Abidjan, Dakar..."]].map(([label,key,type,ph])=>(
              <div key={key}><div style={{fontSize:9,fontWeight:700,color:T.sub,textTransform:"uppercase",marginBottom:3}}>{label}</div><input type={type} placeholder={ph} value={form[key]||""} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))} style={{width:"100%",padding:"7px 9px",background:T.c2,border:`1px solid ${T.border}`,borderRadius:7,color:T.text,fontSize:12,fontFamily:"inherit"}}/></div>
            ))}
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <button onClick={saveCarte} disabled={saving} style={{padding:"11px",borderRadius:10,border:"none",background:saving?T.c3:`linear-gradient(135deg,${form.couleur},${form.couleur2})`,color:"#000",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>{saving?"⏳...":"💾 Sauvegarder"}</button>
          <button onClick={()=>{const txt=`📇 *${form.business}*\n👤 ${form.nom}\n📱 ${form.phone}${form.ville?"\n📍 "+form.ville:""}\n🌍 vierafrik.com`;window.open("https://wa.me/?text="+encodeURIComponent(txt),"_blank");}} style={{padding:"11px",borderRadius:10,border:"none",background:"#25D366",color:"#000",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>💬 WhatsApp</button>
        </div>
      </div>
    );
  });
