    ...
    if(screen===1) return(
      <div style={{animation:"slideUp .3s ease both"}}>
        ...
        <div className="ar-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:8}}>
          {AR_CATS.map(cat=>(
            <div key={cat.id} onClick={(e)=>{e.stopPropagation();e.preventDefault();setSelCat(cat.id);setScreen(2);}}
            ...
            {posts.map(p=>{
              ...
              return(
                <div key={p.id} style={{background:`linear-gradient(135deg,${T.c1},${T.c2})`,border:`1px solid ${T.border}`,borderRadius:16,overflow:"hidden",marginBottom:12}}>
                  {p.image_url&&<img src={p.image_url} alt="service" onClick={(e)=>{e.stopPropagation();e.preventDefault();}} style={{width:"100%",height:155,objectFit:"cover",display:"block",pointerEvents:"none"}}/>}
