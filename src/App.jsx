...
// FedaPay — clé publique live (la clé secrète est côté serveur dans /api/fedapay)
// NE PAS mettre la clé secrète ici — elle est dans les variables d'environnement Vercel
...
// Résultats mémoïsés au niveau module — calculés une seule fois au chargement
const _isAfrica = (() => {
  try {
...
const detectIsAfrica = () => _isAfrica;
const detectCurrency = () => _currency;
...
// fmtPriceShort = alias de fmtPrice (doublon supprimé)
...
// SUPA_KEY: snapshot initial uniquement pour l'affichage dans le panel diagnostic
// getSupa() relit toujours getSupaKey() dynamiquement
...
async function seed(uid){
  ...
  const s=await getSupa();
  // Guard anti-doublon — ne seed que si l'utilisateur n'a aucune transaction
  const { data: existing } = await s.from("transactions").select("id").eq("user_id", uid).limit(1);
  if(existing && existing.length > 0) return; // déjà seedé
  await Promise.all([
      ...
      if(tab==="signup"){
        ...
        if(error){
          if(error.message.includes("already registered")||error.message.includes("already exists")){
            setE({email:"Cet email est déjà utilisé — veuillez vous connecter"});
          } else {
...
// ════════════════════════════════════════════════════════
function PublicPayPage({ invoiceId, freeAmount, freeDesc, uid }) {
  const isFreeMode = !invoiceId && !!freeAmount;
  const [inv, setInv] = useState(null);
  const [loading, setLoading] = useState(!isFreeMode);
  ...
  useEffect(() => {
    if (isFreeMode) return; // mode montant libre — pas de fetch facture
    (async () => {
      try {
        const s = await getSupa();
  ...
  const doPay = async () => {
    ...
    if (!isFreeMode && (!inv?.total || inv.total <= 0)) { showToast("⚠️ Montant invalide", "err"); return; }
    if (isFreeMode && (!freeAmount || freeAmount <= 0)) { showToast("⚠️ Montant QR invalide", "err"); return; }
    setPaying(true);
    showToast("⏳ Création du paiement…");
    try {
      // Montant et description selon le mode
      const payAmt  = isFreeMode ? freeAmount : (inv.total - (inv.amtPaid||0));
      const payDesc = isFreeMode ? (freeDesc || "Paiement VierAfrik") : `Facture ${inv?.num||invoiceId}`;
      const payInvId = isFreeMode ? null : inv?.id;
      const callback = window.location.origin + "/?pay_success=1" +
        (payInvId ? "&invoice_id=" + payInvId : "") +
        (uid ? "&uid=" + uid : "");
      ...
      const res = await fetch("/api/fedapay", {
        ...
        body: JSON.stringify({
          ...
          amount:         payAmt,
          description:    payDesc,
          customer_email: "client@vierafrik.com",
  ...
  const reste  = isFreeMode ? freeAmount : (inv ? Math.max(0, inv.total - (inv.amtPaid||0)) : 0);
  const isPaid = !isFreeMode && (inv?.status === "paid" || inv?.payStatus === "paid" || (inv && reste <= 0));
...
function SmartQRPage({ user }) {
  ...
  // Forms
  const [payForm,     setPayForm]     = useState({ _type:"invoice", invoiceId:"", amount:"", desc:"" });
  ...
  useEffect(() => {
    ...
    if (mode === "payment") {
      if (payForm._type === "invoice" && payForm.invoiceId)
        ...
        val = `${base}/?pay_amount=${payForm.amount}&pay_desc=${encodeURIComponent(payForm.desc||"")}&uid=${user?.id||""}`;
    }
    if (mode === "contact") {
  ...
  const handleSave = async () => {
    ...
    const entry = {
      ...
      mode,
      value: qrValue,
      label: mode==="payment" ? `Paiement ${payForm.amount ? fmt(parseFloat(payForm.amount))+" FCFA" : payForm.invoiceId}`
          ...
          {mode === "payment" && (
            <div style={{ background:"#05090f", border:"1px solid rgba(0,210,120,0.08)", borderRadius:16, padding:"1.2rem" }}>
              ...
              {payForm._type === "invoice" ? (
                <div>
                  <label style={{ fontSize:10, fontWeight:700, color:"#4a7090", display:"block", marginBottom:4 }}>ID Facture</label>
                  <input style={IS} placeholder="VAF-2026-0001"
                    value={payForm.invoiceId}
                    onChange={e => setPayForm(f => ({ ...f, invoiceId:e.target.value }))}/>
                  {payForm.invoiceId && <div style={{ fontSize:9, color:"#4a7090", marginTop:4, fontFamily:"monospace" }}>
                    → vierafrik.com/?pay={payForm.invoiceId}
                  </div>}
                </div>
              ) : (
                <>
...
export default function App(){
  ...
  const payInvId  = _urlP.get("pay");
  const payAmount = _urlP.get("pay_amount");
  const payDesc   = _urlP.get("pay_desc");
  const payUid    = _urlP.get("uid");
  if(payInvId)  return <PublicPayPage invoiceId={payInvId}/>;
  if(payAmount) return <PublicPayPage freeAmount={parseFloat(payAmount)} freeDesc={decodeURIComponent(payDesc||"")} uid={payUid}/>;
...
function SupaDiagPanel({ uid, userEmail }) {
  ...
  const ADMIN_EMAIL = "priscaakoua00@gmail.com";
...
// ════════════════════════════════════════════════════════
// 🪪 CARTE DE VISITE — défini hors Dashboard pour éviter les remounts
// ════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════
// 🔗 IMPORT ANNONCE — défini hors Dashboard
// ════════════════════════════════════════════════════════
...
function Dashboard({ses,logout,updSes}){
  ...
  useEffect(()=>{
    ...
    const trxRef=params.get("id")||params.get("transaction_id")||params.get("reference");
    ...
    if(paySuccess==="1"&&plan&&uid&&ses?.id&&uid===ses.id){
      (async()=>{
        try{
          const s=await getSupa();
          // Vérifier le statut via webhook (déjà géré côté serveur)
          // Fallback : on fait confiance au retour FedaPay avec trxRef
          let confirmed=false;
          if(trxRef){
            try{
              const verif=await fetch("/api/fedapay",{
                method:"POST",
                headers:{"Content-Type":"application/json"},
                body:JSON.stringify({action:"verify",transaction_id:trxRef})
              });
              const verifData=await verif.json();
              const status=verifData?.transaction?.status||verifData?.status;
              confirmed=(status==="approved"||status==="complete"||status==="paid");
            }catch(ve){
              confirmed=true; // Fallback si vérification échoue
            }
          } else {
            confirmed=true; // FedaPay callback = paiement confirmé
          }
          if(confirmed){
...
<script>window.onload=()=>window.print();</script></body></html>`);
    ...
    try{
      const res=await fetch("/api/fedapay",{
        ...
        body:JSON.stringify({
          action:"initialize",
          amount:inv.total,
          description:"Paiement facture "+inv.num+" — "+inv.clientName,
          customer_email:ses.email,
          customer_phone:phone,
          invoice_id:inv.id,
          uid:ses.id,
          callback_url:window.location.origin+"/?pay_success=1&plan=invoice&uid="+ses.id+"&invoice_id="+inv.id,
        })
      });
      const data=await res.json();
      const url=data?.payment_url||data?.url;
  ...
  const FloatingBtns=()=>{
    ...
    useEffect(()=>{
      const move=(e)=>onDragMoveRef.current(e);
      const end=(e)=>onDragEndRef.current(e);
    ...
    },[]);// ✅ stable — refs gèrent le changement de handlers
            ...
            <div key={k} style={{background:T.c1,border:`2px solid ${isCur?p.col:T.border}`,borderRadius:18,padding:"1.4rem",position:"relative",overflow:"hidden",transition:"all .2s"}}
              ...
              {!isCur&&<Btn full sx={{marginTop:15}} v={k==="business"?"gold":k==="pro"?"p":"g"} ch={`Passer à ${p.label} →`} onClick={async()=>{
                    ...
                    try{
                      ...
                      else{toast("❌ "+(data?.message||data?.error||"Erreur paiement — réessayez"),"err");console.error("FedaPay:",data);}
            ...
            try{
              const res=await fetch("/api/fedapay",{
                ...
                body:JSON.stringify({
                  ...
                  customer_email:ses.email,
                  customer_phone:fm.phone,
                  uid:ses.id,
                  plan:"mm_"+fm.prov,
                  callback_url:window.location.origin+"/?pay_success=1&plan=mm_"+fm.prov+"&uid="+ses.id,
                })
              });
              const data=await res.json();
              const url=data?.payment_url||data?.url;
