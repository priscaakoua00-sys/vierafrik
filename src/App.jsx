...
function Dashboard({ses,logout,updSes}){
  ...
  const [profile,setProfile]=useState({
    ...
    country: ses.country||"CI",
  });
  const chatRef=useRef(null);

  // Sync profile state when session updates (after refresh)
  useEffect(()=>{
    setProfile({name:ses.name||"",biz:ses.business||"",phone:ses.phone||"",goal:ses.goal||2500000,country:ses.country||"CI"});
...
Réponds en 2-3 phrases max. Français uniquement. Emojis africains. Sois direct et actionnable.`;
          ...
          {invs.map(inv=>(
            <div key={inv.id} style={{background:T.c2,border:`1px solid ${T.border}`,borderRadius:15,padding:"1rem",position:"relative",overflow:"hidden",transition:"all .2s",animation:flashId===inv.id?"flashGreen .7s ease":"none"}}
              ...
              <div style={{display:"flex",gap:3,flexWrap:"wrap",marginBottom:4}}>
                ...
                {inv.status!=="paid"&&<button onClick={async()=>{try{const n=invs.map(x=>x.id===inv.id?{...x,status:"paid",payStatus:"paid"}:x);setInvs(n);await supaUpdate("invoices",{status:"paid",pay_status:"paid"},inv.id);toast("✅ Marquée payée !");}catch(e){toast("❌ Erreur — réessayez","err");}}} style={{flex:1,background:"rgba(0,212,120,.1)",border:"none",color:T.gr,borderRadius:7,padding:"5px",cursor:"pointer",fontSize:10,fontWeight:700}}>✅</button>}
              </div>
  ...
  const PgParams=()=>{
    ...
    const handleSave=async()=>{
      ...
      setGoal(g);
      await updSes({name:profile.name,business:profile.biz,phone:profile.phone,goal:g,country:profile.country||ses.country||"CI"});
      setSaving(false);
      setSavedAt(new Date());
...
function CommerçantsProches({ user, supabase, accent="#00d478", toast, onAddClient, onCreateInvoice, onPayment }) {
  ...
  const PMAP=useMemo(()=>({dash:<PgDash/>,tx:<PgTx/>,inv:<PgInv/>,cli:<PgCli/>,stats:<PgStats/>,coach:<PgCoach/>,plans:<PgPlans/>,prefs:<PgParams/>,ambass:<PgAmbassadeur/>,avis:<PgAvis/>,action:<PgActionRapide/>,reseau:<PgCommProches/>,carte:<PgCarteVisite/>,logo:<PgLogo/>}),[page]);
