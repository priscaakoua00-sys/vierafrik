  ...
  const go=async ()=>{
    ...
    try{
      ...
      if(tab==="signup"){
        ...
        // Seed uniquement si nouveau compte (pas de données existantes)
        const sSeedKey=`seed_done_${u.id}`;
        if(!sessionStorage.getItem(sSeedKey)){
          sessionStorage.setItem(sSeedKey,"1");
          await seed(u.id);
        }
        onLogin(u);
        return;
  ...
  useEffect(()=>{
    ...
    const loadAll=async()=>{
      ...
      try {
        ...
        // ── Seed démo uniquement si compte vraiment vide (anti-écrasement) ──
        const seedKey=`seed_done_${uid}`;
        const alreadySeeded=sessionStorage.getItem(seedKey);
        if(rawTxs.length===0&&rawClis.length===0&&rawInvs.length===0&&!alreadySeeded){
...
Réponds en 2-3 phrases max. Français uniquement. Emojis africains. Sois direct et actionnable.`;
  ...
  const NAV_BOTTOM=[{id:"action",ic:"⚡",lb:"Action"},{id:"dash",ic:"📊",lb:"Accueil"},{id:"tx",ic:"💸",lb:"Ventes"},{id:"inv",ic:"🧾",lb:"Factures"},{id:"cli",ic:"👥",lb:"Clients"},{id:"reseau",ic:"🗺️",lb:"Réseau"},{id:"coach",ic:"🎥",lb:"Coach"},{id:"prefs",ic:"⚙️",lb:"Compte"}];
...
function CommerçantsProches({ user, supabase, accent="#00d478", toast, onAddClient, onCreateInvoice, onPayment }) {
  ...
  // ── Écran édition profil — rendu inline (évite re-mount et perte de focus) ──
  const profilePanelEl = (
    <div style={{ background:`linear-gradient(135deg,${accent}10,${Tc.c1})`, border:`2px solid ${accent}44`, borderRadius:20, padding:"1.4rem", marginBottom:20 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:editOpen?16:0 }}>
        <div>
          <div style={{ fontWeight:900, fontSize:15, color:Tc.text }}>
            {myProfile ? "✅ Mon profil est visible" : "Fais connaître ton business 🔥"}
          </div>
          {!myProfile && !editOpen && (
            <div style={{ fontSize:11, color:Tc.sub2, marginTop:3 }}>
              +1 200 entrepreneurs utilisent déjà VierAfrik pour trouver des clients
            </div>
          )}
        </div>
        <button onClick={()=>setEditOpen(o=>!o)} style={{ background:editOpen?`${accent}20`:Tc.c2, border:`1px solid ${accent}44`, borderRadius:9, cursor:"pointer", color:accent, fontSize:11, fontWeight:700, padding:"6px 14px", fontFamily:"inherit", flexShrink:0, marginLeft:8 }}>
        ...
        <div>
          <label style={{ display:"block", cursor:"pointer", marginBottom:12 }}>
            ...
            {previewImg||editForm.image_url ? (
              ...
              <div style={{ position:"relative", width:"100%", height:130, borderRadius:14, overflow:"hidden", border:`2px dashed ${accent}44` }}>
                <img src="https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=600&q=70" alt="exemple"
                  style={{ width:"100%", height:"100%", objectFit:"cover", opacity:.4 }}
                  onError={e=>e.target.style.display="none"}/>
                <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:6 }}>
                  <span style={{ fontSize:28 }}>📷</span>
                  <span style={{ fontSize:12, fontWeight:700, color:"#fff", textShadow:"0 1px 4px rgba(0,0,0,.8)" }}>Ajouter une photo</span>
                </div>
              </div>
            )}
          </label>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
            <div>
              <label style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", color:Tc.sub, display:"block", marginBottom:2 }}>Quel service proposes-tu ?</label>
          ...
          {!myProfile && (
            <div style={{ textAlign:"center", fontSize:11, color:accent, fontWeight:700, marginBottom:10 }}>
              🔥 Inscription gratuite (0 FCFA)
            </div>
          )}
          {saving && (
            <div style={{ textAlign:"center", fontSize:12, color:accent, fontWeight:600, marginBottom:10, animation:"pulse .8s infinite" }}>
              ⏳ Ton business va être visible dans quelques secondes…
            </div>
          )}
          <button onClick={saveProfile} disabled={saving} style={{ width:"100%", padding:"13px", borderRadius:12, border:"none", background:saving?Tc.c3:`linear-gradient(135deg,${accent},${Tc.teal})`, color:saving?Tc.sub:Tc.ink, fontFamily:"inherit", fontWeight:900, fontSize:14, cursor:saving?"not-allowed":"pointer", boxShadow:saving?"none":`0 6px 20px ${accent}44`, letterSpacing:"-.01em" }}>
            {saving
              ? <span style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}><span style={{ width:14, height:14, border:"2px solid rgba(0,0,0,.3)", borderTop:"2px solid #000", borderRadius:"50%", display:"inline-block", animation:"spin .7s linear infinite" }}/> En cours…</span>
              : myProfile ? "💾 Mettre à jour" : "Créer mon profil maintenant 🚀"
            }
          </button>
        </div>
      )}
    ...
    if(screen===3&&mode==="propose"){
      ...
      return(
        <div style={{maxWidth:480,margin:"0 auto",animation:"slideUp .3s ease both"}}>
          ...
          {/* ── INPUT FILE — isolé hors de tout bloc cliquable ── */}
          <input id="photo-upload" type="file" accept="image/*" onChange={handleImage} style={{display:"none",position:"fixed",top:-9999,left:-9999}}/>
          {/* Photo */}
          ...
          {/* Images professionnelles — sélection directe, AUCUN input file, AUCUNE navigation */}
          {!propPreview && catObj && (
            <div style={{marginBottom:13}} onClick={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()}>
                ...
                {[
                  ...
                  return(
                    <div key={i}
                      ...
                      tabIndex={0}
                      onClick={selectImg}
                      onTouchEnd={selectImg}
                      style={{borderRadius:10,overflow:"hidden",cursor:"pointer",border:"2px solid transparent",transition:"border .2s",WebkitTapHighlightColor:"transparent",userSelect:"none"}}>
                      <img src={img} alt=""
