...
const I18N = {
  fr: {
    ...
    navAvis:"⭐ Avis", navPrefs:"⚙️ Compte", navEmp:"👷 Employés",
    // Employees module
    empTitle:"👷 Employés & Paiements",
    empSub:"Gérez votre équipe · Payez en 1 clic",
    newEmp:"+ Ajouter un employé",
    empName:"Nom complet *", empPhone:"Téléphone (optionnel)",
    empSalary:"Salaire *", empCurrency:"Devise",
    empAdded:"✅ Employé ajouté !",
    empEdited:"✅ Employé modifié !",
    ...
    securePay:"🔒 Paiement sécurisé via FedaPay · SSL",
...
const FEDAPAY_PK = (typeof globalThis !== "undefined" && globalThis.__VITE_ENV__?.VITE_FEDAPAY_PK)
  || (typeof window !== "undefined" && window.__ENV__?.VITE_FEDAPAY_PK)
  || "";
...
const fmtPrice = (amount, currency) => {
  ...
  // XOF/XAF n'ont pas de décimales
  const noDecimal = ["XOF","XAF","JPY","KRW","VND"].includes(cur);
  try {
    ...
    return `${new Intl.NumberFormat("fr-FR").format(Math.round(amount || 0))} ${cur}`;
  }
};
...
const MM=[
  {id:"cinetpay",  label:"CinetPay",     emoji:"🟠", desc:"CI · SN · CM · BF", col:"#FF6600"},
  {id:"wave",      label:"Wave",          emoji:"🌊", desc:"CI · SN · ML",      col:"#1A9BDB"},
  {id:"mtn",       label:"MTN MoMo",      emoji:"🟡", desc:"CI · GH · CM",      col:"#FFCC00"},
  {id:"orange",    label:"Orange Money",  emoji:"🔶", desc:"CI · SN · ML",      col:"#FF6600"},
  {id:"paystack",  label:"Paystack",      emoji:"🟢", desc:"NG · GH · ZA · KE", col:"#00C300"},
  {id:"flutterwave",label:"Flutterwave",  emoji:"🔵", desc:"20+ pays Afrique",  col:"#F5A623"},
];
...
const _getEnvKey = (name) => {
  if (typeof globalThis !== "undefined" && globalThis.__VITE_ENV__?.[name]) return globalThis.__VITE_ENV__[name];
  if (typeof window !== "undefined" && window.__ENV__?.[name]) return window.__ENV__[name];
  return "";
};
const _SUPA_KEY_BUILD = _getEnvKey("VITE_SUPABASE_ANON_KEY");
const getSupaKey = () => {
  try { return localStorage.getItem("vierafrik_supa_key") || _SUPA_KEY_BUILD; } catch(_e) { return _SUPA_KEY_BUILD; }
};
const SUPA_KEY_DEFAULT = _SUPA_KEY_BUILD;
const SUPA_KEY = getSupaKey(); // pour compatibilité avec le panel diagnostic
...
async function seed(uid){
  // ── Anti-double-seed : vérifier si déjà seedé dans user_metadata ──
  try {
  ...
  if(resTx.error) console.warn("[seed] transactions insert error:", resTx.error.message);
  if(resCli.error) console.warn("[seed] clients insert error:", resCli.error.message);
...
function Confetti({on}){
  ...
  const pieces = useMemo(()=>
    Array.from({length:80},(_,i)=>({
      ...
      del: Math.random()*.8,
      rot: Math.random()*360,
      col: [T.gold,T.gr,T.blue,T.orange,T.purple,T.teal,"#fff"][i%7],
      round: i%3===0,
    }))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ,[on]); // recalcul uniquement quand on passe de false → true

  if(!on)return null;
  return(
    <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:9999,overflow:"hidden"}}>
      {pieces.map((p,i)=>(
        <div key={i} style={{position:"absolute",left:p.left+"%",top:"-15px",width:p.w,height:p.h,borderRadius:p.round?"50%":3,background:p.col,animation:`fall ${p.dur}s ${p.del}s linear forwards`,transform:`rotate(${p.rot}deg)`}}/>
      ))}
    </div>
  );
}
...
function AuthField({ l, ch, err, hint }) {
  return (
    <div style={{marginBottom:12}}>
...
function AuthPage({onLogin,initialTab="login",onBack}){
    ...
    try{
      ...
      if(tab==="reset"){
        const {error}=await supa.auth.resetPasswordForEmail(f.email.toLowerCase(),{
          redirectTo:"https://vierafrik.com/reset-password"
        });
      ...
      if(tab==="signup"){
        ...
        const {data,error}=await supa.auth.signUp({
          ...
          options:{
            ...
            data:{
              name:f.name.trim(),
              business:f.business||"Mon Business",
              plan:"free",
              accent:T.gr,
              goal:2500000,
              phone:f.phone||"",
        ...
        const u={
          ...
          business:user.user_metadata?.business||f.business||"Mon Business",
      ...
      if(error){
        if(error.message.includes("Invalid login")||error.message.includes("invalid_credentials")||error.message.includes("password")){
          ...
          setE({email:"📧 Email non confirmé. Vérifiez votre boîte mail et cliquez sur le lien de confirmation."});
        } else {
          setE({email:"Erreur : "+error.message});
        }
        setL(false);return;
      ...
      const u={
        ...
        business:user.user_metadata?.business||"Mon Business",
  ...
  // IS et FL sont définis hors du composant (voir AuthFieldStyle / AuthField ci-dessus)
  // pour éviter la recréation à chaque frappe → perte de focus
        ...
        <div style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:20,padding:"1.8rem",boxShadow:"0 40px 100px rgba(0,0,0,.8)"}}>
          ...
          {done?(
            ...
            <>
              ...
              {tab==="signup"&&(
                <>
                  <AuthField l={t("fullName")} err={e.name}
              ...
              {/* Email */}
              <AuthField l={t("email")} err={e.email}
                ch={<input type="email" style={AUTH_IS} placeholder="votre@email.com" value={f.email||""} onChange={s("email")} onKeyDown={ev=>ev.key==="Enter"&&go()}/>}/>

              {/* Password */}
              {tab!=="reset"&&(
                <AuthField l={t("password")} err={e.password}
                  ch={<input type="password" style={AUTH_IS} placeholder={tab==="signup"?t("minChars"):"••••••••"} value={f.password||""} onChange={s("password")} onKeyDown={ev=>ev.key==="Enter"&&go()}/>}/>
              )}
              {tab==="signup"&&(
                <AuthField l={t("confirmPwd")} err={e.confirm}
                  ch={<input type="password" style={AUTH_IS} placeholder={t("repeatPwd")} value={f.confirm||""} onChange={s("confirm")} onKeyDown={ev=>ev.key==="Enter"&&go()}/>}/>
              )}
        ...
        <div style={{marginTop:16,display:"flex",gap:7,justifyContent:"center",flexWrap:"wrap"}}>
          {Object.entries(PLANS).map(([k,p])=>(
            <div key={k} style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:12,padding:"8px 13px",textAlign:"center"}}>
              ...
              <div style={{fontWeight:900,fontSize:14,color:T.text,marginTop:1}}>
                {p.price===0?"Free":`${IS_AFRICA ? fmtPrice(p.price,"XOF") : fmtPrice(Math.round(p.price * (DETECTED_CURRENCY==="EUR"?FCFA_TO_EUR:FCFA_TO_USD)*100)/100, DETECTED_CURRENCY)}/mo`}
              </div>
            </div>
          ))}
        </div>
...
const _getPublicSupaKey = () => {
  // Priorité : clé injectée au build → localStorage → clé hardcodée (fallback)
  const fromBuild = (typeof globalThis !== "undefined" && globalThis.__VITE_ENV__?.VITE_SUPABASE_ANON_KEY)
    || (typeof window !== "undefined" && window.__ENV__?.VITE_SUPABASE_ANON_KEY);
  if (fromBuild && fromBuild.startsWith("eyJ")) return fromBuild;
  try {
...
let _publicSupa = null;
let _publicSupaKey = null;
const getPublicSupa = async () => {
...
function PublicPayPage({ invoiceId }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Diagnostic clé
      const diagKey = _getPublicSupaKey();
      try {
        const s = await getPublicSupa();
        let data = null;
        let rlsBlocked = false;

        // ── TENTATIVE 1 : SELECT par UUID (id exact) ──
        const r1 = await s.from("invoices").select("*").eq("id", invoiceId).maybeSingle();

        if (r1.error) {
          const code = r1.error?.code;
          const msg  = r1.error?.message || "";
          rlsBlocked = (code === "42501" || code === "PGRST301" || msg.includes("row-level security") || msg.includes("permission denied") || msg.includes("JWT"));
          console.error("❌ [PublicPayPage] SELECT by id error:", { code, msg, rlsBlocked });
        } else {
          data = r1.data; // peut être null si facture absente mais RLS ok
        }

        // ── TENTATIVE 2 : SELECT par numéro (fallback QR ancien format) ──
        if (!data && !rlsBlocked) {
          const r2 = await s.from("invoices").select("*").eq("number", invoiceId).maybeSingle();
          if (!r2.error && r2.data) {
            data = r2.data;
          }
          if (r2.error) {
            ...
            if (code2 === "42501" || code2 === "PGRST301" || msg2.includes("row-level security")) {
              rlsBlocked = true;
            }
          }
        }
        ...
        if (!data && rlsBlocked) {
          ...
          try {
            ...
            if (!r3.error && r3.data) {
              data = Array.isArray(r3.data) ? r3.data[0] : r3.data;
              rlsBlocked = false;
            }
          } catch(rpcErr) {
            console.warn("⚠️ [PublicPayPage] RPC non disponible:", rpcErr);
          }
        }

        // ── TENTATIVE 4 : API serverless /api/invoice (contournement RLS côté serveur) ──
        // Cette route lit la facture avec la clé SERVICE_ROLE côté serveur, sans RLS
        if (!data) {
          console.warn("⚠️ [PublicPayPage] Tentative via /api/invoice");
          try {
            const r4 = await fetch(`/api/invoice?id=${encodeURIComponent(invoiceId)}`);
            if (r4.ok) {
            ...
            console.warn("⚠️ [PublicPayPage] /api/invoice non disponible:", apiErr);
          }
        }
        ...
        setInv({
          ...
          items,
          notes: data.notes || "",
          payStatus: data.status === "paid" ? "paid" : "unpaid",
          amtPaid: parseFloat(data.amt_paid) || 0,
        });
        if (data.phone) setPhone(data.phone);
      } catch (e) {
        console.error("❌ [PublicPayPage] Exception:", e);
  ...
  const fmtf = (n) => {
    ...
    try {
      return new Intl.NumberFormat("fr-FR", {
        ...
        minimumFractionDigits: 0, maximumFractionDigits: noDecimal ? 0 : 2,
      }).format(n || 0);
    } catch(_) {
      return `${new Intl.NumberFormat("fr-FR").format(Math.round(n || 0))} ${cur}`;
    }
  };
    ...
    try {
      ...
      if (url) {
        ...
        if (txId) {
          try {
            const s = await getPublicSupa();
            await s.from("invoices").update({ pay_ref: txId, pay_prov: prov }).eq("id", inv.id);
          } catch(_) {}
        }
        ...
        {error && (
          <div style={{ textAlign:"center", padding:"32px 16px" }}>
            {error === "RLS_BLOCKED" || error === "NOT_FOUND" ? (
              ...
              <>
                <div style={{ fontSize:48, marginBottom:12 }}>📡</div>
                <div style={{ color:"#ff2255", fontWeight:700, fontSize:16, marginBottom:8 }}>Erreur de connexion</div>
                <div style={{ color:"#80a8c8", fontSize:13, marginBottom:16, lineHeight:1.6 }}>
                  Impossible de joindre le serveur.<br/>Vérifiez votre connexion internet.
                </div>
                <button onClick={() => window.location.reload()} style={{ background:"rgba(0,212,120,0.12)", border:"1px solid rgba(0,212,120,0.3)", color:"#00d478", borderRadius:8, padding:"8px 18px", cursor:"pointer", fontSize:12, fontWeight:700 }}>
                  🔄 Réessayer
                </button>
              </>
            )}
            ...
            {isPaid ? (
              ...
              <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(0,212,120,0.12)", borderRadius:16, padding:20 }}>
                ...
                <div style={{ textAlign:"center", fontSize:10, color:"#4a7090", marginTop:12 }}>
                  🔒 Paiement sécurisé via FedaPay · SSL
                </div>
              </div>
            )}
...
// ── QR Canvas avec QRious ──
function SmartQRCanvas({ value, color="#00d478", bg="#010306", size=220 }) {
  ...
  }, [loaded, value, color, bg, size]);

  if (!value) return (
    <div style={{ width:size, height:size, background:"#0d1828", borderRadius:14,
      display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
      border:"2px dashed rgba(0,210,120,0.08)", gap:10 }}>
      <span style={{ fontSize:36 }}>📱</span>
      <span style={{ fontSize:10, color:"#4a7090", textAlign:"center", padding:"0 16px", lineHeight:1.5 }}>
        Remplissez le formulaire pour générer votre QR
      </span>
    </div>
  );
  ...
  return (
    <canvas ref={canvasRef}
      style={{ borderRadius:12, width:size, height:size, display:"block" }}/>
  );
}

// ── PAGE SMART QR (wrapper pour Dashboard) ──
function SmartQRPage({ user, isAdmin=false }) {
  const planKey = user?.plan || "free";
  const PLAN_LIMITS = {
    free:     { maxQR:2,  custom:false, tracking:false, bizPage:false },
    pro:      { maxQR:999, custom:true,  tracking:true,  bizPage:true  },
    business: { maxQR:999, custom:true,  tracking:true,  bizPage:true  },
  };
  // Admin → toujours plan business complet
  const plan = isAdmin ? PLAN_LIMITS.business : PLAN_LIMITS[planKey];

  const [mode, setMode]               = useState(null);
  const [qrColor, setQrColor]         = useState("#00d478");
  const [qrBg, setQrBg]               = useState("#010306");
  ...
  // Forms
  const [payForm,     setPayForm]     = useState({ _type:"invoice", invoiceId:"", invoiceNum:"", amount:"", desc:"" });
  const [contactForm, setContactForm] = useState({ type:"whatsapp", phone:user?.phone||"", url:"", message:"Bonjour, je vous contacte via VierAfrik !" });
  const [bizForm,     setBizForm]     = useState({ name:user?.business||"", desc:"", phone:user?.phone||"", accent:"#00d478" });

  // ── Factures réelles pour le sélecteur QR paiement ──
  ...
  // ── Charger QR depuis Supabase ──
  useEffect(() => {
    if (!user?.id) return;
    (async () => {
      try {
        const s = await getSupa();
        const { data } = await s.from("qr_codes").select("*")
  ...
  // ── Charger les factures réelles pour le sélecteur ──
  useEffect(() => {
    if (!user?.id) return;
    ...
    (async () => {
      try {
        const s = await getSupa();
        // On charge uniquement les factures non payées (en attente ou en retard)
        const { data } = await s
          .from("invoices")
          ...
          .limit(50);
        if (data && data.length > 0) setInvoicesList(data);
        else {
          // Fallback : toutes les factures si aucune non-payée
          const { data: all } = await s
            ...
            .order("created_at", { ascending:false })
            .limit(50);
          if (all) setInvoicesList(all);
        }
      } catch(e) { console.warn("Chargement factures QR:", e); }
      finally { setLoadingInvoices(false); }
    })();
  }, [user?.id]);

  // ── Construire la valeur QR ──
  useEffect(() => {
    let val = "";
    const base = "https://vierafrik.com";
    if (mode === "payment") {
      // ✅ CORRECTION CRITIQUE : on utilise l'UUID (invoiceId) pas le numéro lisible
      if (payForm._type === "invoice" && payForm.invoiceId)
        val = `${base}/?pay=${payForm.invoiceId}`;
      else if (payForm._type === "amount" && payForm.amount)
        val = `${base}/pay?amount=${payForm.amount}&desc=${encodeURIComponent(payForm.desc||"")}`;
    }
    if (mode === "contact") {
      if (contactForm.type === "whatsapp" && contactForm.phone) {
        const ph = contactForm.phone.replace(/\D/g,"");
        val = `https://wa.me/${ph}?text=${encodeURIComponent(contactForm.message)}`;
      }
      if (contactForm.type === "phone" && contactForm.phone)
        val = `tel:${contactForm.phone}`;
      if (contactForm.type === "url" && contactForm.url)
        val = contactForm.url;
    }
    if (mode === "bizpage")
      val = `${base}/biz/${user?.id || "demo"}`;
    setQrValue(val);
  }, [mode, payForm, contactForm, bizForm, user]);
  ...
  const handleSave = async () => {
    ...
    try {
      const s = await getSupa();
      await s.from("qr_codes").insert(entry);
      setSaved(p => [entry, ...p]);
  ...
  const handleDownload = () => {
    const canvas = document.querySelector("#vierafrik-qr-canvas canvas");
    if (!canvas) { showToast("⚠️ QR non prêt", "err"); return; }
    const link = document.createElement("a");
    link.download = `vierafrik-qr-${mode}-${Date.now()}.png`;
    link.href = canvas.toDataURL("image/png");
    link.click();
    showToast("✅ QR téléchargé !");
  };

  const reset = () => { setMode(null); setQrValue(""); };

  const IS = {
    width:"100%", padding:"10px 13px", background:"#0d1828",
    border:"1px solid rgba(0,210,120,0.08)", borderRadius:10, color:"#dff0ff",
    fontFamily:"inherit", fontSize:13, outline:"none", marginTop:4, boxSizing:"border-box",
  };

  const COLORS = ["#00d478","#00bfcc","#1a78ff","#f0b020","#ff5a18","#9060ff","#fff","#ff2255"];
  const BG_COLS = ["#010306","#05090f","#0a1628","#fff","#1a0008","#001a0a"];

  // ── ÉCRAN ACCUEIL ──
  if (!mode) return (
    <div style={{ color:"#dff0ff", fontFamily:"'Inter','Segoe UI',sans-serif" }}>
      {toastMsg && (
        <div style={{ position:"fixed", top:70, left:"50%", transform:"translateX(-50%)",
          ...
          fontSize:12, zIndex:9999, whiteSpace:"nowrap", boxShadow:"0 4px 20px rgba(0,0,0,.5)" }}>
          {toastMsg.msg}
        </div>
      )}
      <div style={{ marginBottom:20 }}>
        <div style={{ fontWeight:900, fontSize:22, letterSpacing:"-.03em", display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ width:40, height:40, borderRadius:11, background:"linear-gradient(135deg,#00d478,#00bfcc)",
            display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:20,
            boxShadow:"0 0 24px rgba(0,212,120,0.4)", flexShrink:0 }}>📱</span>
          Smart QR VierAfrik
        </div>
        <div style={{ fontSize:12, color:"#4a7090", marginTop:4 }}>
          Générez des QR intelligents pour votre business · {saved.length} QR sauvegardé{saved.length>1?"s":""}
        </div>
      </div>
      <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:20 }}>
        {[
          { id:"payment", emoji:"💳", label:"QR Paiement",   col:"#00d478",
            desc:"Le client scanne et paie directement par Mobile Money",
            tags:["Facture","Vente rapide","Encaissement"], free:true },
          { id:"contact", emoji:"📲", label:"QR Contact",    col:"#1a78ff",
            desc:"WhatsApp pré-rempli, appel direct ou lien personnalisé",
            tags:["WhatsApp","Appel","Lien URL"], free:true },
          { id:"bizpage", emoji:"🏪", label:"Business Page", col:"#f0b020",
            desc:"Mini page vitrine avec boutons Payer et Contacter",
            tags:["Page vitrine","Bouton payer","Pro"], free:false },
        ].map(m => (
          <div key={m.id}
            onClick={() => {
            ...
            style={{ background:"linear-gradient(135deg,#05090f,#08111d)",
              border:`1px solid ${m.col}33`, borderRadius:18, padding:"16px",
              cursor:"pointer", transition:"all .2s", position:"relative", overflow:"hidden" }}
            onMouseEnter={e => { e.currentTarget.style.borderColor=m.col+"88"; e.currentTarget.style.transform="translateY(-1px)"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor=m.col+"33"; e.currentTarget.style.transform="translateY(0)"; }}>
            <div style={{ position:"absolute", right:-16, top:-16, width:80, height:80,
              borderRadius:"50%", background:`${m.col}06`, pointerEvents:"none" }}/>
            <div style={{ display:"flex", alignItems:"center", gap:12 }}>
              <div style={{ width:46, height:46, borderRadius:13, flexShrink:0,
                background:`${m.col}20`, border:`1px solid ${m.col}44`,
                display:"flex", alignItems:"center", justifyContent:"center", fontSize:22 }}>
                {m.emoji}
              </div>
              <div style={{ flex:1 }}>
                <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:3 }}>
                  <span style={{ fontWeight:900, fontSize:15 }}>{m.label}</span>
                  {!m.free && <span style={{ background:"#f0b020", color:"#000", fontSize:7,
                    fontWeight:800, borderRadius:20, padding:"2px 7px" }}>PRO</span>}
                </div>
                <div style={{ fontSize:11, color:"#80a8c8", marginBottom:8, lineHeight:1.5 }}>{m.desc}</div>
                <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                  {m.tags.map(tag => (
                    <span key={tag} style={{ background:`${m.col}15`, border:`1px solid ${m.col}33`,
                      borderRadius:20, padding:"2px 9px", fontSize:9, color:m.col, fontWeight:600 }}>
                      {tag}
                    </span>
                  ))}
                </div>
              </div>
              <span style={{ color:m.col, fontSize:18, flexShrink:0 }}>→</span>
            </div>
          </div>
        ))}
      </div>
      {saved.length > 0 && (
        <div style={{ background:"#05090f", border:"1px solid rgba(0,210,120,0.08)", borderRadius:16, padding:"1.2rem", marginBottom:16 }}>
          <div style={{ fontWeight:800, fontSize:13, marginBottom:12, display:"flex", alignItems:"center", gap:8 }}>
            <span style={{ background:"rgba(0,212,120,0.1)", border:"1px solid rgba(0,212,120,0.2)",
              borderRadius:8, padding:"4px 8px", fontSize:13 }}>🗂️</span>
            Mes QR codes
          </div>
          <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
            {saved.map(q => (
              <div key={q.id} style={{ background:"#08111d", border:"1px solid rgba(0,210,120,0.08)",
                borderRadius:11, padding:"10px 13px", display:"flex", alignItems:"center", gap:10 }}>
                <img src={`https://api.qrserver.com/v1/create-qr-code/?size=36x36&data=${encodeURIComponent(q.value)}&bgcolor=0d1828&color=00d478`}
                  alt="qr" style={{ width:36, height:36, borderRadius:6, flexShrink:0 }}/>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontWeight:700, fontSize:12, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{q.label}</div>
                  <div style={{ fontSize:10, color:"#4a7090", marginTop:1 }}>{q.mode} · {q.scans||0} scans</div>
                </div>
                <button onClick={() => {
                  navigator.clipboard?.writeText(q.value);
                  showToast("📋 Lien copié !");
                }} style={{ background:"rgba(0,212,120,0.1)", border:"1px solid rgba(0,212,120,0.2)",
                  borderRadius:7, padding:"5px 10px", color:"#00d478",
      ...
      {!plan.custom && (
        <div style={{ background:"linear-gradient(135deg,rgba(240,176,32,0.1),rgba(255,90,24,0.06))",
          border:"1px solid rgba(240,176,32,0.3)", borderRadius:14, padding:"1.2rem", textAlign:"center" }}>
          <div style={{ fontSize:20, marginBottom:6 }}>⚡</div>
          <div style={{ fontWeight:800, fontSize:14, marginBottom:4 }}>Passez à Pro</div>
          <div style={{ fontSize:11, color:"#80a8c8", marginBottom:10, lineHeight:1.5 }}>
            QR illimités · Couleurs personnalisées · Tracking · Business Page
          </div>
          <div style={{ fontSize:12, fontWeight:700, color:"#f0b020" }}>4 900 FCFA/mois →</div>
        </div>
      )}
    </div>
  );

  // ── ÉCRAN FORMULAIRE + PREVIEW ──
  const modeInfo = {
    payment:{ emoji:"💳", label:"QR Paiement",   col:"#00d478" },
    contact:{ emoji:"📲", label:"QR Contact",    col:"#1a78ff" },
    bizpage:{ emoji:"🏪", label:"Business Page", col:"#f0b020" },
  }[mode];
  ...
  return (
    <div style={{ color:"#dff0ff", fontFamily:"'Inter','Segoe UI',sans-serif" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:20 }}>
        <button onClick={reset} style={{ background:"#08111d",
          border:"1px solid rgba(0,210,120,0.08)", borderRadius:9,
          color:"#80a8c8", padding:"7px 13px", cursor:"pointer",
          fontFamily:"inherit", fontSize:11, fontWeight:600 }}>
          ← Retour
        </button>
        <div>
          <div style={{ fontWeight:900, fontSize:17 }}>{modeInfo.emoji} {modeInfo.label}</div>
          <div style={{ fontSize:10, color:"#4a7090" }}>Configuration + Aperçu live</div>
        </div>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, alignItems:"start" }}>
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {mode === "payment" && (
            <div style={{ background:"#05090f", border:"1px solid rgba(0,210,120,0.08)", borderRadius:16, padding:"1.2rem" }}>
              <div style={{ fontWeight:800, fontSize:13, marginBottom:12 }}>💳 Configuration paiement</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:7, marginBottom:12 }}>
                {[{ v:"invoice", l:"Facture", ic:"🧾" }, { v:"amount", l:"Montant libre", ic:"💰" }].map(opt => (
                  <div key={opt.v} onClick={() => setPayForm(f => ({ ...f, _type:opt.v }))}
                    style={{ padding:"10px 8px", background: payForm._type===opt.v ? "rgba(0,212,120,0.12)" : "#08111d",
                      border:`1px solid ${payForm._type===opt.v ? "#00d478" : "rgba(0,210,120,0.08)"}`,
                      borderRadius:10, cursor:"pointer", textAlign:"center", transition:"all .18s" }}>
                    <div style={{ fontSize:20, marginBottom:3 }}>{opt.ic}</div>
                    <div style={{ fontSize:10, fontWeight:700, color: payForm._type===opt.v ? "#00d478" : "#80a8c8" }}>{opt.l}</div>
                  </div>
                ))}
              </div>
              {payForm._type === "invoice" ? (
                <div>
                  <label style={{ fontSize:10, fontWeight:700, color:"#4a7090", display:"block", marginBottom:6 }}>
                    Choisir une facture
                  </label>
                  {loadingInvoices ? (
                    <div style={{ fontSize:11, color:"#4a7090", padding:"10px 0" }}>⏳ Chargement des factures…</div>
                  ) : invoicesList.length === 0 ? (
                    <div style={{ fontSize:11, color:"#ff9060", background:"rgba(255,90,24,0.08)", border:"1px solid rgba(255,90,24,0.2)", borderRadius:8, padding:"10px 12px" }}>
                      ⚠️ Aucune facture trouvée. Créez d'abord une facture dans l'onglet Factures.
                    </div>
                  ) : (
                    <>
                      <select
                        style={{ ...IS, appearance:"none", WebkitAppearance:"none", cursor:"pointer", paddingRight:28 }}
                        ...
                        {invoicesList.map(inv => (
                          <option key={inv.id} value={inv.id}>
                            {inv.number} · {inv.client_name} · {fmt(inv.total)} FCFA
                            {inv.status === "overdue" ? " ⚠️" : inv.status === "paid" ? " ✅" : " ⏳"}
                          </option>
                        ))}
                      </select>
                      {payForm.invoiceId && (
                        <div style={{ fontSize:9, color:"#4a7090", marginTop:6, fontFamily:"monospace", background:"rgba(0,210,120,0.04)", border:"1px solid rgba(0,210,120,0.1)", borderRadius:6, padding:"5px 8px", lineHeight:1.6 }}>
                          <div>🔗 vierafrik.com/?pay=<span style={{ color:"#00d478" }}>{payForm.invoiceId}</span></div>
                          <div style={{ color:"#80a8c8", marginTop:2 }}>Facture : {payForm.invoiceNum}</div>
                        </div>
                      )}
                    </>
                  )}
                ...
                <>
                  <div style={{ marginBottom:8 }}>
                    <label style={{ fontSize:10, fontWeight:700, color:"#4a7090", display:"block", marginBottom:4 }}>Montant (FCFA)</label>
                    <input type="number" style={IS} placeholder="Ex: 25000"
                      value={payForm.amount} onChange={e => setPayForm(f => ({ ...f, amount:e.target.value }))}/>
                  </div>
                  <div>
                    <label style={{ fontSize:10, fontWeight:700, color:"#4a7090", display:"block", marginBottom:4 }}>Description</label>
                    <input style={IS} placeholder="Coiffure, vente, service..."
                      value={payForm.desc} onChange={e => setPayForm(f => ({ ...f, desc:e.target.value }))}/>
                  </div>
                </>
              )}
            </div>
          ...
          {mode === "contact" && (
            <div style={{ background:"#05090f", border:"1px solid rgba(0,210,120,0.08)", borderRadius:16, padding:"1.2rem" }}>
              <div style={{ fontWeight:800, fontSize:13, marginBottom:12 }}>📲 Configuration contact</div>
              <div style={{ display:"flex", gap:6, marginBottom:12 }}>
                {[{ v:"whatsapp", ic:"💬", l:"WhatsApp" }, { v:"phone", ic:"📞", l:"Appel" }, { v:"url", ic:"🔗", l:"URL" }].map(opt => (
                  <button key={opt.v} onClick={() => setContactForm(f => ({ ...f, type:opt.v }))}
                    style={{ flex:1, padding:"8px 4px", borderRadius:9, border:"none", fontFamily:"inherit",
                      background: contactForm.type===opt.v ? "rgba(26,120,255,0.2)" : "#08111d",
                      color: contactForm.type===opt.v ? "#1a78ff" : "#80a8c8",
                      outline: contactForm.type===opt.v ? "1px solid #1a78ff" : "none",
                      cursor:"pointer", fontWeight:700, fontSize:10, transition:"all .18s" }}>
                    <div style={{ fontSize:16, marginBottom:2 }}>{opt.ic}</div>{opt.l}
                  </button>
                ))}
              </div>
              {contactForm.type !== "url" && (
                <div style={{ marginBottom:8 }}>
                  <label style={{ fontSize:10, fontWeight:700, color:"#4a7090", display:"block", marginBottom:4 }}>Téléphone</label>
                  <input type="tel" style={IS} placeholder="+225 07 000 0000"
                    value={contactForm.phone} onChange={e => setContactForm(f => ({ ...f, phone:e.target.value }))}/>
                </div>
              )}
              {contactForm.type === "url" && (
                <div style={{ marginBottom:8 }}>
                  <label style={{ fontSize:10, fontWeight:700, color:"#4a7090", display:"block", marginBottom:4 }}>URL</label>
                  <input style={IS} placeholder="https://..."
                    value={contactForm.url} onChange={e => setContactForm(f => ({ ...f, url:e.target.value }))}/>
                </div>
              )}
              {contactForm.type === "whatsapp" && (
                <div>
                  <label style={{ fontSize:10, fontWeight:700, color:"#4a7090", display:"block", marginBottom:4 }}>Message pré-rempli</label>
                  <textarea style={{ ...IS, height:65, resize:"none" }}
          ...
          {mode === "bizpage" && (
            <div style={{ background:"#05090f", border:"1px solid rgba(0,210,120,0.08)", borderRadius:16, padding:"1.2rem" }}>
              <div style={{ fontWeight:800, fontSize:13, marginBottom:12 }}>🏪 Business Page</div>
              {[
                ["Nom du business","name","text","Mon Entreprise"],
                ["Téléphone","phone","tel","+225 07 000 0000"],
              ].map(([label,key,type,ph]) => (
                <div key={key} style={{ marginBottom:8 }}>
                  <label style={{ fontSize:10, fontWeight:700, color:"#4a7090", display:"block", marginBottom:4 }}>{label}</label>
                  <input type={type} style={IS} placeholder={ph}
                    value={bizForm[key]} onChange={e => setBizForm(f => ({ ...f, [key]:e.target.value }))}/>
                </div>
              ))}
              <div style={{ marginBottom:10 }}>
                <label style={{ fontSize:10, fontWeight:700, color:"#4a7090", display:"block", marginBottom:4 }}>Description</label>
                <textarea style={{ ...IS, height:55, resize:"none" }} placeholder="Ce que vous faites..."
                  value={bizForm.desc} onChange={e => setBizForm(f => ({ ...f, desc:e.target.value }))}/>
              </div>
              <div>
                <label style={{ fontSize:10, fontWeight:700, color:"#4a7090", display:"block", marginBottom:6 }}>Couleur accent</label>
                <div style={{ display:"flex", gap:7 }}>
                  {["#00d478","#1a78ff","#f0b020","#ff5a18","#9060ff","#00bfcc"].map(c => (
                    <div key={c} onClick={() => setBizForm(f => ({ ...f, accent:c }))}
                      style={{ width:24, height:24, borderRadius:"50%", background:c, cursor:"pointer",
                        border: bizForm.accent===c ? "3px solid #fff" : "2px solid transparent",
                        boxShadow: bizForm.accent===c ? `0 0 8px ${c}` : "none", transition:"all .15s" }}/>
                  ))}
                </div>
              </div>
            </div>
          )}
          <div style={{ background:"#05090f", border:"1px solid rgba(0,210,120,0.08)",
            borderRadius:16, padding:"1.2rem", position:"relative", overflow:"hidden" }}>
            {!plan.custom && (
              <div style={{ position:"absolute", inset:0, background:"rgba(1,3,6,0.88)",
            ...
            <div style={{ fontWeight:800, fontSize:13, marginBottom:10 }}>🎨 Personnalisation</div>
            <div style={{ marginBottom:10 }}>
              <div style={{ fontSize:10, fontWeight:700, color:"#4a7090", marginBottom:6 }}>Couleur du QR</div>
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                {COLORS.map(c => (
                  <div key={c} onClick={() => setQrColor(c)}
                    style={{ width:24, height:24, borderRadius:"50%", background:c, cursor:"pointer",
                      border: qrColor===c ? "3px solid #fff" : "2px solid transparent",
                      boxShadow: qrColor===c ? `0 0 8px ${c}88` : "none", transition:"all .15s" }}/>
                ))}
              </div>
            </div>
            <div>
              <div style={{ fontSize:10, fontWeight:700, color:"#4a7090", marginBottom:6 }}>Fond du QR</div>
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                {BG_COLS.map(c => (
                  <div key={c} onClick={() => setQrBg(c)}
                    style={{ width:24, height:24, borderRadius:"50%", background:c, cursor:"pointer",
                      border: qrBg===c ? `3px solid #00d478` : `1px solid rgba(0,210,120,0.2)`,
                      transition:"all .15s" }}/>
                ))}
              </div>
            </div>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={handleSave}
              style={{ flex:2, padding:"11px", borderRadius:10, border:"none",
                ...
                fontFamily:"inherit", fontWeight:900, fontSize:13, cursor:"pointer",
                boxShadow:"0 6px 20px rgba(0,212,120,0.4)" }}>
              💾 Sauvegarder
            </button>
            <button onClick={reset}
              style={{ flex:1, padding:"11px", borderRadius:10,
                border:"1px solid rgba(0,210,120,0.08)", background:"#08111d",
                color:"#80a8c8", fontFamily:"inherit", fontWeight:700, fontSize:12, cursor:"pointer" }}>
              ← Retour
            </button>
          </div>
        </div>
        <div style={{ position:"sticky", top:70 }}>
          <div style={{ background:"#05090f", border:"1px solid rgba(0,210,120,0.08)",
            borderRadius:18, padding:"1.2rem", display:"flex", flexDirection:"column",
            alignItems:"center", gap:14 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#4a7090",
              textTransform:"uppercase", letterSpacing:".08em" }}>
              Aperçu live
            </div>
            <div id="vierafrik-qr-canvas" style={{ background:qrBg, borderRadius:14, padding:8,
              boxShadow:`0 8px 32px rgba(0,0,0,.6), 0 0 0 1px ${qrColor}22` }}>
              <SmartQRCanvas value={qrValue} color={qrColor} bg={qrBg} size={200}/>
            </div>
            {qrValue && (
              <div style={{ background:"#0d1828", border:"1px solid rgba(0,210,120,0.08)",
                borderRadius:9, padding:"7px 11px", width:"100%", boxSizing:"border-box" }}>
                <div style={{ fontSize:9, color:"#4a7090", fontWeight:700, marginBottom:2,
                  textTransform:"uppercase" }}>Lien encodé</div>
                <div style={{ fontSize:9, color:"#80a8c8", fontFamily:"monospace",
                  wordBreak:"break-all", lineHeight:1.5 }}>
                  {qrValue.length > 55 ? qrValue.slice(0,52)+"…" : qrValue}
                </div>
              </div>
            )}
            {plan.tracking && qrValue && (
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:6,
                width:"100%", boxSizing:"border-box" }}>
                {[["Scans","0","#1a78ff"],["Clics","0","#00d478"],["Conv.","0%","#f0b020"]].map(([l,v,c]) => (
                  <div key={l} style={{ background:"#0d1828", borderRadius:9, padding:"7px",
                    textAlign:"center", border:`1px solid ${c}22` }}>
                    <div style={{ fontSize:15, fontWeight:900, color:c }}>{v}</div>
                    <div style={{ fontSize:8, color:"#4a7090", fontWeight:600, marginTop:2 }}>{l}</div>
                  </div>
                ))}
              </div>
            )}
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:7,
                ...
                <button onClick={handleDownload}
                  style={{ width:"100%", padding:"11px", borderRadius:11, border:"none",
                    ...
                    boxShadow:"0 4px 16px rgba(0,212,120,0.35)" }}>
                  ⬇️ Télécharger PNG
                </button>
                <button onClick={() => { navigator.clipboard?.writeText(qrValue); showToast("📋 Lien copié !"); }}
                  style={{ width:"100%", padding:"9px", borderRadius:11,
                    border:"1px solid rgba(0,210,120,0.12)", background:"#08111d",
                    color:"#dff0ff", fontFamily:"inherit", fontWeight:700, fontSize:11, cursor:"pointer" }}>
                  📋 Copier le lien
                </button>
                <button onClick={() => {
            ...
            <div style={{ fontSize:9, color:"#4a7090" }}>🔒 Sécurisé · vierafrik.com</div>
          </div>
        </div>
      </div>
...
//  Accessible via : https://vierafrik.vercel.app/reset-password
//  Supabase envoie : #access_token=...&type=recovery dans le hash
// ════════════════════════════════════════════════════════
function ResetPasswordPage(){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      const supa=await getSupa();
      // Vérifier si une session recovery est déjà active (ex: retour direct depuis email)
      ...
      const {data}=supa.auth.onAuthStateChange((event,session)=>{
        if(event==="PASSWORD_RECOVERY"){
          // Token de récupération reçu → on peut mettre à jour le mot de passe
          setReady(true);
        }
  ...
  const AUTH_IS={
    ...
    borderRadius:11,color:"#dff0ff",
    fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",
    fontSize:14,outline:"none",marginTop:5,
    transition:"all .2s",
  };
  ...
  return(
    <div style={{
      ...
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800;900&display=swap');*{box-sizing:border-box}input:focus{border-color:${T.gr}!important;box-shadow:0 0 0 3px rgba(0,212,120,.15)!important;outline:none}`}</style>
      {/* Halo vert */}
      <div style={{position:"absolute",top:"18%",left:"50%",transform:"translateX(-50%)",width:500,height:240,background:`radial-gradient(ellipse,${T.gr}0b 0%,transparent 70%)`,pointerEvents:"none"}}/>
      <div style={{position:"absolute",inset:0}}><Particles/></div>

      <div style={{position:"relative",zIndex:1,width:"95%",maxWidth:420}}>

        {/* Logo */}
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{width:68,height:68,borderRadius:20,background:`linear-gradient(135deg,${T.gr},${T.teal})`,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:32,marginBottom:12,boxShadow:`0 0 50px ${T.gr}44`}}>🌍</div>
          <div style={{fontWeight:900,fontSize:30,letterSpacing:"-.04em",lineHeight:1}}>
            <span style={{color:T.gr}}>Vier</span><span style={{color:"#dff0ff"}}>Afrik</span>
          </div>
          <div style={{color:"#4a7090",fontSize:12,marginTop:6}}>Réinitialisation du mot de passe</div>
        </div>

        <div style={{
          background:`linear-gradient(160deg,${T.c1},${T.c2})`,
          border:`1px solid rgba(0,210,120,.18)`,
          ...
          {done?(
            ...
            <div style={{textAlign:"center",padding:"1rem 0"}}>
              ...
              <div style={{fontWeight:900,fontSize:18,color:T.gr,marginBottom:8,letterSpacing:"-.02em"}}>
                Mot de passe mis à jour !
              </div>
              <div style={{fontSize:13,color:"#80a8c8",lineHeight:1.7,marginBottom:6}}>
            ...
            <>
              <div style={{fontWeight:900,fontSize:19,marginBottom:6,letterSpacing:"-.03em"}}>
                🔑 Nouveau mot de passe
              </div>
              <div style={{fontSize:12,color:"#80a8c8",marginBottom:22,lineHeight:1.6}}>
                Choisissez un nouveau mot de passe sécurisé pour votre compte VierAfrik.
              </div>

              {/* Champ mot de passe */}
              <div style={{marginBottom:14}}>
                <label style={{display:"block",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",color:"#4a7090"}}>
                  Nouveau mot de passe
                </label>
                <input
                  type="password"
                  ...
                  <div style={{display:"flex",gap:5,marginBottom:4}}>
                    {[1,2,3,4].map(i=>(
                      <div key={i} style={{
                        ...
                        background:
                          pwd.length>=8&&/[A-Z]/.test(pwd)&&/[0-9]/.test(pwd)?T.gr:
                          pwd.length>=8&&(/[A-Z]/.test(pwd)||/[0-9]/.test(pwd))?T.teal:
                          pwd.length>=6?T.gold:T.red,
                        opacity: i<=( pwd.length>=8&&/[A-Z]/.test(pwd)&&/[0-9]/.test(pwd)?4: pwd.length>=8&&(/[A-Z]/.test(pwd)||/[0-9]/.test(pwd))?3: pwd.length>=6?2:1)?1:.2,
                      }}/>
                    ))}
                  </div>
                  <div style={{fontSize:10,color:"#4a7090"}}>
                    {pwd.length>=8&&/[A-Z]/.test(pwd)&&/[0-9]/.test(pwd)?"🟢 Mot de passe fort":
                     pwd.length>=8&&(/[A-Z]/.test(pwd)||/[0-9]/.test(pwd))?"🔵 Bon mot de passe":
                     pwd.length>=6?"🟡 Mot de passe acceptable":"🔴 Trop court"}
                  </div>
                </div>
              )}
              ...
              {error&&(
                <div style={{
                  background:"rgba(255,34,85,.08)",border:"1px solid rgba(255,34,85,.25)",
                  borderRadius:10,padding:"10px 13px",marginBottom:14,
                  fontSize:12,color:"#ff6680",display:"flex",gap:8,alignItems:"flex-start",
                }}>
              ...
              {!ready&&(
                <div style={{
                  background:`rgba(0,212,120,.06)`,border:`1px solid ${T.gr}22`,
                  borderRadius:10,padding:"10px 13px",marginBottom:14,
                  fontSize:12,color:"#80a8c8",
                }}>
              ...
              <button
                ...
                style={{
                  ...
                  background:loading||!ready?T.c3:`linear-gradient(135deg,${T.gr},${T.teal})`,
                  color:loading||!ready?"#4a7090":"#000",
                  fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",
                  fontWeight:900,fontSize:15,cursor:loading||!ready?"not-allowed":"pointer",
                  boxShadow:loading||!ready?"none":`0 6px 24px ${T.gr}44`,
                  transition:"all .2s",letterSpacing:"-.01em",
                }}>
                {loading?"⏳ Mise à jour en cours…":"✅ Valider le nouveau mot de passe"}
              </button>
              ...
              <div style={{textAlign:"center",marginTop:14}}>
                <button
                  ...
                  style={{background:"none",border:"none",color:"#4a7090",fontSize:12,cursor:"pointer",textDecoration:"underline",fontFamily:"inherit"}}
                >
        ...
        <div style={{textAlign:"center",marginTop:14,fontSize:11,color:"#4a7090"}}>
          🔒 Sécurisé · Supabase Auth · SSL
        </div>
      </div>
    </div>
  );
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      const {data}=supa.auth.onAuthStateChange((event,session)=>{
        if(event==="PASSWORD_RECOVERY"){
          // Token recovery reçu → rediriger vers la page reset si pas déjà dessus
...
function SupaDiagPanel({ uid, userEmail }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      for (const table of ["transactions", "invoices", "clients", "payments", "employees"]) {
        try {
  ...
  const applyNewKey = async () => {
    ...
    setSaving(false);
    alert(
      "✅ Clé sauvegardée dans localStorage pour cette session.\n\n" +
      "⚠️ POUR APPLIQUER DÉFINITIVEMENT :\n" +
      "Vercel Dashboard → Settings → Environment Variables\n" +
      "→ VITE_SUPABASE_ANON_KEY = " + newKey.trim().slice(0,32) + "…\n\n" +
        ...
        <div style={{marginTop:14}}>
          ...
          <div style={{
            ...
            {!keyOk && (
              <div style={{marginTop:6,color:"#f0b020",fontSize:10,lineHeight:1.5}}>
                ...
                → Vercel Dashboard → Settings → Environment Variables → <strong>VITE_SUPABASE_ANON_KEY</strong>
              </div>
            )}
          </div>
          ...
          <div style={{background:T.c2,border:`1px solid ${T.border}`,borderRadius:10,padding:"10px 12px"}}>
            ...
            <div style={{fontSize:9,color:T.sub,marginTop:6,lineHeight:1.4}}>
              ℹ️ La clé sera sauvegardée dans localStorage pour cette session. Pour l'appliquer définitivement, mettez à jour VITE_SUPABASE_ANON_KEY dans vos variables d'environnement Vercel.
            </div>
          </div>
...
function Dashboard({ses,logout,updSes}){
  ...
  // Calcule les jours restants à partir de expires_at
  const calcDaysLeft=(expiresAt)=>{
  ...
  const loadSubscription=useCallback(async()=>{
    ...
    try{
      ...
      const {data,error}=await s
        ...
        .limit(1)
        .maybeSingle();
      if(error){console.error("[SUB] Erreur lecture:",error);setSubLoading(false);return;}
      if(!data){setSubscription(null);setSubLoading(false);return;}
      const daysLeft=calcDaysLeft(data.expires_at);
      const isActive=daysLeft>=0 && data.status==="active";
      setSubscription({
        plan: data.plan,
      ...
      if(isActive && data.plan && data.plan!==ses.plan){
        updSes({plan:data.plan});
      }
      if(!isActive && ses.plan && ses.plan!=="free"){
        updSes({plan:"free"});
      }
    }catch(e){console.error("[SUB] Exception:",e);}
  ...
  // Vérifie si l'abonnement est actif (utilisé pour bloquer les pages premium)
  const isSubActive=useMemo(()=>{
    if(!subscription) return false;
    return subscription.status==="active" && subscription.days_left>=0;
  },[subscription]);

  // Plan réel basé sur abonnement (pas seulement session)
  const activePlan=useMemo(()=>{
    if(isSubActive && subscription?.plan) return subscription.plan;
    return "free";
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // ══════════════════════════════════════════════════════
  const [userCurrency, setUserCurrency] = useState(ses.currency || DEFAULT_CURRENCY);

  // Taux de conversion FCFA fixes (mis à jour manuellement)
  ...
  const fmtWithHint = (v, skipHint = false) => {
    ...
    const isAfrican = ["XOF","XAF"].includes(userCurrency);

    if (isAfrican) {
      const main = fmtPrice(n, "XOF");
      if (skipHint || n === 0) return { main, hint: "" };
      const inEur = Math.round(n * FCFA_TO_EUR);
      return { main, hint: inEur > 0 ? `~${fmtPrice(inEur, "EUR")}` : "" };
    } else {
      const rate = HINT_RATES[userCurrency] || FCFA_TO_EUR;
      const converted = Math.round(n * rate * 100) / 100;
      const main = fmtPrice(converted, userCurrency);
      if (skipHint || n === 0) return { main, hint: "" };
      return { main, hint: `~${fmtPrice(n, "XOF")}` };
    }
  };
  ...
  const FmtHint = ({ v, size = 22, color, noHint = false }) => {
    ...
    return (
      <span>
        <span style={{ fontSize: size, fontWeight: 900, color: color || "inherit", lineHeight: 1, letterSpacing: "-.03em" }}>{main}</span>
        {hint && <span style={{ fontSize: Math.max(9, size * 0.42), fontWeight: 500, color: T.sub, marginLeft: 4, letterSpacing: 0 }}>{hint}</span>}
      </span>
    );
  };
  ...
  // Écran de blocage si abonnement expiré (gardé pour les vrais renouvellements)
  const SubscriptionExpiredScreen=({onRenew})=>(
    <div style={{
      display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
      minHeight:320,textAlign:"center",padding:"2rem",
    }}>
      <div style={{fontSize:52,marginBottom:12}}>🔒</div>
      <div style={{fontWeight:900,fontSize:20,letterSpacing:"-.03em",marginBottom:8,color:T.text}}>
        Abonnement expiré
      </div>
      <div style={{
        background:"rgba(255,34,85,.08)",border:"1px solid rgba(255,34,85,.28)",
        borderRadius:14,padding:"14px 20px",marginBottom:20,
        fontSize:13,color:"#ff6680",lineHeight:1.7,maxWidth:340,
      }}>
        Votre abonnement a expiré.<br/>
        Veuillez renouveler votre paiement pour continuer à accéder aux fonctions premium.
      </div>
      <button onClick={onRenew} style={{
        padding:"13px 32px",borderRadius:12,border:"none",
        background:`linear-gradient(135deg,${T.gr},${T.teal})`,
        color:T.ink,fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",
        fontWeight:900,fontSize:15,cursor:"pointer",
        boxShadow:`0 6px 24px ${T.gr}44`,
  ...
  // Bannière d'alerte si abonnement expire bientôt (≤ 3 jours)
  const SubExpiryBanner=()=>{
    if(!subscription||!isSubActive) return null;
    if(subscription.days_left>3) return null;
    return(
      <div style={{
        background:"linear-gradient(135deg,rgba(240,176,32,.1),rgba(255,90,24,.06))",
        border:"1px solid rgba(240,176,32,.4)",borderRadius:12,
        padding:"10px 16px",marginBottom:12,
        display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",
      }}>
        <span style={{fontSize:16}}>⚠️</span>
        <div style={{flex:1}}>
          <div style={{fontWeight:700,fontSize:12,color:T.gold}}>
            {subscription.days_left===0
              ?"Votre abonnement expire aujourd'hui !"
              :`Votre abonnement expire dans ${subscription.days_left} jour${subscription.days_left>1?"s":""}`}
          </div>
          <div style={{fontSize:10,color:T.sub2,marginTop:1}}>
            Renouvelez maintenant pour éviter une interruption de service
          </div>
        </div>
        <button onClick={()=>setPage("plans")} style={{
          background:T.gold,color:T.ink,border:"none",borderRadius:8,
          padding:"6px 14px",fontFamily:"inherit",fontWeight:800,fontSize:11,
  ...
  useEffect(()=>{
    ...
    if(paySuccess==="1"&&plan&&uidFromUrl&&ses?.id&&uidFromUrl===ses.id){
      (async()=>{
        try{
          const s=await getSupa();
          // Vérifier le statut réel du paiement via FedaPay si on a la référence
          let confirmed=false;
          if(trxRef){
            try{
              const verif=await fetch("/api/fedapay",{
                method:"POST",
                headers:{"Content-Type":"application/json"},
          ...
          if(confirmed){            // ── Enregistrer l'abonnement dans la table subscriptions ──
            ...
            // Chercher un abonnement existant pour upsert (utilise uid du Dashboard = ses.id)
            const {data:existingSub}=await s
              .from("subscriptions")
              .select("id")
              .eq("user_id",uid)
              .maybeSingle()
            ...
            if(existingSub?.id){
              ...
              }).eq("user_id",uid).catch(e=>console.error("[SUB] update error:",e));
            } else {
              await s.from("subscriptions").insert({
                id:xid(),user_id:uid,plan,
                paid_at:paidAt,expires_at:expiresAt,status:"active",
                created_at:paidAt,updated_at:paidAt,
              }).catch(e=>console.error("[SUB] insert error:",e));
            }
            // Recharger l'abonnement dans le state
  ...
  const insights=useMemo(()=>{
    ...
    const mkt=txs.filter(t=>(t.cat||t.category)==="Marketing").reduce((s,t)=>s+t.amount,0);
  ...
  const trackUsage=async()=>{
    ...
    try{
      const s=await getSupa();
      const{data:ex}=await s.from("user_activity").select("id").eq("user_id",uid).maybeSingle();
  ...
  const nextNum=()=>{
    ...
    const maxFromState=invs.reduce((max,inv)=>{
      if(inv.num&&inv.num.startsWith(prefix)){
        const rawSeq=inv.num.slice(prefix.length).split("-")[0];
  ...
  //  - Vente = transaction + client auto + facture auto
  // ══════════════════════════════════════════════
  const saveTx=async()=>{
    // Snapshot des valeurs du formulaire AVANT tout reset
    const fmSnap = {...fm};
    try {
      ...
      // ── CAS MODIFICATION ──
      if (isEdit) {
        const nextTxs = txs.map(x => x.id === txObj.id ? txObj : x);
        setTxs(nextTxs);
        setMdl(null); setFm({});
        toast("✅ Transaction modifiée !");
        await supaUpdate("transactions", {
          type: txObj.type, amount: txObj.amount, category: txObj.cat,
          who: txObj.who, date: txObj.date, note: txObj.note, user_id: uid,
        }, txObj.id);
        return;
      }
      ...
      // 1. Mise à jour state local immédiate
      const nextTxs = [txObj, ...txs];
      setTxs(nextTxs);
      flashNew(txObj.id);
      ...
      // 3. Toast selon type
      const newProfit = nextTxs.filter(x => x.type === "sale" && mkey(x.date) === cm).reduce((s, x) => s + x.amount, 0)
                      - nextTxs.filter(x => x.type === "expense" && mkey(x.date) === cm).reduce((s, x) => s + x.amount, 0);
      if (txObj.type === "sale" && newProfit >= goal && profit < goal) {
        setBoom(true); setTimeout(() => setBoom(false), 5000);
        toast("🏆 Objectif mensuel dépassé !");
      } else {
        toast(txObj.type === "sale" ? "💰 Vente enregistrée !" : "📤 Dépense enregistrée !");
      }

      // 4. INSERT Supabase en arrière-plan
      const txOk = await supaInsert("transactions", {
        id: txObj.id, user_id: uid, type: txObj.type,
        amount: txObj.amount, category: txObj.cat,
        who: txObj.who, date: txObj.date, note: txObj.note,
      });

      // 5. Marquer utilisateur actif
      markUserActive(uid);
      trackUsage(); // v34

      // 6. AUTO-CRÉATION CLIENT + FACTURE si vente avec nom client
      if (txObj.type === "sale" && txObj.who) {
        const phoneClean = cleanP(fmSnap.clientPhone || "");
        const existing = clis.find(c =>
          c.name.toLowerCase() === txObj.who.toLowerCase() ||
          (phoneClean && cleanP(c.phone) === phoneClean)
        );
        let clientId = "";
        ...
        if (!existing) {
          ...
          if (canAdd("cli")) {
            ...
            // INSERT Supabase
            const cliOk = await supaInsert("clients", {
              id: newCli.id, user_id: uid,
              name: newCli.name, phone: newCli.phone,
              email: "", country: newCli.pays,
              category: newCli.cat, status: "active", revenue: amt,
            });
        ...
        if (canAdd("inv")) {
          ...
          const invItems = [{
            id: xid(),
            name: fmSnap.note || txObj.cat || "Vente",
            qty: 1, price: amt, line: amt,
          }];
          const newInv = {
            id: invId, uid, num: invNum, clientId,
            clientName: txObj.who,
            phone: fmSnap.clientPhone || existing?.phone || "",
            total: amt, sub: amt, tax: 0,
            currency: DEFAULT_CURRENCY,
            status: "pending", issued: txObj.date, due: "",
          ...
          const invOk = await supaInsert("invoices", {
            ...
            client_name: txObj.who,
            phone: newInv.phone,
            total: amt, subtotal: amt, tax: 0,
            currency: DEFAULT_CURRENCY,
            status: "pending",
            issued: txObj.date, due: null,
            items: JSON.stringify(invItems),
            notes: "", amt_paid: 0,
          });
          if (invOk) toast("🧾 Facture " + invNum + " créée !", "ok", T.blue);
        }
      }
    } catch (err) {
  ...
  // CRUD Cli
  const saveCli=async()=>{
    if(!fm.name||!fm.name.trim()){toast("⚠️ Le nom du client est obligatoire","err");return;}
    const snap={...fm}; // snapshot avant reset
    const c={
  ...
  // CRUD Inv
  const saveInv=async()=>{
    if(!fm.clientName||!fm.clientName.trim()){toast("⚠️ Le nom du client est obligatoire","err");return;}
    const snap={...fm}; // snapshot avant reset
    const items=(snap.items||[]).filter(it=>it.name&&parseFloat(it.price)>0);
    ...
    const tax=parseFloat(snap.tax)||0;
    const total=sub+tax;
    const inv={
      id:snap._edit?snap.id:xid(), uid,
      user_id:uid, // clé Supabase
      num:snap._edit?snap.num:nextNum(),
      clientId:snap.clientId||"", clientName:snap.clientName.trim(),
      phone:snap.phone||"",
      total, sub, tax,
      currency:snap.currency||DEFAULT_CURRENCY,
      status:snap.status||"pending",
      issued:snap.issued||today(), due:snap.due||"",
      items:items.map(it=>({...it,id:it.id||xid(),line:(it.qty||1)*(it.price||0)})),
      notes:snap.notes||"", payStatus:"unpaid", payRef:"", payProv:"",
    ...
    try{
      const s=await getSupa();
      const {data:fresh}=await s.from("invoices").select("*").eq("user_id",uid).order("created_at",{ascending:false});
      if(fresh&&fresh.length>0){
        setInvs(fresh.map(r=>({id:r.id,uid:r.user_id,num:r.number||"",clientId:r.client_id||"",clientName:r.client_name||"",phone:r.phone||"",total:parseFloat(r.total)||0,sub:parseFloat(r.subtotal)||0,tax:parseFloat(r.tax)||0,currency:r.currency||DEFAULT_CURRENCY,status:r.status||"pending",issued:r.issued||today(),due:r.due||"",items:typeof r.items==="string"?JSON.parse(r.items||"[]"):r.items||[],notes:r.notes||"",payStatus:r.status==="paid"?"paid":"unpaid",payRef:"",payProv:"",amtPaid:parseFloat(r.amt_paid)||0})));
      }
      toast(`✅ Facture ${inv.num} sauvegardée !`);
    }catch(e){
      console.warn("Re-fetch factures échoué:",e);
      toast(`✅ Facture ${inv.num} créée !`);
    }
    markUserActive(uid);
    trackUsage(); // v34
  };
...
<div class="tot">
  <div>Sous-total : <strong>${fp(inv.sub)}</strong></div>
  ${inv.tax>0?`<div>Taxes : <strong>${fp(inv.tax)}</strong></div>`:""}
  <div class="grand">TOTAL : ${fp(inv.total)}</div>
  ${amtPaid>0?`<div style="color:#006677;font-weight:700;margin-top:4px">Déjà payé : ${fp(amtPaid)}</div><div style="color:${reste>0?"#c05000":"#0a6e3d"};font-weight:900;font-size:16px">Reste à payer : ${fp(reste)}</div>`:""}
</div>
...
<div style="margin:16px 0;text-align:center;padding:14px;background:#f2fdf7;border-radius:12px;border:1px solid #00d47830"><div style="font-size:11px;font-weight:700;color:#555;margin-bottom:8px">📲 Scanner pour payer via Mobile Money</div><img src="https://api.qrserver.com/v1/create-qr-code/?size=140x140&data=${encodeURIComponent((window.location.origin||"https://vierafrik.com")+"/?pay="+inv.id)}" style="width:120px;height:120px;border-radius:8px" /><div style="font-size:10px;color:#888;margin-top:6px">Wave · Orange Money · MTN · vierafrik.com</div></div>`, `Facture-${inv.num}.pdf`);
  ...
  const doPay=async()=>{
    ...
    try{
      ...
      if(url){
        ...
        setInvs(next);
        await supaUpdate("invoices",{status:"pending",pay_ref:txId||"",pay_prov:prov},inv.id);
        setMdl(null);setFm({});
    ...
    <div>
      ...
      <div style={{background:`linear-gradient(135deg,${accent}14,${T.teal}08,${T.c2})`,border:`1px solid ${accent}30`,borderRadius:18,padding:"1.3rem 1.5rem",marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10,position:"relative",overflow:"hidden",boxShadow:`0 8px 32px ${accent}12`}}>
        ...
        <div style={{position:"relative"}}>
          ...
          <div style={{fontSize:11,color:T.sub2,marginTop:5}}>
            {gPct}% atteint · Bénéfice : <strong style={{color:profit>=0?T.gr:T.red}}><FmtHint v={Math.abs(profit)} size={12} color={profit>=0?T.gr:T.red} noHint={true}/></strong>
          </div>
        </div>
      ...
      <div className="pg-grid-kpi" style={{marginBottom:12}}>
        {[
          ...
          {ic:"👥",l:"Clients actifs",v:clis.filter(c=>c.status==="active").length,sub:`/ ${clis.length} total`,co:T.purple,bg:"rgba(144,96,255,.1)",grad:"rgba(144,96,255,.2)",isCount:true},
        ].map(({ic,l,v,sub,co,bg,grad,isCount})=>(
          <div key={l} style={{background:`linear-gradient(135deg,${T.c1},${T.c2})`,border:`1px solid ${co}28`,borderRadius:16,padding:"1.1rem",position:"relative",overflow:"hidden",transition:"transform .2s,box-shadow .2s"}}
            ...
            <div style={{fontSize:22,fontWeight:900,color:co,lineHeight:1,letterSpacing:"-.03em",marginBottom:4}}>
              {isCount || typeof v !== "number"
                ? <span style={{fontSize:22,fontWeight:900,color:co}}>{v}</span>
                : <FmtHint v={v} size={22} color={co}/>
              }
            </div>
    ...
    return(
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:11,flexWrap:"wrap",gap:7}}>
          ...
          <Btn ch="+ Nouvelle" onClick={()=>{setFm({issued:today(),status:"pending",tax:0,currency:DEFAULT_CURRENCY,items:[{id:xid(),name:"",qty:1,price:0}]});setMdl("inv");}}/>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))",gap:9}}>
          {invs.map(inv=>{
            ...
            <div key={inv.id} style={{background:T.c2,border:`1px solid ${T.border}`,borderRadius:15,padding:"1rem",position:"relative",overflow:"hidden",transition:"all .2s",animation:flashId===inv.id?"flashGreen .7s ease":"none"}}
              ...
              <div style={{marginBottom:8}}>
                <div style={{fontSize:21,fontWeight:900,color:T.gr,letterSpacing:"-.02em"}}>{fmtPrice(inv.total, inv.currency)}</div>
                {amtPaid>0&&(
                  <div style={{display:"flex",gap:10,marginTop:4,flexWrap:"wrap"}}>
                    <span style={{fontSize:10,color:T.teal,fontWeight:700}}>✅ Payé : {fmtPrice(amtPaid, inv.currency)}</span>
                    <span style={{fontSize:10,color:reste>0?T.orange:T.gr,fontWeight:700}}>{reste>0?`🕐 Reste : ${fmtPrice(reste, inv.currency)}`:"✔ Soldée"}</span>
                  </div>
                )}
              ...
              {inv.status!=="paid"&&(
                <button onClick={()=>{setFm({_encaisse:true,inv,encMontant:Math.max(0,inv.total-(inv.amtPaid||0))});setMdl("encaisse");}}
                  ...
                  💰 Encaisser paiement{reste<inv.total&&amtPaid>0?` (reste ${fmtPrice(reste, inv.currency)})`:""}
                </button>
              )}
  ...
  // ══════════════════════════════════════════════════════════
  //  MODULE EMPLOYÉS & PAIEMENTS
  //  - CRUD employés (Supabase : table "employees")
  //  - Paiement 1 clic (Supabase : table "payments")
  //  - Historique simple trié par date
  //  - Plan Free = 1 employé max / Pro = illimité
  //  - Devises internationales (aucun calcul fiscal)
  // ══════════════════════════════════════════════════════════

  const CURRENCIES = [
    "XOF","EUR","USD","GBP","CAD","CHF","XAF","NGN","GHS","KES",
    "MAD","DZD","TND","EGP","ZAR","MXN","BRL","INR","CNY","JPY",
  ];
  ...
  const PgEmployees = () => {
    ...
    const [mdlEmp, setMdlEmp]         = useState(null); // "add"|"edit"|"pay"|"history"
    const [fmEmp, setFmEmp]           = useState({});
    const [payTarget, setPayTarget]   = useState(null);
    const [histTarget, setHistTarget] = useState(null);
    const [histPays, setHistPays]     = useState([]);
    const [loadingH, setLoadingH]     = useState(false);
    const [saving, setSaving]         = useState(false);
    const [lastReceipt, setLastReceipt] = useState(null); // reçu après paiement employé
    ...
    const loadEmps = async () => {
      ...
      try {
        ...
        const { data, error } = await s
          ...
          .eq("user_id", uid)
          .order("created_at", { ascending: false });
        if (!error && data) setEmps(data);
      } catch(e) { console.error("employees load:", e); }
      finally { setLoadingE(false); }
    };

    // ── Charger les paiements (tous ou par employé) ──
    const loadPays = async (employee_id = null) => {
      setLoadingH(true);
      try {
        const s = await getSupa();
        let q = s.from("payments").select("*, employees(name)").eq("user_id", uid).order("created_at", { ascending: false }).limit(100);
        if (employee_id) q = q.eq("employee_id", employee_id);
        const { data, error } = await q;
        if (!error && data) {
          employee_id ? setHistPays(data) : setPays(data);
        }
    ...
    // ── Valider formulaire employé ──
    const validateEmp = () => {
      if (!fmEmp.name?.trim()) { toast("⚠️ Le nom est obligatoire", "err"); return false; }
      if (!fmEmp.salary || parseFloat(fmEmp.salary) <= 0) { toast("⚠️ Salaire invalide", "err"); return false; }
      return true;
    };
    ...
    const deleteEmployee = (emp) => {
      setConfirm({
        ...
        msg: `Supprimer ${emp.name} ? Ses paiements resteront dans l'historique.`,
    ...
    const payEmployee = async () => {
      if (!payTarget) return;
      setSaving(true);
      const empName = payTarget.name; // capturé avant le reset
      const payment = {
        id: xid(), user_id: uid,
        employee_id: payTarget.id,
        amount: payTarget.salary,
        currency: payTarget.currency || "XOF",
        date: today(),
        status: "paid",
        created_at: new Date().toISOString(),
      };
      setMdlEmp(null); setPayTarget(null);
      // ✅ toast déplacé APRÈS confirmation Supabase (pas avant)
      try {
        const s = await getSupa();
        ...
        if (error) {
          ...
          markUserActive(uid);
          trackUsage(); // v34
          toast(t("payDone"), "ok", T.gr); // ← ici, après succès confirmé
          setLastReceipt({
            receiptId: Date.now(),
            name: empName,
    ...
    // ── Ouvrir historique d'un employé ──
    const openHistory = async (emp) => {
      setHistTarget(emp);
      setMdlEmp("history");
      await loadPays(emp.id);
    };

    // ── Helpers ──
    const fmtAmt = (amt, cur) => `${new Intl.NumberFormat("fr-FR").format(Math.round(amt||0))} ${cur||""}`;
      ...
      <div style={{ animation: "slideUp .3s ease both" }}>
        ...
        {emps.length > 0 && (
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:16 }}>
            {[
              { l:"Équipe",   v: emps.length,                                        co: T.blue,  ic:"👷" },
              { l:"Masse salariale", v: (() => {
                  // Grouper par devise pour éviter de mélanger XOF + EUR + USD
                  const byCur = emps.reduce((acc, e) => {
                    const cur = e.currency || "XOF";
                    acc[cur] = (acc[cur] || 0) + e.salary;
                    return acc;
                  }, {});
                  const entries = Object.entries(byCur);
                  if (entries.length === 1) return fmtAmt(entries[0][1], entries[0][0]);
                  return entries.map(([cur, total]) => fmtAmt(total, cur)).join(" + ");
                })(),                                                                   co: T.orange, ic:"💰" },
              { l:"Paiements", v: pays.length,                                       co: T.gr,    ic:"✅" },
            ].map(({ l, v, co, ic }) => (
              <div key={l} style={{ background:`linear-gradient(135deg,${T.c1},${T.c2})`, border:`1px solid ${co}22`, borderRadius:14, padding:"11px 10px", textAlign:"center" }}>
                <div style={{ fontSize:18, marginBottom:4 }}>{ic}</div>
                <div style={{ fontWeight:900, fontSize:16, color:co, lineHeight:1 }}>{v}</div>
                <div style={{ fontSize:9, color:T.sub, fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", marginTop:3 }}>{l}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {loadingE ? (
          ...
          <div style={{ textAlign:"center", padding:"3rem 1.5rem", background:T.c1, border:`1px solid ${T.border}`, borderRadius:20 }}>
            <div style={{ fontSize:52, marginBottom:12 }}>👷</div>
            <div style={{ fontWeight:800, fontSize:15, marginBottom:6 }}>Aucun employé</div>
            <div style={{ fontSize:12, color:T.sub2, marginBottom:16, lineHeight:1.6 }}>Ajoutez votre première personne pour commencer à gérer et payer votre équipe.</div>
            <Btn ch={t("newEmp")} onClick={() => { setFmEmp({ currency: IS_AFRICA ? "XOF" : DETECTED_CURRENCY }); setMdlEmp("add"); }}/>
          </div>
        ) : (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {emps.map(emp => {
              const lp = lastPay(emp.id);
              const tp = totalPaid(emp.id);
              return (
                <div key={emp.id} style={{ background:`linear-gradient(135deg,${T.c1},${T.c2})`, border:`1px solid ${T.border}`, borderRadius:18, padding:"1.1rem", transition:"border-color .2s", animation: flashId===emp.id?"flashGreen .7s ease":"none" }}
                  onMouseEnter={e => e.currentTarget.style.borderColor=`${accent}44`}
                  onMouseLeave={e => e.currentTarget.style.borderColor=T.border}>
                  <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:10 }}>
                    {/* Avatar initiales */}
                    <div style={{ width:44, height:44, borderRadius:13, background:`linear-gradient(135deg,${accent},${T.teal})`, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:900, fontSize:15, color:T.ink, flexShrink:0, boxShadow:`0 4px 14px ${accent}33` }}>
                    ...
                    {/* Salaire */}
                    <div style={{ textAlign:"right", flexShrink:0 }}>
                      <div style={{ fontWeight:900, fontSize:17, color:accent, letterSpacing:"-.02em" }}>{fmtAmt(emp.salary, emp.currency)}</div>
                      <div style={{ fontSize:9, color:T.sub, fontWeight:600 }}>par période</div>
                    </div>
                  </div>
                  ...
                  {(lp || tp > 0) && (
                    <div style={{ display:"flex", gap:8, marginBottom:10 }}>
                      {tp > 0 && (
                        <div style={{ flex:1, background:`${T.gr}10`, border:`1px solid ${T.gr}20`, borderRadius:9, padding:"6px 9px" }}>
                          <div style={{ fontSize:8, color:T.sub, fontWeight:700, textTransform:"uppercase" }}>Total payé</div>
                          <div style={{ fontSize:12, fontWeight:800, color:T.gr }}>{fmtAmt(tp, emp.currency)}</div>
                        </div>
                      )}
                      {lp && (
                        <div style={{ flex:1, background:`${T.blue}10`, border:`1px solid ${T.blue}20`, borderRadius:9, padding:"6px 9px" }}>
                          <div style={{ fontSize:8, color:T.sub, fontWeight:700, textTransform:"uppercase" }}>Dernier paiement</div>
                          <div style={{ fontSize:11, fontWeight:700, color:T.blue }}>{lp.date}</div>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {/* Actions */}
                  <div style={{ display:"flex", gap:7 }}>
                    <button onClick={() => { setPayTarget(emp); setMdlEmp("pay"); }}
                      style={{ flex:2, padding:"10px", borderRadius:10, border:"none", background:`linear-gradient(135deg,${accent},${T.teal})`, color:T.ink, fontFamily:"inherit", fontWeight:900, fontSize:13, cursor:"pointer", boxShadow:`0 4px 14px ${accent}33`, display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
                      💸 {t("payEmp")}
                    </button>
                    <button onClick={() => openHistory(emp)}
                      style={{ flex:1, padding:"10px", borderRadius:10, border:`1px solid ${T.border}`, background:T.c2, color:T.sub2, fontFamily:"inherit", fontWeight:700, fontSize:11, cursor:"pointer" }}>
                      📋
                    </button>
                    <button onClick={() => { setFmEmp({...emp}); setMdlEmp("edit"); }}
                      style={{ flex:1, padding:"10px", borderRadius:10, border:`1px solid ${T.blue}33`, background:`${T.blue}10`, color:T.blue, fontFamily:"inherit", fontWeight:700, fontSize:11, cursor:"pointer" }}>
                      ✏️
                    </button>
                    <button onClick={() => deleteEmployee(emp)}
                      style={{ padding:"10px 12px", borderRadius:10, border:`1px solid rgba(255,34,85,.25)`, background:"rgba(255,34,85,.08)", color:T.red, fontFamily:"inherit", fontSize:11, cursor:"pointer" }}>
                      🗑
                    </button>
                  </div>
                </div>
        ...
        {pays.length > 0 && (
          <div style={{ marginTop:20 }}>
            ...
            <div style={{ background:T.c1, border:`1px solid ${T.border}`, borderRadius:16, overflow:"hidden" }}>
              {pays.slice(0, 10).map((p, i) => (
                <div key={p.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"11px 14px", borderBottom: i < Math.min(pays.length,10)-1 ? `1px solid ${T.border}` : "none" }}>
                  ...
                  <div style={{ fontWeight:800, fontSize:13, color:T.gr, flexShrink:0 }}>
        ...
        <Modal open={mdlEmp==="add"||mdlEmp==="edit"} onClose={()=>{setMdlEmp(null);setFmEmp({});}}
          title={mdlEmp==="edit"?"✏️ Modifier l'employé":t("newEmp")}>
          <FL l={t("empName")} ch={
            <input style={IS2} placeholder="Jean Koné" value={fmEmp.name||""} onChange={ev=>setFmEmp(f=>({...f,name:ev.target.value}))}/>
          }/>
          <FL l={t("empPhone")} ch={
            <input type="tel" style={IS2} placeholder="+225 07 000 0000" value={fmEmp.phone||""} onChange={ev=>setFmEmp(f=>({...f,phone:ev.target.value}))}/>
          }/>
          <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:10 }}>
            <FL l={t("empSalary")} ch={
              <input type="number" style={IS2} placeholder="150000" value={fmEmp.salary||""} onChange={ev=>setFmEmp(f=>({...f,salary:ev.target.value}))}/>
            }/>
            <FL l={t("empCurrency")}>
              <select style={IS2} value={fmEmp.currency||"XOF"} onChange={ev=>setFmEmp(f=>({...f,currency:ev.target.value}))}>
          ...
          <div style={{ display:"flex", gap:7 }}>
            <Btn dis={saving} ch={saving?"⏳…":mdlEmp==="edit"?"✅ Modifier":"✅ Enregistrer"} full
              onClick={mdlEmp==="edit" ? updateEmployee : addEmployee}/>
            <Btn v="g" ch={t("cancel")} onClick={()=>{setMdlEmp(null);setFmEmp({});}}/>
          </div>
        </Modal>

        {/* ── MODAL : Confirmer paiement ── */}
        <Modal open={mdlEmp==="pay"} onClose={()=>{setMdlEmp(null);setPayTarget(null);}} title={t("payConfirmTitle")}>
          {payTarget && (
            <>
              {/* Récap employé */}
              <div style={{ background:`linear-gradient(135deg,${T.c2},${T.c3})`, border:`1px solid ${accent}22`, borderRadius:16, padding:"1.2rem", marginBottom:16, textAlign:"center" }}>
                <div style={{ width:56, height:56, borderRadius:16, background:`linear-gradient(135deg,${accent},${T.teal})`, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:900, fontSize:20, color:T.ink, margin:"0 auto 10px", boxShadow:`0 6px 20px ${accent}44` }}>
                  {payTarget.name.split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase()}
                </div>
                <div style={{ fontWeight:800, fontSize:17, marginBottom:4 }}>{payTarget.name}</div>
                <div style={{ fontWeight:900, fontSize:28, color:accent, letterSpacing:"-.03em" }}>
                  {fmtAmt(payTarget.salary, payTarget.currency)}
                </div>
                <div style={{ fontSize:11, color:T.sub2, marginTop:4 }}>
                  📅 Date : <strong>{today()}</strong>
                </div>
              </div>
              <div style={{ background:`${T.gr}08`, border:`1px solid ${T.gr}20`, borderRadius:9, padding:"9px 12px", marginBottom:14, fontSize:11, color:T.sub2, textAlign:"center" }}>
                ✅ Ce paiement sera enregistré dans l'historique
              </div>
              <div style={{ display:"flex", gap:8 }}>
                <Btn full dis={saving} ch={saving?"⏳…":"✅ Confirmer le paiement"} onClick={payEmployee}/>
                <Btn full v="g" ch={t("cancel")} onClick={()=>{setMdlEmp(null);setPayTarget(null);}}/>
              </div>
            </>
          )}
        </Modal>
        ...
        <Modal open={mdlEmp==="receipt"} onClose={()=>{setMdlEmp(null);setLastReceipt(null);}} title="🧾 Reçu de salaire">
          {lastReceipt && (() => {
  ...
  <div class="footer">
    ...
    Ce document constitue une preuve de paiement de salaire.
            ...
            return (
              <div style={{ textAlign:"center" }}>
                {/* Icône succès */}
                <div style={{ width:64, height:64, borderRadius:"50%", background:`${T.gr}18`, border:`2px solid ${T.gr}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, margin:"0 auto 6px" }}>✅</div>
                <div style={{ fontSize:11, color:T.sub, marginBottom:18, fontWeight:600 }}>🧾 {receiptNum}</div>

                {/* Bloc lignes */}
                <div style={{ background:`linear-gradient(135deg,${T.c2},${T.c3})`, border:`1px solid ${T.gr}22`, borderRadius:16, padding:"1rem", marginBottom:14, textAlign:"left" }}>
                  {[
                    { icon:"🏢", label:"Entreprise", val: businessName },
                    { icon:"👤", label:"Employé",    val: lastReceipt.name },
                    { icon:"💰", label:"Montant net", val: fmtReceiptAmt(lastReceipt.amount, lastReceipt.currency), green:true },
                    { icon:"📅", label:"Date",        val: lastReceipt.date },
                    { icon:"📌", label:"Statut",      val: "✅ Payé", badge:true },
                  ].map(({icon,label,val,green,badge})=>(
                    <div key={label} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 0", borderBottom:`1px solid ${T.border}` }}>
                      <span style={{ fontSize:12, color:T.sub2 }}>{icon} {label}</span>
                      {badge
                        ? <span style={{ fontWeight:800, fontSize:12, color:T.gr, background:`${T.gr}15`, padding:"3px 10px", borderRadius:20 }}>{val}</span>
                        : <span style={{ fontWeight:green?900:700, fontSize:green?15:13, color:green?T.gr:T.text }}>{val}</span>
                      }
                    </div>
                  ))}
                </div>
                ...
                <button onClick={()=>{setMdlEmp(null);setLastReceipt(null);}}
                  style={{ width:"100%", padding:"11px", background:T.gr, border:"none", borderRadius:11, color:T.ink, cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:900 }}>
                  ✅ Fermer
                </button>
              </div>
            );
        ...
        <Modal open={mdlEmp==="history"} onClose={()=>{setMdlEmp(null);setHistTarget(null);setHistPays([]);}}
          title={`📋 ${histTarget?.name || "Historique"}`}>
          {loadingH ? (
            <div style={{ textAlign:"center", padding:"2rem", color:T.sub }}>⏳ Chargement…</div>
          ) : histPays.length === 0 ? (
            ...
            <>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
                <div style={{ background:`${T.gr}10`, border:`1px solid ${T.gr}20`, borderRadius:12, padding:"10px 12px", textAlign:"center" }}>
                  <div style={{ fontSize:10, color:T.sub, fontWeight:700, textTransform:"uppercase" }}>Total versé</div>
                  <div style={{ fontWeight:900, fontSize:16, color:T.gr, marginTop:3 }}>
                    {fmtAmt(histPays.reduce((s,p)=>s+p.amount,0), histTarget?.currency)}
                  </div>
                </div>
                <div style={{ background:`${T.blue}10`, border:`1px solid ${T.blue}20`, borderRadius:12, padding:"10px 12px", textAlign:"center" }}>
                  <div style={{ fontSize:10, color:T.sub, fontWeight:700, textTransform:"uppercase" }}>Nb paiements</div>
                  <div style={{ fontWeight:900, fontSize:16, color:T.blue, marginTop:3 }}>{histPays.length}</div>
                </div>
              </div>
              {/* Liste */}
  ...
  const PgPlans=()=>{
    ...
    const doSubPayment=async(planKey,planObj)=>{
      ...
      try{
        const res=await fetch("/api/fedapay",{
          method:"POST",
          headers:{"Content-Type":"application/json"},
          body:JSON.stringify({action:"initialize",amount:planObj.price,email:ses.email,description:"Abonnement VierAfrik "+planObj.label,plan:planKey,uid:ses.id})
        });
        const data=await res.json();
        const url=data?.payment_url||data?.url||data?.transaction?.payment_url;
    ...
    <div>
      ...
      <div style={{
        ...
        border:`2px solid ${subStatusColor}44`,borderRadius:18,
        padding:"1.3rem 1.5rem",marginBottom:18,
        position:"relative",overflow:"hidden",
      }}>
        <div style={{position:"absolute",right:-30,top:-30,width:120,height:120,borderRadius:"50%",background:`${subStatusColor}06`}}/>
        <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:".06em",color:T.sub,marginBottom:6}}>
          Statut de votre abonnement
        </div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
          <div>
            <div style={{fontWeight:900,fontSize:18,color:subStatusColor,marginBottom:2}}>{subStatusLabel}</div>
            {isSubActive&&subscription&&(
              <div style={{fontSize:11,color:T.sub2}}>
                Plan <strong style={{color:PLANS[subscription.plan]?.col||T.gr}}>{PLANS[subscription.plan]?.emoji} {PLANS[subscription.plan]?.label}</strong>
                {" · "}
                Expire le {new Date(subscription.expires_at).toLocaleDateString("fr-FR",{day:"numeric",month:"long",year:"numeric"})}
              </div>
            )}
            {subscription?.status==="expired"&&(
              <div style={{fontSize:11,color:T.red,marginTop:2}}>
                Expiré le {new Date(subscription.expires_at).toLocaleDateString("fr-FR",{day:"numeric",month:"long",year:"numeric"})}
              </div>
            )}
          ...
          {isSubActive&&subscription&&(
            <div style={{textAlign:"right",flexShrink:0}}>
              <div style={{fontWeight:900,fontSize:36,color:subStatusColor,lineHeight:1,letterSpacing:"-.04em"}}>
                {subscription.days_left}
              </div>
              <div style={{fontSize:9,color:T.sub,fontWeight:600,textTransform:"uppercase",letterSpacing:".06em"}}>jours restants</div>
              <div style={{marginTop:6,background:T.c3,borderRadius:20,height:5,width:100,overflow:"hidden"}}>
                <div style={{
        ...
        {subscription?.status==="expired"&&(
          <button onClick={()=>{
            const p=PLANS[subscription.plan]||PLANS.pro;
            const pk=subscription.plan||"pro";
            doSubPayment(pk,p);
          }} style={{
            ...
            boxShadow:`0 6px 20px ${T.gr}44`,
          }}>
            🔄 Renouveler mon abonnement {PLANS[subscription.plan]?.label||"Pro"}
          </button>
        )}
      </div>
        ...
        <div style={{background:`linear-gradient(135deg,${T.gold}10,${T.orange}06)`,border:`1px solid ${T.gold}33`,borderRadius:12,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontSize:11,fontWeight:700,color:T.gold}}>
              Prix affichés en FCFA + équivalent en devise locale
            </div>
            <div style={{fontSize:10,color:T.sub2,marginTop:1}}>
            ...
            <div key={k} style={{background:T.c1,border:`2px solid ${isCur?p.col:T.border}`,borderRadius:18,padding:"1.4rem",position:"relative",overflow:"hidden",transition:"all .2s"}}
              ...
              <div style={{marginBottom:14}}>
                ...
                {!isAfrica&&p.price>0&&(
                  <div style={{marginTop:3,fontSize:12,color:T.sub2,fontWeight:600}}>
                    {currency==="EUR"
                      ?`~${(p.price*FCFA_TO_EUR).toFixed(2).replace(".",",")} €/mois`
                      :`~$${(p.price*FCFA_TO_USD).toFixed(2)}/mois`}
                  </div>
                )}
              </div>
              ...
              {isCur&&k!=="free"&&isSubActive&&(
                <div style={{marginTop:15,textAlign:"center",fontSize:11,color:p.col,fontWeight:700}}>
                  ✅ Actif · {subscription?.days_left} jour{subscription?.days_left>1?"s":""} restant{subscription?.days_left>1?"s":""}
                </div>
              )}
              {isCur&&k!=="free"&&!isSubActive&&(
                <Btn full sx={{marginTop:15}} v="p" ch={`🔄 Renouveler ${p.label}`} onClick={()=>doSubPayment(k,p)}/>
              )}
            </div>
          );
        })}
    ...
    <div>
      ...
      <div className="pg-grid-2">
        ...
        <div style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:16,padding:"1.4rem"}}>
          ...
          {/* ── Devise principale ── */}
          <div style={{marginBottom:12}}>
            <label style={{display:"block",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:".07em",color:T.sub,marginBottom:6}}>
              🌍 Devise principale
            </label>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:7}}>
              {[
                {id:"XOF", label:"FCFA",  flag:"🇨🇮", desc:"Afrique CFA"},
                {id:"EUR", label:"Euro",  flag:"🇪🇺", desc:"Europe"},
                {id:"USD", label:"USD",   flag:"🇺🇸", desc:"International"},
              ].map(({id,label,flag,desc})=>(
                <div key={id}
                  onClick={()=>setLocalProfile(p=>({...p,currency:id}))}
                  style={{
                    padding:"10px 6px",borderRadius:11,cursor:"pointer",textAlign:"center",
                    background:localProfile.currency===id?`${accent}18`:T.c3,
                    border:`2px solid ${localProfile.currency===id?accent:T.border}`,
                    transition:"all .18s",
                  }}>
                  <div style={{fontSize:18,marginBottom:2}}>{flag}</div>
                  <div style={{fontWeight:800,fontSize:11,color:localProfile.currency===id?accent:T.text}}>{label}</div>
                  <div style={{fontSize:9,color:T.sub,marginTop:1}}>{desc}</div>
                </div>
              ))}
            </div>
            {localProfile.currency!=="XOF"&&(
              <div style={{marginTop:7,padding:"7px 10px",background:`${T.blue}10`,border:`1px solid ${T.blue}22`,borderRadius:8,fontSize:10,color:T.sub2,lineHeight:1.5}}>
                ℹ️ Les montants s'afficheront en {localProfile.currency} avec une équivalence FCFA indicative.
                Les données sont toujours stockées en FCFA.
              </div>
            )}
            {localProfile.currency==="XOF"&&(
              <div style={{marginTop:7,padding:"7px 10px",background:`${T.gr}10`,border:`1px solid ${T.gr}22`,borderRadius:8,fontSize:10,color:T.sub2,lineHeight:1.5}}>
                ℹ️ Les montants s'afficheront en FCFA avec une équivalence Euro indicative (~1 EUR = 656 FCFA).
              </div>
            )}
          </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{paddingTop:54,paddingBottom:70,position:"relative",zIndex:1}}>
        {/* BANNIERE ALERTE CLE SUPABASE INVALIDE */}
        {!isKeyValid()&&(
      ...
      <Modal open={mdl==="inv"} onClose={()=>{setMdl(null);setFm({});}} title={fm._edit?t("editInvoice"):t("newInvoice")} wide>
        <div className="pg-grid-2">
          ...
          <FL l="Devise">
            <select style={IS} value={fm.currency||DEFAULT_CURRENCY} onChange={ev=>setFm(f=>({...f,currency:ev.target.value}))}>
              {CURRENCIES.map(c=><option key={c} value={c}>{c}</option>)}
            </select>
          </FL>
        ...
        <div style={{marginBottom:12}}>
          ...
          {(fm.items||[]).some(it=>it.price>0)&&(
            <div style={{textAlign:"right",marginTop:7,fontWeight:700,color:T.gr,fontSize:13}}>
              Total : {fmtPrice((fm.items||[]).reduce((s,it)=>s+(it.qty||1)*(it.price||0),0)+(fm.tax||0), fm.currency||DEFAULT_CURRENCY)}
            </div>
          )}
        </div>
      ...
      <Modal open={mdl==="encaisse"} onClose={()=>{setMdl(null);setFm({});}} title={t("collectPayment")}>
        {fm.inv&&(
          <>
            <div style={{background:T.c2,border:`1px solid ${T.border}`,borderRadius:11,padding:"12px 14px",marginBottom:16}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:8}}>
                <div style={{textAlign:"center"}}>
                  ...
                  <div style={{fontWeight:900,fontSize:15,color:T.text}}>{fmtPrice(fm.inv.total, fm.inv.currency||DEFAULT_CURRENCY)}</div>
                </div>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:9,color:T.sub,fontWeight:700,textTransform:"uppercase",marginBottom:3}}>{t("alreadyPaid")}</div>
                  <div style={{fontWeight:900,fontSize:15,color:T.teal}}>{fmtPrice(fm.inv.amtPaid||0, fm.inv.currency||DEFAULT_CURRENCY)}</div>
                </div>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:9,color:T.sub,fontWeight:700,textTransform:"uppercase",marginBottom:3}}>{t("remaining")}</div>
                  <div style={{fontWeight:900,fontSize:15,color:T.orange}}>{fmtPrice(Math.max(0,fm.inv.total-(fm.inv.amtPaid||0)), fm.inv.currency||DEFAULT_CURRENCY)}</div>
                </div>
              </div>
              {(fm.inv.amtPaid||0)>0&&(
            ...
            <FL l={`Montant encaissé (${fm.inv.currency||DEFAULT_CURRENCY})`} ch={
              <input type="number" style={IS} placeholder={`Max : ${fmtPrice(Math.max(0,fm.inv.total-(fm.inv.amtPaid||0)), fm.inv.currency||DEFAULT_CURRENCY)}`}
                value={fm.encMontant||""}
      ...
      {showUpgradeWall&&(
        <div onClick={()=>setShowUpgradeWall(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:980,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(18px)",padding:"16px"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:`linear-gradient(160deg,${T.c1},${T.c2})`,border:`2px solid ${T.gr}55`,borderRadius:28,padding:"2rem 1.8rem",width:"100%",maxWidth:420,textAlign:"center",boxShadow:`0 40px 120px rgba(0,0,0,.95)`,animation:"pop .3s cubic-bezier(.34,1.56,.64,1)"}}>
            ...
            <div style={{fontWeight:900,fontSize:30,color:T.gr,letterSpacing:"-.04em",lineHeight:1,marginBottom:4}}>4 900 FCFA</div>
            <div style={{fontSize:11,color:T.sub,marginBottom:18}}>/mois · Annulable à tout moment</div>
            <button onClick={()=>{setShowUpgradeWall(false);setPage("plans");}} style={{width:"100%",padding:"15px",borderRadius:14,border:"none",background:`linear-gradient(135deg,${T.gr},${T.teal})`,color:T.ink,fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",fontWeight:900,fontSize:16,cursor:"pointer",marginBottom:10,boxShadow:`0 8px 28px ${T.gr}44`}}>
              ⚡ Passer en Pro maintenant →
            </button>
            <button onClick={()=>setShowUpgradeWall(false)} style={{width:"100%",padding:"11px",borderRadius:12,border:`1px solid ${T.border}`,background:"transparent",color:T.sub2,fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",fontWeight:600,fontSize:12,cursor:"pointer"}}>
              Continuer à explorer (limité)
            </button>
            <div style={{marginTop:12,fontSize:10,color:T.sub}}>🔒 Paiement sécurisé · FedaPay · Mobile Money</div>
          </div>
        </div>
      )}
      ...
      <Modal open={mdl==="pay"} onClose={()=>{setMdl(null);setFm({});}} title={t("payNow")}>
        {fm.inv&&(
          <div style={{background:T.c2,border:`1px solid ${T.border}`,borderRadius:11,padding:"12px 14px",marginBottom:16}}>
            ...
            <div style={{fontWeight:900,fontSize:22,color:T.gr,letterSpacing:"-.02em"}}>{fmtPrice(fm.inv.total, fm.inv.currency||DEFAULT_CURRENCY)}</div>
        ...
        <div style={{marginBottom:10}}>
          ...
          <Btn full ch={fm.prov?`💳 ${fm.prov.toUpperCase()}${fm.amount?", "+fmtPrice(parseFloat(fm.amount)||0, DEFAULT_CURRENCY):""}` : t("chooseOperator")} onClick={async()=>{
            ...
            try{
              ...
              const txId=data?.transaction_id||data?.transaction?.id;
              if(url){
                if(txId) console.info("[FedaPay] txId:", txId); // tracé pour webhook manuel
                setMdl(null);setFm({});
                toast("🔗 Redirection…");
                setTimeout(()=>window.location.href=url,800);
              } else {
