...
<script>window.onload=()=>window.print();</script></body></html>`);
  ...
  // Coach IA — moteur local intelligent basé sur les données réelles de l'utilisateur
  // Pas d'appel réseau → fonctionne toujours, réponses instantanées
  const sendAI=useCallback(()=>{
    ...
    setTimeout(()=>{
      try{
        ...
        const overdueInvs=invs.filter(i=>i.status==="overdue").length;
        const topClientEntry=Object.entries(
          txs.filter(t=>t.type==="sale"&&t.who)
            .reduce((a,t)=>{a[t.who]=(a[t.who]||0)+t.amount;return a;},{})
        ).sort((a,b)=>b[1]-a[1])[0];
        let reply="";
        if(q.includes("vente")||q.includes("gagner")||q.includes("argent")||q.includes("revenu")||q.includes("chiffre")){
          if(sales>0){
            reply=`💰 Ce mois : ${fmtf(sales)} de ventes (${mTxs.filter(t=>t.type==="sale").length} transactions). `+
              (topClientEntry?`Top client : ${topClientEntry[0]} → ${fmtf(topClientEntry[1])}. Contactez-le pour une nouvelle commande 📱.`:`Continuez à enregistrer chaque vente pour suivre votre progression.`);
          }else{
            reply=`🎯 Aucune vente ce mois encore. Publiez votre service dans "Action Rapide" dès maintenant — vos premiers clients peuvent vous appeler ou vous écrire sur WhatsApp directement 🌍`;
          }
        }else if(q.includes("dépense")||q.includes("charge")||q.includes("coût")||q.includes("depense")){
          reply=exps>0
            ?`📤 Dépenses ce mois : ${fmtf(exps)}. `+(profit>=0?`✅ Rentable : ${fmtf(profit)} de bénéfice net. Bien géré !`:`⚠️ Déficit de ${fmtf(Math.abs(profit))}. Identifiez les charges à réduire en priorité.`)
            :`📝 Aucune dépense enregistrée. Notez toutes vos charges (transport, matières, etc.) pour connaître votre vraie rentabilité.`;
        }else if(q.includes("bénéfice")||q.includes("profit")||q.includes("rentable")||q.includes("benefice")){
          reply=profit>=0
            ?`📈 Bénéfice net : ${fmtf(profit)} (${gPct}% de l'objectif ${fmtf(goal)}). `+(gPct>=100?`🏆 Objectif dépassé ! Réinvestissez 20% dans votre croissance.`:`Il vous reste ${fmtf(Math.max(0,goal-profit))} pour atteindre votre cible ce mois.`)
            :`⚠️ Résultat négatif : -${fmtf(Math.abs(profit))}. Action immédiate : augmenter les ventes OU réduire les dépenses non essentielles. Objectif : ${fmtf(goal)}.`;
        }else if(q.includes("client")||q.includes("prospect")||q.includes("trouver")||q.includes("contact")){
          reply=activeClients>0
            ?`👥 ${activeClients} client${activeClients>1?"s":""} actif${activeClients>1?"s":""}. `+
              (overdueInvs>0?`Relancez d'abord vos ${overdueInvs} facture(s) en retard par WhatsApp. `:`Tous les clients sont à jour ✅. `)+
              `Pour de nouveaux clients : publiez dans "Action Rapide" 🚀`
            :`🌍 Ajoutez vos premiers clients. Allez dans "Action Rapide" pour être visible, puis enregistrez chaque contact dans "Clients" pour les suivre.`;
        }else if(q.includes("facture")||q.includes("paiement")||q.includes("impayé")||q.includes("retard")||q.includes("recouvr")){
          reply=overdueInvs>0
            ?`🔴 ${overdueInvs} facture${overdueInvs>1?"s":""} en retard. Allez dans "Factures" → bouton 📲 WhatsApp → envoyez ce message : "Bonjour, petit rappel pour votre facture — merci de régler dès que possible 🙏"`
            :invs.length>0
              ?`✅ Toutes vos factures sont à jour ! ${invs.filter(i=>i.status==="paid").length} payée(s). Créez une facture pour chaque nouvelle vente, c'est professionnel et ça accélère les paiements.`
              :`📄 Créez votre première facture depuis l'onglet "Factures". Une facture professionnelle rassure le client et accélère le paiement.`;
        }else if(q.includes("objectif")||q.includes("goal")||q.includes("cible")||q.includes("target")){
          reply=`🎯 Objectif : ${fmtf(goal)}/mois. Actuellement à ${gPct}% (${fmtf(profit)} de bénéfice). `+
            (gPct>=100?`Félicitations 🏆 Pensez à hausser votre objectif pour continuer à grandir.`:
             gPct>50?`Bonne trajectoire ! Encore ${fmtf(Math.max(0,goal-profit))} à réaliser ce mois.`:
             `Concentrez-vous sur ${mTxs.filter(t=>t.type==="sale").length>0?Math.ceil((goal-profit)/Math.max(1,sales/Math.max(1,mTxs.filter(t=>t.type==="sale").length)))+" ventes supplémentaires":"augmenter vos ventes quotidiennement"} 💪`);
        }else if(q.includes("mobile money")||q.includes("orange")||q.includes("wave")||q.includes("mtn")||q.includes("encaisser")||q.includes("payer")){
          reply=`💳 Pour encaisser : bouton "💰 Gagner" → "Encaisser" → Orange Money, MTN ou Wave. `+
            (invs.filter(i=>i.status==="pending"||i.status==="overdue").length>0
              ?`Vous avez ${invs.filter(i=>i.status!=="paid").length} facture(s) non payée(s). Envoyez le lien de paiement depuis "Factures" → 💳 Payer.`
              :`Créez une facture, puis utilisez "💳 Payer" pour envoyer le lien Mobile Money directement à votre client.`);
        }else if(q.includes("stratégie")||q.includes("conseil")||q.includes("améliorer")||q.includes("développer")||q.includes("comment")){
          const idx=Math.floor(Date.now()/10000)%3;
          const conseils=[
            `🚀 3 priorités cette semaine : 1) Relancez vos ${activeClients} clients actifs 2) Publiez dans "Action Rapide" 3) Créez une facture pour chaque vente. La régularité fait la différence 🌍`,
            `💡 Notez chaque vente dans l'app, même les petites. En 30 jours vous verrez exactement quels clients et services sont les plus rentables 📊`,
            `🎯 ${activeClients>0?`Vos ${activeClients} clients actifs sont votre ressource principale. Contactez-en un aujourd'hui pour une nouvelle commande.`:`Trouvez vos premiers clients via "Action Rapide". Publiez votre service et répondez vite aux contacts.`}`,
          ];
          reply=conseils[idx];
        }else if(q.includes("bonjour")||q.includes("salut")||q.includes("hello")||q.includes("bonsoir")||q.includes("bonne")){
          reply=`Bonjour ! 👋 `+(sales>0?`Ce mois : ${fmtf(sales)} de ventes, ${fmtf(profit)} de bénéfice. `:`Prêt à vous aider ! `)+
            `Posez votre question sur vos ventes, clients, factures ou stratégie. Je connais vos données en temps réel 🌍`;
        }else if(q.includes("merci")||q.includes("super")||q.includes("bravo")||q.includes("parfait")){
          reply=`🙏 Avec plaisir ! C'est vous qui faites le vrai travail. `+(gPct>0?`${gPct}% de l'objectif atteint — continuez ! `:`Chaque action compte. `)+`N'hésitez pas si vous avez d'autres questions 💪`;
        }else{
          const idx=Math.floor(Date.now()/10000)%3;
          const general=[
            `📊 Votre situation aujourd'hui : ${fmtf(sales)} de ventes · ${fmtf(exps)} de dépenses · ${fmtf(profit)} de bénéfice · ${activeClients} clients actifs${overdueInvs>0?` · ${overdueInvs} facture(s) en retard`:""}. Sur quoi souhaitez-vous un conseil ?`,
            `🌍 Je vois vos données en temps réel. Posez votre question sur : vos ventes (${fmtf(sales)}), vos ${activeClients} clients, vos factures${overdueInvs>0?` (${overdueInvs} en retard)`:""}. Je vous donne un conseil concret et actionnable.`,
            `💡 ${gPct}% de l'objectif atteint. ${activeClients} clients actifs${overdueInvs>0?`, ${overdueInvs} facture(s) en retard`:""
            }. Dites-moi ce qui vous préoccupe : ventes, dépenses, clients, factures ou stratégie ?`,
          ];
          reply=general[idx];
        }
        setChat(h=>[...h,{r:"ai",t:reply}]);
      }catch(e){
        setChat(h=>[...h,{r:"ai",t:"🌍 Je suis votre Coach VierAfrik. Posez votre question sur vos ventes, clients, factures ou stratégie et je vous donne un conseil basé sur vos données réelles."}]);
      }finally{
        setCL(false);
      }
    },600);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[cMsg,sales,exps,profit,gPct,goal,clis,invs,txs,mTxs]);
  ...
  const NAV_BOTTOM=[{id:"action",ic:"⚡",lb:"Action"},{id:"dash",ic:"📊",lb:"Accueil"},{id:"tx",ic:"💸",lb:"Ventes"},{id:"reseau",ic:"🗺️",lb:"Réseau"},{id:"cli",ic:"👥",lb:"Clients"},{id:"prefs",ic:"⚙️",lb:"Compte"}];
