...
<script>window.onload=()=>window.print();</script></body></html>`);
  ...
  // Coach IA — moteur local, fonctionne sans dépendance réseau
  const sendAI=()=>{
    if(!cMsg.trim())return;
    const msg=cMsg.trim();
    setCMsg("");
    ...
    setTimeout(()=>{
      try{
        ...
        if(q.includes("vente")||q.includes("gagner")||q.includes("argent")||q.includes("revenu")){
          r=sales>0
            ?`💰 Ce mois : ${fmtf(sales)} de ventes (${mTxs.filter(t=>t.type==="sale").length} transactions). `+(top?`Top client : ${top[0]} — ${fmtf(top[1])}. Relancez-le 📱.`:`Continuez à enregistrer chaque vente.`)
            :`🎯 Aucune vente ce mois encore. Publiez votre service dans "Action Rapide" pour que les clients vous contactent directement 🌍`;
        }else if(q.includes("dépense")||q.includes("charge")||q.includes("depense")||q.includes("coût")){
          r=exps>0
            ?`📤 Dépenses ce mois : ${fmtf(exps)}. `+(profit>=0?`✅ Bénéfice net : ${fmtf(profit)}. Vous êtes rentable !`:`⚠️ Déficit de ${fmtf(Math.abs(profit))}. Réduisez les charges non essentielles.`)
          ...
          r=profit>=0
            ?`📈 Bénéfice net : ${fmtf(profit)} — ${gPct}% de l'objectif ${fmtf(goal)}. `+(gPct>=100?`🏆 Objectif dépassé ! Réinvestissez 20% dans votre croissance.`:`Encore ${fmtf(Math.max(0,goal-profit))} pour atteindre la cible.`)
            :`⚠️ Résultat : -${fmtf(Math.abs(profit))}. Priorité : augmenter les ventes ou réduire les dépenses. Objectif mensuel : ${fmtf(goal)}.`;
        }else if(q.includes("client")||q.includes("trouver")||q.includes("prospect")){
          r=ac>0
            ?`👥 ${ac} client${ac>1?"s":""} actif${ac>1?"s":""}. `+(ov>0?`Relancez d'abord vos ${ov} facture(s) en retard par WhatsApp. `:`Tous à jour ✅. `)+`Pour de nouveaux clients : publiez dans "Action Rapide" 🚀`
            :`🌍 Allez dans "Action Rapide" pour être visible, puis enregistrez chaque contact dans "Clients" pour les suivre.`;
        }else if(q.includes("facture")||q.includes("paiement")||q.includes("retard")||q.includes("impayé")){
          r=ov>0
            ?`🔴 ${ov} facture${ov>1?"s":""} en retard. Allez dans "Factures" → bouton 📲 WhatsApp → envoyez : "Bonjour, petit rappel pour votre facture — merci 🙏"`
            :invs.length>0
              ?`✅ Toutes vos factures sont à jour ! Créez une facture pour chaque nouvelle vente, c'est professionnel.`
              :`📄 Créez votre première facture depuis l'onglet "Factures" — ça accélère les paiements.`;
        }else if(q.includes("objectif")||q.includes("cible")||q.includes("goal")){
          r=`🎯 Objectif : ${fmtf(goal)}/mois. Vous êtes à ${gPct}% avec ${fmtf(profit)} de bénéfice. `+(gPct>=100?`Félicitations 🏆 Pensez à rehausser votre objectif !`:gPct>50?`Encore ${fmtf(Math.max(0,goal-profit))} à réaliser ce mois.`:`Augmentez vos ventes quotidiennes pour y arriver 💪`);
        }else if(q.includes("mobile money")||q.includes("orange")||q.includes("wave")||q.includes("mtn")||q.includes("encaisser")){
          r=`💳 Pour encaisser : bouton "💰 Gagner" → "Encaisser" → choisissez Orange Money, MTN ou Wave. `+(invs.filter(i=>i.status!=="paid").length>0?`Vous avez ${invs.filter(i=>i.status!=="paid").length} facture(s) non payée(s) → "Factures" → 💳 Payer.`:`Créez une facture puis utilisez "💳 Payer" pour envoyer le lien de paiement.`);
        }else if(q.includes("stratégie")||q.includes("conseil")||q.includes("comment")||q.includes("améliorer")||q.includes("développer")){
          r=[
            `🚀 3 priorités cette semaine : 1) Relancez vos ${ac} clients actifs 2) Publiez dans "Action Rapide" 3) Créez une facture pour chaque vente. La régularité crée la croissance 🌍`,
            `💡 Notez chaque vente dans l'app. En 30 jours vous verrez exactement quels clients et services sont les plus rentables 📊`,
            `🎯 ${ac>0?`Vos ${ac} clients actifs sont votre ressource principale. Contactez-en un aujourd'hui pour une nouvelle commande.`:`Publiez votre service dans "Action Rapide" et répondez rapidement aux contacts.`}`,
          ][Math.floor(Date.now()/10000)%3];
        }else if(q.includes("bonjour")||q.includes("salut")||q.includes("hello")||q.includes("bonsoir")){
          r=`Bonjour ! 👋 `+(sales>0?`Ce mois : ${fmtf(sales)} de ventes et ${fmtf(profit)} de bénéfice. `:`Prêt à vous aider à développer votre business ! `)+`Posez votre question sur vos ventes, clients, factures ou stratégie 🌍`;
        }else if(q.includes("merci")||q.includes("super")||q.includes("bravo")||q.includes("parfait")){
          r=`🙏 Avec plaisir ! C'est vous qui faites le travail sur le terrain. `+(gPct>0?`${gPct}% de l'objectif atteint — continuez ! `:`Chaque action compte. `)+`N'hésitez pas si vous avez d'autres questions 💪`;
        }else{
          r=[
            `📊 Votre situation : ${fmtf(sales)} de ventes · ${fmtf(profit)} de bénéfice · ${ac} clients actifs${ov>0?` · ${ov} facture(s) en retard`:""}. Sur quoi voulez-vous un conseil ?`,
            `🌍 Je connais vos données en temps réel. Posez votre question : ventes (${fmtf(sales)}), ${ac} clients, factures${ov>0?` (${ov} en retard)`:""}. Je vous donne un conseil concret.`,
            `💡 ${gPct}% de l'objectif atteint. ${ac} clients actifs. Dites-moi ce qui vous préoccupe : ventes, dépenses, clients, factures ou stratégie ?`,
          ][Math.floor(Date.now()/10000)%3];
        }
