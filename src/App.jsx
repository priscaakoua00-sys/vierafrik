...
export default function App(){
  ...
  const updSes=async(upd)=>{
    // ── SÉCURITÉ : le plan ne peut PAS être modifié côté client ──
    // Seul le webhook NotchPay (via /api/notchpay) peut changer le plan
    // via update_user_plan RPC côté serveur.
    // On retire silencieusement tout changement de plan venant du client
    // sauf pour le downgrade vers "free" (qui ne donne aucun avantage).
    const safeUpd = {...upd};
    if(safeUpd.plan && safeUpd.plan !== "free") {
      // Vérifier que le plan demandé est bien confirmé dans Supabase Auth
      try {
        const supa = await getSupa();
        const {data:{user}} = await supa.auth.getUser();
        const confirmedPlan = user?.user_metadata?.plan || "free";
        // N'accepter la mise à jour que si le plan en base correspond
        if(confirmedPlan !== safeUpd.plan) {
          console.warn("[VierAfrik] Plan non confirmé côté serveur — ignoré");
          delete safeUpd.plan; // Ne pas mettre à jour le plan localement
        }
      } catch(e) {
        delete safeUpd.plan; // En cas d'erreur, ignorer le changement de plan
      }
    }
    setSes(prev=>{
      const ns={...prev,...safeUpd};
      // Mettre à jour les métadonnées utilisateur dans Supabase
      // (hors plan — le plan est géré côté serveur uniquement)
      getSupa().then(supa=>{
        supa.auth.updateUser({data:{
          name:ns.name,
          business:ns.business,
          // plan intentionnellement absent — ne pas permettre l'auto-modification
          accent:ns.accent,
          goal:ns.goal,
          phone:ns.phone,
          country:ns.country,
...
function Dashboard({ses,logout,updSes}){
  ...
  const notifs=[
    ...
    ...invs.filter(i=>i.status==="partial").map(i=>({id:"pt"+i.id,k:"info",msg:`Part. payée : ${i.num} — Reste ${fmtf(Math.max(0,i.total-(i.amtPaid||0)))}`})),
      ...
      if(t.type==="sale"&&t.who){
        ...
        if(!existing){
          if(canAdd("cli")){
            ...
            clientId=newCli.id;
            toast(`👤 Client "${t.who}" créé automatiquement`,"ok",T.teal);
          }
        } else {
  ...
  const saveInv=async()=>{
    ...
    if(fm._edit){
      ...
      await supaInsert("invoices",{id:inv.id,user_id:uid,number:inv.num,client_name:inv.clientName,phone:inv.phone,total:inv.total,subtotal:inv.sub,tax:inv.tax,status:inv.status,issued:inv.issued,due:inv.due,items:JSON.stringify(inv.items),notes:inv.notes,amt_paid:0});
      // Marquer utilisateur actif
      markUserActive(uid);
    }
    setInvs(next);
...
<script>window.onload=()=>window.print();</script></body></html>`);
  ...
  // Coach IA — moteur local, sans dépendance réseau
  const sendAI=()=>{
    if(!cMsg.trim())return;
    const msg=cMsg.trim();
    setCMsg("");
    ...
    setTimeout(()=>{
      try{
        ...
        let r="";
        if(q.includes("vente")||q.includes("gagner")||q.includes("argent")||q.includes("revenu")){
          r=sales>0?`💰 Ce mois : ${fmtf(sales)} de ventes. `+(top?`Top client : ${top[0]} — ${fmtf(top[1])}. Relancez-le 📱.`:`Continuez à enregistrer chaque vente.`):`🎯 Aucune vente ce mois. Publiez dans "Action Rapide" 🌍`;
        }else if(q.includes("dépense")||q.includes("charge")||q.includes("depense")){
          r=exps>0?`📤 Dépenses : ${fmtf(exps)}. `+(profit>=0?`✅ Bénéfice : ${fmtf(profit)}.`:`⚠️ Déficit de ${fmtf(Math.abs(profit))}. Réduisez les charges.`):`📝 Notez vos charges pour connaître votre rentabilité.`;
        }else if(q.includes("bénéfice")||q.includes("profit")||q.includes("rentable")){
          r=profit>=0?`📈 Bénéfice : ${fmtf(profit)} — ${gPct}% de l'objectif. `+(gPct>=100?`🏆 Objectif dépassé !`:`Encore ${fmtf(Math.max(0,goal-profit))} à réaliser.`):`⚠️ Déficit de ${fmtf(Math.abs(profit))}. Augmentez les ventes ou réduisez les dépenses.`;
        }else if(q.includes("client")||q.includes("trouver")||q.includes("prospect")){
          r=`👥 ${ac} client${ac>1?"s":""} actif${ac>1?"s":""}. `+(ov>0?`Relancez vos ${ov} facture(s) en retard. `:``)+ `Publiez dans "Action Rapide" pour de nouveaux clients 🚀`;
        }else if(q.includes("facture")||q.includes("paiement")||q.includes("retard")){
          r=ov>0?`🔴 ${ov} facture${ov>1?"s":""} en retard. Allez dans "Factures" → 📲 WhatsApp → envoyez un rappel.`:invs.length>0?`✅ Toutes vos factures sont à jour !`:`📄 Créez votre première facture depuis "Factures".`;
        }else if(q.includes("objectif")||q.includes("cible")||q.includes("goal")){
          r=`🎯 Objectif : ${fmtf(goal)}/mois. Vous êtes à ${gPct}% avec ${fmtf(profit)} de bénéfice. `+(gPct>=100?`Félicitations 🏆`:gPct>50?`Encore ${fmtf(Math.max(0,goal-profit))} à réaliser.`:`Augmentez vos ventes 💪`);
        }else if(q.includes("mobile money")||q.includes("orange")||q.includes("wave")||q.includes("encaisser")){
          r=`💳 Bouton "💰 Gagner" → "Encaisser" → choisissez l'opérateur. `+(invs.filter(i=>i.status!=="paid").length>0?`${invs.filter(i=>i.status!=="paid").length} facture(s) non payée(s) → "Factures" → 💳 Payer.`:`Créez une facture puis envoyez le lien de paiement.`);
        }else if(q.includes("stratégie")||q.includes("conseil")||q.includes("améliorer")){
          r=[`🚀 3 priorités : 1) Relancez vos ${ac} clients 2) Publiez dans "Action Rapide" 3) Une facture par vente 🌍`,`💡 Notez chaque vente. En 30 jours vous verrez quels clients sont les plus rentables 📊`,`🎯 ${ac>0?`Contactez un client actif aujourd'hui pour une nouvelle commande.`:`Publiez votre service dans "Action Rapide".`}`][Math.floor(Date.now()/10000)%3];
        }else if(q.includes("bonjour")||q.includes("salut")||q.includes("hello")){
          r=`Bonjour ! 👋 `+(sales>0?`Ce mois : ${fmtf(sales)} ventes, ${fmtf(profit)} bénéfice. `:`Prêt à vous aider ! `)+`Posez votre question 🌍`;
        }else{
          r=[`📊 Situation : ${fmtf(sales)} ventes · ${fmtf(profit)} bénéfice · ${ac} clients actifs${ov>0?` · ${ov} facture(s) en retard`:""}. Sur quoi voulez-vous un conseil ?`,`🌍 Ventes : ${fmtf(sales)}, ${ac} clients. Posez votre question, je vous réponds concrètement.`,`💡 ${gPct}% de l'objectif atteint. Question sur ventes, dépenses, clients ou factures ?`][Math.floor(Date.now()/10000)%3];
        }
        setChat(h=>[...h,{r:"ai",t:r}]);
      }catch(e){
        setChat(h=>[...h,{r:"ai",t:"🌍 Posez votre question sur vos ventes, clients ou factures."}]);
      }finally{
        setCL(false);
      }
    },600);
  ...
  const NAV_BOTTOM=[{id:"action",ic:"⚡",lb:"Action"},{id:"dash",ic:"📊",lb:"Accueil"},{id:"inv",ic:"🧾",lb:"Factures"},{id:"reseau",ic:"🗺️",lb:"Réseau"},{id:"cli",ic:"👥",lb:"Clients"},{id:"prefs",ic:"⚙️",lb:"Compte"}];
...
function FeedCard({ c, i, accent, Tc, ratingMap, onRate, doCall, doWA, onAddClient, onCreateInvoice, onPayment, CATS_VIS, CAT_IMG }) {
  ...
  // ── Badge plan vérifié — affiché UNIQUEMENT si plan confirmé côté serveur ──
  // c.plan vient de commercants_profils.plan, qui est écrit par saveProfile()
  // en lisant supabase.auth.getUser() — jamais depuis le client
  const planBadge = (() => {
    if (c.plan === "business" && c.is_verified) {
      return { label:"Business", emoji:"🏆", col:"#f0b020", bg:"rgba(240,176,32,.15)" };
    }
    if (c.plan === "pro" && c.is_verified) {
      return { label:"Pro", emoji:"⚡", col:"#00d478", bg:"rgba(0,212,120,.12)" };
    }
    return null; // Free ou non vérifié → aucun badge plan
  })();
  ...
  return (
    <div style={{
      ...
      <div style={{ position:"relative", height:220, background:`linear-gradient(135deg,${accent}18,${Tc.c3})`, overflow:"hidden" }}>
        ...
        {/* Badge plan Pro/Business — uniquement si paiement confirmé côté serveur */}
        {planBadge && (
          <div style={{ position:"absolute", top:12, left: catObj?.id ? 130 : 12, background:planBadge.bg, backdropFilter:"blur(6px)", border:`1px solid ${planBadge.col}55`, borderRadius:20, padding:"4px 10px", fontSize:10, fontWeight:800, color:planBadge.col, display:"flex", alignItems:"center", gap:4 }}>
            {planBadge.emoji} {planBadge.label}
          </div>
        )}
        {c.ville && (
...
function CommerçantsProches({ user, supabase, accent="#00d478", toast, onAddClient, onCreateInvoice, onPayment }) {
  ...
  const saveProfile = async () => {
    ...
    try {
      const s = await supabase();
      // ── SÉCURITÉ : le plan stocké dans commercants_profils vient UNIQUEMENT
      // de Supabase Auth (user_metadata), jamais du client.
      // Cela garantit que le badge Pro/Business affiché dans le réseau
      // correspond à un paiement réellement confirmé.
      let verifiedPlan = "free";
      try {
        const {data:{user:authUser}} = await s.auth.getUser();
        verifiedPlan = authUser?.user_metadata?.plan || "free";
      } catch(e) { verifiedPlan = "free"; }

      const row = {
        id:          user?.id,
        nom:         user?.name || "Commerçant",
        business:    user?.business || "",
        activite:    editForm.activite.trim(),
        ville:       editForm.ville.trim(),
        pays:        user?.country || "CI",
        visible:     editForm.visible,
        phone:       editForm.phone || "",
        image_url:   previewImg || editForm.image_url || "",
        category:    editForm.category || "",
        // Plan vérifié côté serveur — ne JAMAIS accepter une valeur envoyée par le client
        plan:        verifiedPlan,
        is_verified: verifiedPlan === "pro" || verifiedPlan === "business",
      };
  ...
  const PgCoach=(()=>{
    ...
    function CoachPage(){
      ...
      const [localChat, setLocalChat] = useState([{r:"ai",t:`Bonjour ${ses.name?.split(" ")[0]||"entrepreneur"} 👋 Je suis votre Coach VierAfrik. Je suis là pour vous aider à gagner de l'argent ! Posez votre question.`}]);
      ...
      const handleSend = useCallback(() => {
        ...
        setTimeout(()=>{
          try{
            ...
            setLocalChat(h=>[...h,{r:"ai",t:"🌍 Posez votre question sur vos ventes, clients ou factures."}]);
          }finally{
            setLocalCL(false);
          }
        },600);
        ...
        <div style={{animation:"slideUp .3s ease both"}}>
          ...
          <div style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:14,padding:"1rem",marginBottom:9}}>
            <div style={{height:220,overflowY:"auto",display:"flex",flexDirection:"column",gap:9,marginBottom:9}}>
              ...
              {localCLoad&&<div style={{display:"flex"}}><div style={{background:T.c2,padding:"9px 12px",borderRadius:"3px 14px 14px 14px",fontSize:11,color:T.sub}}>⏳ Analyse en cours…</div></div>}
            ...
            <div style={{display:"flex",gap:7}}>
              ...
              />
              <Btn ch={localCLoad?"⏳":"→"} dis={localCLoad} onClick={handleSend} sx={{flexShrink:0}}/>
            </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",position:"relative",overflow:"hidden"}}>
      ...
      {/* Transaction */}
      <Modal open={mdl==="tx"} onClose={()=>{setMdl(null);setFm({});}} title={fm._edit?"✏️ Modifier la transaction":fm.type==="expense"?"📤 Nouvelle dépense":"💰 Nouvelle vente"}>
        ...
        {fm.type==="sale"&&(
          <FL l="Téléphone client (optionnel)" hint="Pour créer automatiquement la fiche client" ch={<input type="tel" style={IS} placeholder="+225 07 000 0000" value={fm.clientPhone||""} onChange={ev=>setFm(f=>({...f,clientPhone:ev.target.value}))}/>}/>
        )}
        ...
        {fm.type==="sale"&&(
          <div style={{background:`${T.gr}08`,border:`1px solid ${T.gr}20`,borderRadius:9,padding:"9px 12px",marginBottom:4,fontSize:11,color:T.sub2}}>
            ⚡ Une facture sera générée automatiquement · Le client sera créé s'il est nouveau
          </div>
        )}
      ...
      <Modal open={mdl==="encaisse"} onClose={()=>{setMdl(null);setFm({});}} title="💰 Encaisser un paiement">
        {fm.inv&&(
          <>
            <div style={{background:T.c2,border:`1px solid ${T.border}`,borderRadius:11,padding:"12px 14px",marginBottom:16}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginTop:8}}>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:9,color:T.sub,fontWeight:700,textTransform:"uppercase",marginBottom:3}}>Total</div>
                  <div style={{fontWeight:900,fontSize:15,color:T.text}}>{fmtk(fm.inv.total)}<span style={{fontSize:9}}> F</span></div>
                </div>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:9,color:T.sub,fontWeight:700,textTransform:"uppercase",marginBottom:3}}>Déjà payé</div>
                  <div style={{fontWeight:900,fontSize:15,color:T.teal}}>{fmtk(fm.inv.amtPaid||0)}<span style={{fontSize:9}}> F</span></div>
                </div>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:9,color:T.sub,fontWeight:700,textTransform:"uppercase",marginBottom:3}}>Reste</div>
            ...
            <div style={{display:"flex",gap:7}}>
              <Btn ch="💰 Confirmer encaissement" full onClick={()=>encaisserPaiement(fm.inv,fm.encMontant)}/>
              <Btn v="g" ch="Annuler" onClick={()=>{setMdl(null);setFm({});}}/>
            </div>
          </>
        )}
