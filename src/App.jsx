...
function Dashboard({ses,logout,updSes}){
  ...
  const [activityNotif,setActivityNotif]=useState(null);
  ...
  // Notifications d'activité — créent l'effet "app vivante"
  useEffect(()=>{
    const NOTIFS=[
    ...
    const show=()=>{
      ...
      setActivityNotif(n);
      setTimeout(()=>setActivityNotif(null),3800);
      idx++;
    };
    // Premier après 8s, puis toutes les 18s
...
Réponds en 2-3 phrases max. Français uniquement. Emojis africains. Sois direct et actionnable.`;
  ...
  const PgDash=()=>(
    <div>
      {/* ── SOCIAL PROOF BANNER — compteur + badges ── */}
      <div style={{background:`linear-gradient(135deg,${T.c1},${T.c2})`,border:`1px solid ${T.border}`,borderRadius:16,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <div>
            ...
            <div style={{fontSize:10,color:T.sub2}}>utilisent déjà VierAfrik en Afrique</div>
          </div>
        </div>
        <div style={{display:"flex",gap:6}}>
          {[{ic:"🔒",lb:"Sécurisé"},{ic:"🚀",lb:"Croissance"},{ic:"💰",lb:"Rentable"}].map(({ic,lb})=>(
            <div key={lb} style={{background:T.c3,border:`1px solid ${T.border}`,borderRadius:20,padding:"3px 10px",display:"flex",alignItems:"center",gap:4}}>
              <span style={{fontSize:11}}>{ic}</span>
              <span style={{fontSize:9,fontWeight:700,color:T.sub2}}>{lb}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  // ── Bouton flottant "Besoin d'aide ?" — visible sur toutes les pages sauf coach ──
  const HelpBtn=()=>{
    if(page==="coach")return null;
    return(
      <button onClick={()=>setPage("coach")}
        style={{position:"fixed",bottom:76,right:14,zIndex:600,background:`linear-gradient(135deg,${accent},${T.teal})`,border:"none",borderRadius:28,padding:"10px 16px",color:T.ink,fontFamily:"inherit",fontWeight:800,fontSize:11,cursor:"pointer",boxShadow:`0 6px 24px ${accent}55`,display:"flex",alignItems:"center",gap:7,letterSpacing:"-.01em",animation:"pop .4s cubic-bezier(.34,1.56,.64,1)"}}>
  ...
  const NAV_BOTTOM=[{id:"action",ic:"⚡",lb:"Action"},{id:"dash",ic:"📊",lb:"Accueil"},{id:"tx",ic:"💸",lb:"Ventes"},{id:"inv",ic:"🧾",lb:"Factures"},{id:"cli",ic:"👥",lb:"Clients"},{id:"reseau",ic:"🗺️",lb:"Réseau"},{id:"pay_mm",ic:"📱",lb:"Mobile Money"},{id:"plans",ic:"💎",lb:"Plans"},{id:"coach",ic:"🎥",lb:"Coach"},{id:"prefs",ic:"⚙️",lb:"Compte"}];
...
function CommerçantsProches({ user, supabase, accent="#00d478", toast, onAddClient, onCreateInvoice, onPayment }) {
  ...
  const PMAP={dash:<PgDash/>,tx:<PgTx/>,inv:<PgInv/>,cli:<PgCli/>,stats:<PgStats/>,coach:<PgCoach/>,plans:<PgPlans/>,prefs:<PgParams/>,ambass:<PgAmbassadeur/>,avis:<PgAvis/>,action:<PgActionRapide/>,reseau:<PgCommProches/>};
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",position:"relative",overflow:"hidden"}}>
      ...
      {/* ── NOTIFICATION D'ACTIVITÉ — pop-up bas gauche ── */}
      {activityNotif&&(
        <div style={{position:"fixed",bottom:76,left:14,zIndex:700,maxWidth:260,animation:"slideUp .4s cubic-bezier(.34,1.56,.64,1)",pointerEvents:"none"}}>
          <div style={{background:`linear-gradient(135deg,${T.c1},${T.c2})`,border:`1px solid ${T.border}`,borderRadius:14,padding:"10px 14px",boxShadow:"0 8px 32px rgba(0,0,0,.7)",display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:34,height:34,borderRadius:10,background:`${T.gr}18`,border:`1px solid ${T.gr}33`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{activityNotif.emoji}</div>
            <div>
              <div style={{fontSize:11,fontWeight:700,color:T.text,lineHeight:1.4}}>{activityNotif.msg}</div>
              <div style={{fontSize:10,color:T.gr,marginTop:1,fontWeight:600}}>VierAfrik · maintenant</div>
            </div>
          </div>
        </div>
      )}
