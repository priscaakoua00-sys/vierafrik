...
<script>window.onload=()=>window.print();</script></body></html>`);
  ...
  // Coach IA — via API Anthropic directe
  const sendAI=async()=>{
    if(!cMsg.trim())return;
    if(!plan.ai){
      const msg=cMsg.trim();setCMsg("");
      setChat(h=>[...h,{r:"user",t:msg},{r:"ai",t:"🔒 Le Coach IA avancé est disponible en plan Pro. Passez à Pro pour des conseils personnalisés en temps réel ! 🚀 En attendant : concentrez-vous sur vos clients actifs et ajoutez vos ventes quotidiennement 💪"}]);
      return;
    }
    const msg=cMsg.trim();setCMsg("");
          ...
          <div style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:14,padding:"1rem",marginBottom:9}}>
            ...
            <div style={{display:"flex",gap:7}}>
              <input style={{...IS,flex:1,marginTop:0,fontSize:12}}
                placeholder={plan.ai?"Votre question business…":"Posez une question (Pro pour réponses avancées)"}
