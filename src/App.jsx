...
function AuthPage({onLogin}){
  ...
  const go=()=>{
    ...
    setTimeout(()=>{
      ...
      if(tab==="signup"){
        // Inscription
        if(users[f.email.toLowerCase()]){
          setE({email:"Email déjà utilisé — connectez-vous"});setL(false);return;
        }
        const id=xid();
        const u={
          id,
        ...
        const u=users[email];
        if(!u){setE({email:"Aucun compte avec cet email"});setL(false);return;}
        if(u.password!==f.password){setE({password:"Mot de passe incorrect"});setL(false);return;}
...
export default function App(){
  ...
  useEffect(()=>{
    // Charger la session sauvegardée (connexion persistante)
    try{
      const saved=db.g("session");
      // Vérifier que la session est valide (a un id et un email)
  ...
  const updSes=upd=>{
    setSes(prev=>{
      ...
      const users=db.g("users")||{};
      if(users[ns.email]){
        users[ns.email]={...users[ns.email],...upd};
        db.s("users",users);
      }
      return ns;
    });
  ...
  // Non connecté
  if(!ses){
    return <AuthPage onLogin={u=>{db.s("session",u);setSes(u);}}/>;
  }
