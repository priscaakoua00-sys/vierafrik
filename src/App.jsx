...
const PLANS = {
  ...
  pro:     {label:"Pro",     emoji:"⚡",price:10,col:T.gr,   maxTx:INF,maxCli:INF,maxInv:INF,pdf:true,wa:true,mm:true,ai:true},
  business:{label:"Business",emoji:"🏆",price:25,col:T.gold, maxTx:INF,maxCli:INF,maxInv:INF,pdf:true,wa:true,mm:true,ai:true},
};
...
function AuthPage({onLogin}){
  ...
  const ok=()=>{
    const err={};
    if(!f.email?.includes("@"))err.email="Email invalide";
    if(tab!=="reset"&&(f.password||"").length<6)err.password="6 caractères minimum";
    if(tab==="signup"){if(!f.name)err.name="Nom requis";if(f.password!==f.confirm)err.confirm="Mots de passe différents";}
  ...
  const go=()=>{
    ...
    setTimeout(()=>{
      ...
      if(tab==="signup"){
        if(users[f.email]){setE({email:"Email déjà utilisé"});setL(false);return;}
        const id=xid();
        const u={id,name:f.name,email:f.email,password:f.password,business:f.business||"Mon Entreprise",country:"CI",plan:"free",accent:T.gr,goal:2500000,phone:""};
        users[f.email]=u;db.s("users",users);db.s("session",u);seed(id);onLogin(u);
      }else{
        const u=users[f.email];
        if(!u||u.password!==f.password){setE({password:"Identifiants incorrects"});setL(false);return;}
        db.s("session",u);onLogin(u);
      }
      setL(false);
    },650);
  };
        ...
        <div style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:20,padding:"1.8rem",boxShadow:"0 40px 100px rgba(0,0,0,.8)"}}>
          ...
          {done?(
            <div style={{textAlign:"center",padding:"1rem 0"}}>
              ...
              <div style={{fontSize:13,color:T.sub2,marginBottom:18}}>Consultez votre boîte mail</div>
              <Btn full ch="← Retour" onClick={()=>{setTab("login");setD(false);setF({});}}/>
            </div>
          ):(
            <>
              ...
              {tab==="signup"&&<FL l="Confirmer" err={e.confirm} ch={<input type="password" style={IS} placeholder="Répétez" value={f.confirm||""} onChange={s("confirm")} onKeyDown={ev=>ev.key==="Enter"&&go()}/>}/>}
        ...
        <div style={{marginTop:16,display:"flex",gap:7,justifyContent:"center",flexWrap:"wrap"}}>
          {Object.entries(PLANS).map(([k,p])=>(
            <div key={k} style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:12,padding:"8px 13px",textAlign:"center"}}>
              ...
              <div style={{fontWeight:900,fontSize:14,color:T.text,marginTop:1}}>{p.price===0?"Gratuit":`${p.price}€/mois`}</div>
            </div>
          ))}
        </div>
        <div style={{textAlign:"center",marginTop:12,fontSize:11,color:T.sub}}>🔒 Multi-tenant · Supabase RLS · SSL · RGPD</div>
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  const updSes=upd=>{
    const ns={...ses,...upd};setSes(ns);db.s("session",ns);
    const users=db.g("users")||{};
    if(users[ses?.email]){users[ses.email]={...users[ses.email],...upd};db.s("users",users);}
  };
...
function Dashboard({ses,logout,updSes}) {
  ...
  const saveTx=()=>{
    ...
    let next;
    if(isEdit)next=txs.map(x=>x.id===t.id?t:x);
    else{
  ...
  const saveCli=()=>{
    ...
    let next;
    if(fm._edit)next=clis.map(x=>x.id===c.id?c:x);
    else{
  ...
  const saveInv=()=>{
    ...
    let next;
    if(fm._edit)next=invs.map(x=>x.id===inv.id?inv:x);
    else{
...
Réponds en 2-3 phrases max. Français. Emojis africains.`,
  ...
  // Charts
  const chartD=[
    {m:"Jan",v:3200000,d:1750000},{m:"Fév",v:2900000,d:1500000},{m:"Mar",v:3700000,d:1950000},
    {m:"Avr",v:4200000,d:2050000},{m:"Mai",v:3900000,d:2100000},{m:"Jun",v:sales||4600000,d:exps||2250000},
  ];
          ...
          {rows.map(t=>(
            <tr key={t.id} style={{borderBottom:`1px solid ${T.border}`}}>
              ...
              {editable&&<td style={{padding:"7px"}}><div style={{display:"flex",gap:3}}>
                ...
                <button onClick={()=>{if(!window.confirm("Supprimer ?")){return;}const n=txs.filter(x=>x.id!==t.id);setTxs(n);save("tx_"+uid,n);toast("Supprimée","warn");}} style={{background:"rgba(255,34,85,.1)",border:"none",color:T.red,borderRadius:5,padding:"3px 8px",cursor:"pointer",fontSize:10}}>🗑</button>
              </div></td>}
            </tr>
          ))}
          ...
          {invs.map(inv=>(
            <div key={inv.id} style={{background:T.c2,border:`1px solid ${T.border}`,borderRadius:15,padding:"1rem",position:"relative",overflow:"hidden",transition:"all .2s"}}
              ...
              <div style={{display:"flex",gap:3}}>
                ...
                <button onClick={()=>{if(!window.confirm(`Supprimer ${inv.num} ?`)){return;}const n=invs.filter(x=>x.id!==inv.id);setInvs(n);save("inv_"+uid,n);toast("Supprimée","warn");}} style={{background:"rgba(255,34,85,.08)",border:"none",color:T.red,borderRadius:7,padding:"4px 9px",cursor:"pointer",fontSize:10}}>🗑</button>
              </div>
            </div>
          ))}
        ...
        {clis.map(cl=>(
          <div key={cl.id} style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:14,padding:"1.1rem",transition:"border-color .2s"}}
            ...
            <div style={{display:"flex",gap:4}}>
              ...
              <button onClick={()=>{if(!window.confirm(`Supprimer ${cl.name} ?`)){return;}const n=clis.filter(x=>x.id!==cl.id);setClis(n);save("cli_"+uid,n);toast(cl.name+" supprimé","warn");}} style={{background:"rgba(255,34,85,.1)",border:"none",color:T.red,borderRadius:7,padding:"5px 9px",cursor:"pointer",fontSize:10}}>🗑</button>
            </div>
          </div>
        ))}
    ...
    <div>
      ...
      {!plan.ai&&(
        <div style={{background:"rgba(240,176,32,.07)",border:"1px solid rgba(240,176,32,.2)",borderRadius:12,padding:"12px 14px",marginBottom:12}}>
          ...
          <Btn sm v="gold" ch="Passer à Pro — 10€/mois →" onClick={()=>setPage("plans")}/>
        </div>
      )}
        ...
        {Object.entries(PLANS).map(([k,p])=>{
          ...
          return(
            <div key={k} style={{background:T.c1,border:`2px solid ${isCur?p.col:T.border}`,borderRadius:18,padding:"1.4rem",position:"relative",overflow:"hidden",transition:"all .2s"}}
              ...
              <div style={{fontWeight:900,fontSize:28,marginBottom:14,letterSpacing:"-.03em"}}>{p.price===0?"Gratuit":`${p.price}€`}<span style={{fontSize:13,fontWeight:400,color:T.sub}}>{p.price>0?"/mois":""}</span></div>
              {feats.map(([feat,ok])=>(
              ...
              {!isCur&&<Btn full sx={{marginTop:15}} v={k==="business"?"gold":k==="pro"?"p":"g"} ch={`Passer à ${p.label} →`} onClick={()=>{updSes({plan:k});toast(`✅ Plan ${p.label} activé ! 🚀`);}}/>}
              {isCur&&k==="free"&&<Btn full sx={{marginTop:15}} v="out" ch="Passer à Pro — 10€/mois →" onClick={()=>{updSes({plan:"pro"});toast("✅ Plan Pro activé !");}}/>}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:9}}>
          ...
          <div style={{background:"rgba(255,34,85,.04)",border:"1px solid rgba(255,34,85,.13)",borderRadius:14,padding:"1.1rem"}}>
            ...
            <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
              ...
              <Btn sm v="d" ch="🔒 Déconnexion" onClick={()=>{if(window.confirm("Se déconnecter ?"))logout();}}/>
            </div>
          </div>
        </div>
      </div>
