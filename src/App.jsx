...
function CommerçantsProches({ user, supabase, accent="#00d478", toast, onAddClient, onCreateInvoice, onPayment }) {
  ...
  function PgActionRapide(){
    ...
    const BackBtn=({to})=>(
      <button type="button" onClick={(e)=>{e.stopPropagation();e.preventDefault();to();}} style={{background:"none",border:`1px solid ${T.border}`,borderRadius:8,color:T.sub2,padding:"4px 12px",cursor:"pointer",fontFamily:"inherit",fontSize:11,marginBottom:16,display:"inline-flex",alignItems:"center",gap:5}}>
        ← Retour
      </button>
    );
            ...
            {posts.map(p=>{
              ...
              return(
                <div key={p.id} style={{background:`linear-gradient(135deg,${T.c1},${T.c2})`,border:`1px solid ${T.border}`,borderRadius:16,overflow:"hidden",marginBottom:12}}>
                  {p.image_url&&<img src={p.image_url} alt="service" style={{width:"100%",height:155,objectFit:"cover",display:"block",pointerEvents:"none"}}/>}
                  <div style={{padding:"12px 14px"}}>
                    ...
                    <div style={{display:"flex",gap:7}}>
                      <button type="button" onClick={(e)=>{e.stopPropagation();e.preventDefault();setMdl("cli");setFm({name:p.phone,phone:p.phone,cat:p.category||"Services",status:"active"});}}
                      ...
                      <button type="button" onClick={(e)=>{e.stopPropagation();e.preventDefault();setMdl("inv");setFm({clientName:p.phone,phone:p.phone,items:[{id:xid(),name:c?.label||"Service",qty:1,price:0}]});}}
