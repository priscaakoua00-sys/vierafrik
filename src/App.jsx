...
// FedaPay — Clé publique (variable d'environnement Vercel uniquement)
const FEDAPAY_PK = (typeof window !== "undefined" && window.__ENV__?.VITE_FEDAPAY_PK) || "";
...
// ✅ Clé chargée depuis variable d'environnement Vite (jamais en dur dans le code)
// Vercel → Settings → Environment Variables → VITE_SUPABASE_ANON_KEY
const SUPA_KEY_DEFAULT = (typeof window !== "undefined" && window.__ENV__?.VITE_SUPABASE_ANON_KEY) || "";
// Permet un override temporaire via le panel diagnostic (localStorage) sans toucher le code
const getSupaKey = () => localStorage.getItem("vierafrik_supa_key") || SUPA_KEY_DEFAULT;
        ...
        {Object.entries(PLANS).map(([k,p])=>{
          ...
          return(
            <div key={k} style={{background:T.c1,border:`2px solid ${isCur?p.col:T.border}`,borderRadius:18,padding:"1.4rem",position:"relative",overflow:"hidden",transition:"all .2s"}}
              ...
              {!isCur&&<Btn full sx={{marginTop:15}} v={k==="business"?"gold":k==="pro"?"p":"g"} ch={`Passer à ${p.label} →`} onClick={async()=>{
                    ...
                    try{
                      const res=await fetch("/api/fedapay",{
                        method:"POST",
                        headers:{"Content-Type":"application/json"},
                        body:JSON.stringify({action:"initialize",amount:p.price,email:ses.email,description:"Abonnement VierAfrik "+p.label,plan:k,uid:ses.id})
                      });
                      const data=await res.json();
                      console.log("✅ [FedaPay] Plan upgrade:", data);
                      const url=data?.payment_url||data?.url||data?.transaction?.payment_url;
              ...
              {isCur&&k==="free"&&<Btn full sx={{marginTop:15}} v="out" ch={`Passer à Pro — ${fmtPriceShort(4900)}/mois →`} onClick={async()=>{
                    toast("⏳ Création du paiement…","ok");
                    try{
                      const res=await fetch("/api/fedapay",{
                        method:"POST",
                        headers:{"Content-Type":"application/json"},
                        body:JSON.stringify({action:"initialize",amount:4900,email:ses.email,description:"Abonnement VierAfrik Pro",plan:"pro",uid:ses.id})
                      });
                      const data=await res.json();
                      console.log("✅ [FedaPay] Pro upgrade:", data);
                      const url=data?.payment_url||data?.url||data?.transaction?.payment_url;
                      if(url){window.location.href=url;}
                      else{console.error("❌ [FedaPay] Réponse inattendue:",data);toast("❌ "+(data?.message||data?.error||"Erreur paiement — réessayez"),"err");}
                    }catch(e){console.error("❌ [FedaPay] Exception:",e);toast("❌ Erreur réseau — vérifiez votre connexion","err");}
