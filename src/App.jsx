...
Réponds en 2-3 phrases max. Français uniquement. Emojis africains. Sois direct et actionnable.`;
        ...
        {showPanel&&(
          <div onClick={()=>setShowPanel(false)}
            ...
            <div onClick={e=>e.stopPropagation()}
              style={{position:"fixed",bottom:140,right:14,zIndex:595,background:`linear-gradient(135deg,${T.c1},${T.c2})`,border:`2px solid ${accent}44`,borderRadius:20,padding:"16px",width:240,boxShadow:`0 20px 60px rgba(0,0,0,.8),0 0 0 1px ${accent}22`,animation:"pop .25s cubic-bezier(.34,1.56,.64,1)"}}>
              ...
              {[
                ...
                {ic:"📊",label:"Voir mes gains",      sub:"Dashboard",            col:T.gold, fn:()=>{setPage("dash");setShowPanel(false);}},
              ].map(({ic,label,sub,col,fn})=>(
                <button key={label} onClick={fn}
                  style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"10px 12px",marginBottom:7,background:`${col}12`,border:`1px solid ${col}33`,borderRadius:13,cursor:"pointer",fontFamily:"inherit",textAlign:"left",transition:"all .18s"}}
                  onMouseEnter={e=>{e.currentTarget.style.background=`${col}22`;}}
                  onMouseLeave={e=>{e.currentTarget.style.background=`${col}12`;}}>
                  <div style={{width:36,height:36,borderRadius:10,background:`${col}25`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{ic}</div>
                  <div>
                    <div style={{fontWeight:800,fontSize:12,color:T.text}}>{label}</div>
                    <div style={{fontSize:10,color:T.sub2,marginTop:1}}>{sub}</div>
                  </div>
                  <div style={{marginLeft:"auto",color:col,fontSize:14}}>→</div>
                </button>
              ))}
            </div>
          </div>
        )}
        {/* Bouton "Gagner maintenant" */}
        <button onClick={()=>setShowPanel(p=>!p)}
          style={{position:"fixed",bottom:76,right:14,zIndex:600,background:`linear-gradient(135deg,${accent},${T.teal})`,border:"none",borderRadius:28,padding:"11px 18px",color:T.ink,fontFamily:"inherit",fontWeight:900,fontSize:12,cursor:"pointer",boxShadow:`0 6px 24px ${accent}55`,display:"flex",alignItems:"center",gap:7,letterSpacing:"-.01em",animation:"pop .4s cubic-bezier(.34,1.56,.64,1)",transition:"all .2s"}}>
          {showPanel?"✕ Fermer":"💰 Gagner maintenant"}
        </button>
        {/* Bouton aide coach — plus discret */}
  ...
  const NAV_BOTTOM=[{id:"action",ic:"⚡",lb:"Action"},{id:"dash",ic:"📊",lb:"Accueil"},{id:"tx",ic:"💸",lb:"Ventes"},{id:"inv",ic:"🧾",lb:"Factures"},{id:"cli",ic:"👥",lb:"Clients"},{id:"carte",ic:"📇",lb:"Carte"},{id:"reseau",ic:"🗺️",lb:"Réseau"},{id:"pay_mm",ic:"📱",lb:"Mobile Money"},{id:"plans",ic:"💎",lb:"Plans"},{id:"coach",ic:"🎥",lb:"Coach"},{id:"prefs",ic:"⚙️",lb:"Compte"}];
  const NAV=[{id:"action",ic:"⚡",lb:"Action Rapide"},{id:"dash",ic:"📊",lb:"Dashboard"},{id:"tx",ic:"💸",lb:"Transactions"},{id:"inv",ic:"🧾",lb:"Factures"},{id:"cli",ic:"👥",lb:"Clients"},{id:"reseau",ic:"🗺️",lb:"Réseau"},{id:"stats",ic:"📈",lb:"Stats"},{id:"coach",ic:"🎥",lb:"Coach IA"},{id:"plans",ic:"💎",lb:"Plans"},{id:"ambass",ic:"🤝",lb:"Ambassadeur"},{id:"avis",ic:"⭐",lb:"Avis"},{id:"prefs",ic:"⚙️",lb:"Compte"}];



  // ── CarteVisite Component (persisté dans Supabase table "cartes_visite") ──
  function CarteVisite({ user, accent = "#00d478", toast }) {
      ...
      (async () => {
        try {
          ...
          if (data) {
            ...
            // Pas encore de carte — ouvrir formulaire
            setEditing(true);
          }
    ...
    const saveCarte = async () => {
      ...
      try {
        ...
        toast("✅ Carte de visite sauvegardée !");
        setEditing(false);
    ...
    return (
      <div>
        ...
        <div style={{ fontSize: 11, color: T.sub2, marginBottom: 18 }}>
          {carte ? "Votre carte est sauvegardée — elle ne disparaît jamais ✅" : "Créez votre carte de visite professionnelle"}
        </div>
        ...
        {!editing && carte && (
          <div ref={cardRef} style={{
            ...
            border: `2px solid ${form.couleur}44`,
            borderRadius: 20, padding: "1.6rem", marginBottom: 16,
            boxShadow: `0 20px 60px rgba(0,0,0,.6), 0 0 0 1px ${form.couleur}22`,
            position: "relative", overflow: "hidden",
          }}>
            {/* Décoration fond */}
            <div style={{ position: "absolute", top: -30, right: -30, width: 120, height: 120, borderRadius: "50%", background: `${form.couleur}08`, pointerEvents: "none" }}/>
            <div style={{ position: "absolute", bottom: -20, left: -20, width: 80, height: 80, borderRadius: "50%", background: `${form.couleur}06`, pointerEvents: "none" }}/>

            {/* Logo + Nom */}
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
              <div style={{
                width: 56, height: 56, borderRadius: 16,
                background: `linear-gradient(135deg, ${form.couleur}, #00bfcc)`,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontWeight: 900, fontSize: 22, color: "#000", flexShrink: 0,
                boxShadow: `0 6px 20px ${form.couleur}44`,
              }}>{initials}</div>
              <div>
                <div style={{ fontWeight: 900, fontSize: 18, color: "#fff", letterSpacing: "-.02em" }}>{form.business}</div>
                <div style={{ fontSize: 12, color: form.couleur, fontWeight: 600, marginTop: 1 }}>{form.activite || "Business"}</div>
              </div>
            </div>

            {/* Infos */}
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {form.nom && <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#dff0ff" }}>
                <span style={{ fontSize: 15 }}>👤</span> {form.nom}
              </div>}
              {form.phone && <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#dff0ff" }}>
                <span style={{ fontSize: 15 }}>📱</span> {form.phone}
              </div>}
              {form.ville && <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#dff0ff" }}>
                <span style={{ fontSize: 15 }}>📍</span> {form.ville}
              </div>}
            </div>

            {/* Footer carte */}
            <div style={{ marginTop: 16, paddingTop: 12, borderTop: `1px solid ${form.couleur}22`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        {/* FORMULAIRE */}
        {editing && (
          <div style={{ background: T.c1, border: `1px solid ${T.border}`, borderRadius: 16, padding: "1.4rem", marginBottom: 16 }}>
            <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 14, color: T.text }}>✏️ {carte ? "Modifier" : "Créer"} ma carte</div>
            {[
              ["Nom complet", "nom", "text", "Votre prénom et nom"],
              ["Nom du business", "business", "text", "Nom de votre entreprise"],
              ["Téléphone", "phone", "tel", "+225 07 000 0000"],
              ["Activité / Catégorie", "activite", "text", "Ex: Coiffure, Commerce, BTP..."],
              ["Ville", "ville", "text", "Ex: Abidjan, Dakar, Douala..."],
              ["WhatsApp", "whatsapp", "tel", "+225 07 000 0000"],
            ].map(([label, key, type, ph]) => (
              <div key={key} style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: T.sub2, marginBottom: 4, textTransform: "uppercase", letterSpacing: ".05em" }}>{label}</div>
                <input type={type} placeholder={ph} value={form[key] || ""} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                  style={{ width: "100%", padding: "9px 12px", background: T.c2, border: `1px solid ${T.border}`, borderRadius: 9, color: T.text, fontSize: 13, fontFamily: "inherit" }}/>
              </div>
            ))}
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: T.sub2, marginBottom: 6, textTransform: "uppercase", letterSpacing: ".05em" }}>Couleur</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {["#00d478","#1a78ff","#f0b020","#ff5a18","#9060ff","#ff2255","#00bfcc","#25D366"].map(c => (
                  <div key={c} onClick={() => setForm(f => ({ ...f, couleur: c }))}
                    style={{ width: 28, height: 28, borderRadius: "50%", background: c, cursor: "pointer", border: form.couleur === c ? "3px solid #fff" : "2px solid transparent", transition: "all .15s" }}/>
                ))}
              </div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={saveCarte} disabled={saving} style={{ flex: 1, padding: "10px", borderRadius: 9, border: "none", background: accent, color: "#000", fontWeight: 800, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>
                {saving ? "⏳ Sauvegarde..." : "✅ Sauvegarder"}
              </button>
              {carte && <button onClick={() => setEditing(false)} style={{ flex: 1, padding: "10px", borderRadius: 9, border: `1px solid ${T.border}`, background: T.c2, color: T.text, fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Annuler</button>}
            </div>
          </div>
        )}
        ...
        {!editing && carte && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={() => setEditing(true)} style={{ flex: 1, padding: "10px", borderRadius: 10, border: `1px solid ${T.border}`, background: T.c2, color: T.text, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>✏️ Modifier</button>
            <button onClick={shareWA} style={{ flex: 1, padding: "10px", borderRadius: 10, border: "none", background: "#25D366", color: "#000", fontWeight: 800, fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>💬 WhatsApp</button>
            <button onClick={() => { navigator.clipboard?.writeText(`${form.business}\n${form.phone}\n${form.ville}\nvierafrik.com`); toast("📋 Copié !"); }} style={{ flex: 1, padding: "10px", borderRadius: 10, border: `1px solid ${accent}44`, background: `${accent}12`, color: accent, fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>📋 Copier</button>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div style={{ fontFamily:"'Inter','Segoe UI',system-ui,sans-serif", color:T.text }}>
      ...
      <button onClick={saveLogo} disabled={saving} style={{
        ...
        {saving ? "⏳ Sauvegarde..." : saved ? "✅ Logo sauvegardé — Modifier" : "💾 Sauvegarder mon logo"}
      </button>
    </div>
  );
}
...
function CommerçantsProches({ user, supabase, accent="#00d478", toast, onAddClient, onCreateInvoice, onPayment }) {
            ...
            <div style={{marginBottom:13}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>
                {[
                  ...
                  <img key={i} src={img} alt="" onClick={()=>{setPropPreview(img);setPropForm(p=>({...p,imageUrl:img}));toast("✅ Image sélectionnée !");}}
  ...
  const PMAP={dash:<PgDash/>,tx:<PgTx/>,inv:<PgInv/>,cli:<PgCli/>,stats:<PgStats/>,coach:<PgCoach/>,plans:<PgPlans/>,prefs:<PgParams/>,ambass:<PgAmbassadeur/>,avis:<PgAvis/>,action:<PgActionRapide/>,reseau:<PgCommProches/>,carte:<PgCarteVisite/>};
