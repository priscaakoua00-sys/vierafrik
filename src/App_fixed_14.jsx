...
const supaInsert = async (table, data) => {
  try {
    ...
    if(error){
      console.error('Supabase insert error:', table, JSON.stringify(error));
      return false;
    }
    return true;
...
<script>window.onload=()=>window.print();</script></body></html>`);
  ...
  const PgInv=()=>{
    // ── Re-fetch automatique à chaque ouverture de la page Factures ──
    // Garantit que les factures sont toujours à jour depuis Supabase
    useEffect(()=>{
      (async()=>{
        try{
          const s=await getSupa();
