...
function Dashboard({ses,logout,updSes}){
        ...
        try{
          ...
          if(trxRef){
            try{
              ...
              console.warn("[FedaPay] Vérification échouée, fallback optimiste:", ve);
  ...
  const saveCli=async()=>{
    ...
    if(snap._edit){
      ...
      // 3. Re-fetch depuis Supabase pour synchroniser
      try{
        const s=await getSupa();
        const {data:fresh}=await s.from("clients").select("*").eq("user_id",uid).order("created_at",{ascending:false});
        if(fresh&&fresh.length>0){
          setClis(fresh.map(r=>({id:r.id,uid:r.user_id,name:r.name,phone:r.phone||"",email:r.email||"",pays:r.country||PAYS[0],cat:r.category||"Commerce",status:r.status||"active",ca:parseFloat(r.revenue)||0})));
          console.log("✅ Clients re-fetchés:", fresh.length);
        }
      }catch(e){console.warn("Re-fetch clients échoué:",e);}
      markUserActive(uid);
    }
  };
