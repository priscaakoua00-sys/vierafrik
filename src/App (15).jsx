...
function CommerçantsProches({ user, supabase, accent="#00d478", toast, onAddClient, onCreateInvoice, onPayment }) {
  ...
  // PgCommProches déplacé hors de Dashboard — voir PgReseauPage défini après Dashboard
  ...
  const PgCoach=useCallback(()=>{
    ...
    function CoachPage(){
      ...
      // ── Menu principal ──
      // ── Menu principal — composant stable avec state local pour l'input ──
  ...
  const PMAP=useMemo(()=>({
    ...
    reseau:<PgReseauPage ses={ses} accent={accent} toast={toast} setFm={setFm} setMdl={setMdl}/>,
...
// ════════════════════════════════
//  PgReseauPage — composant stable HORS Dashboard
//  Reçoit ses données via props → pas de remount à chaque frappe
// ════════════════════════════════
function PgReseauPage({ ses, accent, toast, setFm, setMdl }) {
  ...
  return (
    <div>
      ...
      {onglet==="forum" && (
        <ReseauCommerçants user={ses} supabase={getSupa} accent={accent} toast={toast}/>
      )}
    </div>
  );
}
