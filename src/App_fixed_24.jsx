...
function Dashboard({ses,logout,updSes}){
  ...
  // CRUD Tx
  const saveTx=async()=>{
    console.log("SAVE TX START", {amount:fm.amount, type:fm.type, who:fm.who});
    try {
      const amt=parseFloat(fm.amount);
      if(!amt||amt<=0){toast("Veuillez saisir un montant valide superieur a 0","err");return;}
      const isEdit=!!fm._edit;
      // FIX #1: renomme "t" en "tx" pour ne pas ecraser la fonction i18n t()
      const tx={id:isEdit?fm.id:xid(),uid,type:fm.type||"sale",amount:amt,cat:fm.cat||"Commerce",who:fm.who||"",date:fm.date||today(),note:fm.note||""};

      if(isEdit){
        // MODIFICATION
        const nextTxs=txs.map(x=>x.id===tx.id?tx:x);
        const ok=await supaUpdate("transactions",{type:tx.type,amount:tx.amount,category:tx.cat,who:tx.who,date:tx.date,note:tx.note,user_id:uid},tx.id);
        console.log("EDIT TX OK:", ok);
        setTxs(nextTxs);
        toast(t("txEdited"));
        setMdl(null);setFm({});
        return;
      ...
      // FIX #2: Mettre a jour le state LOCAL immediatement avant les appels reseau
      const nextTxs=[tx,...txs];
      setTxs(nextTxs);
      flashNew(tx.id);

      // FIX #3: Fermer le modal IMMEDIATEMENT — UX fluide
      setMdl(null);setFm({});

      // Toast succes
      const ns=nextTxs.filter(x=>x.type==="sale"&&mkey(x.date)===cm).reduce((s,x)=>s+x.amount,0)
      ...
      console.log("INSERT TX...");
      const txOk=await supaInsert("transactions",{id:tx.id,user_id:uid,type:tx.type,amount:tx.amount,category:tx.cat,who:tx.who,date:tx.date,note:tx.note});
      console.log("INSERT TX RESULT:", txOk);
      markUserActive(uid);

      // AUTO-CREATION CLIENT si vente avec nom
      if(tx.type==="sale"&&tx.who){
        const phoneClean=cleanP(fm.clientPhone||"");
        const existing=clis.find(c=>c.name.toLowerCase()===tx.who.toLowerCase()||
          (phoneClean&&cleanP(c.phone)===phoneClean));
        let clientId="";
        if(!existing){
          if(canAdd("cli")){
            const newCli={id:xid(),uid,name:tx.who,phone:fm.clientPhone||"",email:"",pays:PAYS[0],cat:tx.cat||"Commerce",status:"active",ca:amt};
            ...
            clientId=newCli.id;
            toast(t("newClient").replace("👤 ","")+": "+tx.who,"ok",T.teal);
          }
        } else {
        ...
        // GENERATION AUTOMATIQUE DE LA FACTURE
        if(canAdd("inv")){
          const invId=xid();
          const invNum=nextNum();
          const invItems=[{id:xid(),name:fm.note||tx.cat||"Vente",qty:1,price:amt,line:amt}];
          const inv={
            id:invId,uid,num:invNum,clientId,
            clientName:tx.who,phone:fm.clientPhone||existing?.phone||"",
            total:amt,sub:amt,tax:0,status:"pending",
            issued:tx.date,due:"",items:invItems,notes:"",
          ...
          const invOk=await supaInsert("invoices",{
            ...
            client_id:clientId,client_name:tx.who,
            phone:inv.phone,total:amt,subtotal:amt,tax:0,
            status:"pending",issued:tx.date,due:null,
            items:JSON.stringify(invItems),notes:"",amt_paid:0,
          });
          console.log("INSERT INVOICE RESULT:", invOk);
          if(invOk){
            setInvs(prev=>[inv,...prev]);
            toast("Facture "+invNum+" creee !","ok",T.blue);
          }
        }
      }
    } catch(err) {
      // FIX #4: Catch global — le modal ne reste jamais bloque
      console.error("saveTx ERROR:", err);
      toast("Erreur inattendue — reessayez","err");
      setMdl(null);setFm({});
    }
  };
