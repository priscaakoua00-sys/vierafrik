...
function Dashboard({ses,logout,updSes}){
  ...
  const saveTx=async()=>{
    ...
    // ✅ FIX: renommé "t" → "tx" pour ne pas écraser la fonction i18n t()
    ...
    let next;
    if(isEdit){
      next=txs.map(x=>x.id===tx.id?tx:x);
      await supaUpdate("transactions",{type:tx.type,amount:tx.amount,category:tx.cat,who:tx.who,date:tx.date,note:tx.note,user_id:uid},tx.id);
    } else {
      if(!canAdd("tx")){toast(`🔒 ${t("planFree")} — max ${plan.maxTx} tx. Pro 🚀`,"warn");return;}
      next=[tx,...txs];flashNew(tx.id);
      ...
      await supaInsert("transactions",{id:tx.id,user_id:uid,type:tx.type,amount:tx.amount,category:tx.cat,who:tx.who,date:tx.date,note:tx.note});
      markUserActive(uid);
      ...
      if(tx.type==="sale"&&tx.who){
        ...
        if(canAdd("inv")){
          ...
          if(invOk){
            ...
            console.error("Auto-facture non sauvegardée pour la vente:",tx.id);
          }
        }
      }
    }
