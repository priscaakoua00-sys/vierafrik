...
function CommerçantsProches({ user, supabase, accent="#00d478", toast, onAddClient, onCreateInvoice, onPayment }) {
  ...
  // ProfilePanel est mémoïsé pour éviter la recréation à chaque frappe (perte de focus)
  const ProfilePanel = useCallback(function ProfilePanel() { return (
        ...
        <div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
            <div>
              ...
              <input style={IS2} placeholder="Coiffure, Épicerie…" value={editForm.activite}
                onChange={e=>{ const v=e.target.value; setEditForm(f=>({...f,activite:v})); }}/>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="tel" style={IS2} placeholder="+225 07 000 0000" value={editForm.phone}
                onChange={e=>{ const v=e.target.value; setEditForm(f=>({...f,phone:v})); }}/>
            </div>
          </div>
          {/* Toggle visibilité */}
  ...
  }, [editOpen, myProfile, editForm, previewImg, saving, accent, Tc, IS2, VILLES_VIS, CATS_VIS, handleImg, saveProfile]);
  ...
  const PgCoach=(()=>{
    ...
    function CoachPage(){
      ...
      // ── Menu principal ──
      // ── Menu principal — utilise cMsg/setCMsg du Dashboard (niveau stable) ──
      function MenuPrincipal(){
        ...
        useEffect(()=>{ localChatRef.current?.scrollIntoView({behavior:"smooth"}); },[chat]);
          ...
          <div style={{background:T.c1,border:`1px solid ${T.border}`,borderRadius:14,padding:"1rem",marginBottom:9}}>
            ...
            <div style={{display:"flex",gap:7}}>
              <input
                ...
                value={cMsg}
                onChange={ev=>setCMsg(ev.target.value)}
                onKeyDown={ev=>{if(ev.key==="Enter"&&!cLoad){ev.preventDefault();handleSend();}}}
              />
              <Btn ch={cLoad?"⏳":"→"} dis={cLoad} onClick={handleSend} sx={{flexShrink:0}}/>
            </div>
