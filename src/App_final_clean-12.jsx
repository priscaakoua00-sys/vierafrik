...
// Utilise IS_AFRICA et DETECTED_CURRENCY calculés une seule fois au démarrage
const fmtPrice = (fcfa) => {
  if(fcfa === 0) return _globalLang === "fr" ? "Gratuit" : "Free";
  const base = `${fcfa.toLocaleString("fr-FR")} FCFA`;
  if(IS_AFRICA) return base;
...
// fmtPriceShort = alias de fmtPrice (suppression du doublon)
const fmtPriceShort = (fcfa) => fmtPrice(fcfa);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // Fonction SQL à créer dans Supabase pour contourner le RLS sur les lectures publiques
        if (!data && rlsBlocked) {
          ...
          try {
            ...
            if (!r3.error && r3.data) {
              ...
              rlsBlocked = false; // RPC a fonctionné
            }
          } catch(rpcErr) {
          ...
          <div style={{ textAlign:"center", padding:"32px 16px" }}>
            {error === "RLS_BLOCKED" ? (
              ...
              <>
                ...
                <div style={{ color:"#80a8c8", fontSize:13, marginBottom:16, lineHeight:1.6 }}>
                  Cette facture n'existe pas ou a été supprimée.<br/>
                  Vérifiez que le QR code est bien lié à une facture existante.
                </div>
                ...
                <div style={{ marginTop:12, fontSize:10, color:"#4a7090", fontFamily:"monospace" }}>ID : {invoiceId}</div>
              </>
            ) : (
              <>
                <div style={{ fontSize:48, marginBottom:12 }}>📡</div>
    ...
    setTimeout(() => {
      try {
        ...
        const top = Object.entries(
          txs.filter(tx => tx.type === "sale" && tx.who)
            .reduce((a, tx) => { a[tx.who] = (a[tx.who] || 0) + tx.amount; return a; }, {})
        ).sort((a, b) => b[1] - a[1])[0];
        ...
        if (has(kw.sale)) {
          r = isFr
            ...
            : (sales > 0 ? `💰 This month: ${fmtf(sales)} in sales. ` + (top ? `Top client: ${top[0]} — ${fmtf(top[1])}. Follow up 📱.` : `Keep recording every sale.`) : `🎯 No sales this month. Post in "Quick Action" 🌍`);
          ...
          r = isFr
            ...
            : (profit >= 0 ? `📈 Profit: ${fmtf(profit)} — ${gPct}% of goal. ` + (gPct >= 100 ? `🏆 Goal exceeded!` : `${fmtf(Math.max(0, goal - profit))} more to go.`) : `⚠️ Deficit of ${fmtf(Math.abs(profit))}. Increase sales or cut expenses.`);
          ...
          r = isFr
            ...
            : `👥 ${ac} active client${ac > 1 ? "s" : ""}. ` + (ov > 0 ? `Follow up on ${ov} overdue invoice${ov > 1 ? "s" : ""}. ` : "") + `Post in "Quick Action" to find new clients 🚀`;
          ...
          r = isFr
            ...
            : `💳 Tap "💰 Earn" → "Collect" → choose your operator. ` + (invs.filter(i => i.status !== "paid").length > 0 ? `${invs.filter(i => i.status !== "paid").length} unpaid invoice(s) → "Invoices" → 💳 Pay.` : `Create an invoice then send the payment link.`);
        } else if (has(kw.strategy)) {
          const opts = isFr
            ? [`🚀 3 priorités : 1) Relancez vos ${ac} clients 2) Publiez dans "Action Rapide" 3) Une facture par vente 🌍`, `💡 Notez chaque vente. En 30 jours vous verrez quels clients sont les plus rentables 📊`, `🎯 ${ac > 0 ? `Contactez un client actif aujourd'hui.` : `Publiez votre service dans "Action Rapide".`}`]
            : [`🚀 3 priorities: 1) Follow up with ${ac} clients 2) Post in "Quick Action" 3) One invoice per sale 🌍`, `💡 Log every sale. In 30 days you'll see which clients are most profitable 📊`, `🎯 ${ac > 0 ? `Reach out to an active client today.` : `Post your service in "Quick Action".`}`];
          ...
          const opts = isFr
            ...
            : [`📊 Summary: ${fmtf(sales)} sales · ${fmtf(profit)} profit · ${ac} active clients${ov > 0 ? ` · ${ov} overdue invoice${ov > 1 ? "s" : ""}` : ""}. What would you like advice on?`, `🌍 Sales: ${fmtf(sales)}, ${ac} clients. Ask me anything.`, `💡 ${gPct}% of your goal reached. Questions about sales, expenses, clients or invoices?`];
          r = opts[Math.floor(Date.now() / 10000) % 3];
        }

        setLocalChat(h => [...h, { r: "ai", t: r }]);
      } catch (e) {
        setLocalChat(h => [...h, { r: "ai", t: isFr ? "🌍 Posez votre question sur vos ventes, clients ou factures." : "🌍 Ask me about your sales, clients or invoices." }]);
      } finally {
        setLocalCL(false);
      }
    }, 600);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [localMsg, sales, exps, profit, gPct, goal, clis, invs, txs]);
...
function Dashboard({ses,logout,updSes}){
  ...
  const saveInv=async()=>{
    ...
    if(snap._edit){
      ...
      setMdl(null);setFm({});
      const ok=await supaUpdate("invoices",{client_name:inv.clientName,phone:inv.phone,total:inv.total,subtotal:inv.sub,tax:inv.tax,status:inv.status,issued:inv.issued,due:inv.due||null,items:JSON.stringify(inv.items),notes:inv.notes,user_id:uid},inv.id);
    ...
    setMdl(null);setFm({});
    // 2. INSERT Supabase
    console.log("💾 INSERT facture:", inv.num);
    const ok=await supaInsert("invoices",{id:inv.id,user_id:uid,number:inv.num,client_name:inv.clientName,phone:inv.phone,total:inv.total,subtotal:inv.sub,tax:inv.tax,status:inv.status,issued:inv.issued,due:inv.due||null,items:JSON.stringify(inv.items),notes:inv.notes,amt_paid:0});
    console.log("💾 INSERT facture résultat:", ok?"OK":"ERREUR");
    if(!ok){
      // Rollback si INSERT échoué
