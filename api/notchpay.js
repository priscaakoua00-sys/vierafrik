...
export default async function handler(req, res) {
  ...
  const PK = process.env.VITE_NOTCHPAY_
  ...
  try {
    ...
    if (action === "initialize") {
      ...
      const payload = {
        ...
        currency: "XOF",          // Franc CFA Afrique de l'Ouest — FORCÉ
        email: email,
        reference: ref,
        description: "VierAfrik Plan " + plan.charAt(0).toUpperCase() + plan.slice(1) + " — " + parseInt(amount).toLocaleString("fr-FR") + " FCFA/mois",
